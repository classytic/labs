...
import { CanvasLayer, Label, Segment, Stage, Vector, useCoords } from "@classytic/stage";

//#region src/physics/rain-relative/preset.tsx
/**
...
function Car({ baseY, wheelAngle }) {
	const [px, py] = useCoords().toPx(0, baseY);
		...
		children: [
			/* @__PURE__ */ jsx("defs", { children: /* @__PURE__ */ jsxs("linearGradient", {
				...
				children: [/* @__PURE__ */ jsx("stop", {
					offset: "0",
					stopColor: "color-mix(in oklab, var(--stage-accent-2) 82%, white)"
				}), /* @__PURE__ */ jsx("stop", {
					offset: "1",
					stopColor: "var(--stage-accent-2)"
				})]
			}) }),
			...
			/* @__PURE__ */ jsx("path", {
				d: "M -46 -47 Q -40 -64 -18 -64 L -4 -64 L -4 -47 Z",
				fill: "#dceeff",
				opacity: .92
			}),
			/* @__PURE__ */ jsx("path", {
				d: "M 2 -47 L 2 -64 L 24 -64 Q 44 -64 52 -47 Z",
				fill: "#dceeff",
				opacity: .92
			}),
			/* @__PURE__ */ jsx("ellipse", {
			...
			[-50, 54].map((wx) => /* @__PURE__ */ jsxs("g", { children: [
				/* @__PURE__ */ jsx("circle", {
					cx: wx,
					cy: -14,
					r: 16,
...
function RainRelativeLab({ maxSpeed = 10, start = 0, title = "Rain on a moving car", prompt = "Speed up, watch the rain slant. In your frame the rain comes at V_rain − V_car.", height = 360 }) {
	const [speed, setSpeed] = useState(start);
	const drops = useRef([]);
	const lastT = useRef(0);
	const wheelAngle = useRef(0);
	const rng = useRef(mulberry32(20240621));
	const reduce = useReducedMotion();
	const gate = usePlayGate();
	const ratio = speed / maxSpeed * 1.7;
	const thetaDeg = Math.round(Math.atan(ratio) * 180 / Math.PI);
	useFrameTick(gate.running && !reduce, (f) => {
		wheelAngle.current = (wheelAngle.current + speed * f.dtMs * .05) % 360;
	});
	const draw = (ctx, c) => {
		...
		let dt = (now - lastT.current) / 1e3;
		if (!(dt > 0) || dt > .1) dt = .016;
		lastT.current = now;
		const vy = RAIN_FALL_PX;
		const vx = -ratio * RAIN_FALL_PX;
	...
	const figure = /* @__PURE__ */ jsx(PlayWrap, {
		...
		children: /* @__PURE__ */ jsxs("div", {
			...
			children: [
				...
				/* @__PURE__ */ jsx(LiveRegion, { children: `Car speed ${speed}. Apparent rain ${thetaDeg} degrees from vertical.` })
			]
		})
	});
