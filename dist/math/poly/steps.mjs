...
import { deflate, degree, evalPoly, factorTex, polyTex, solve, trim } from "./core.mjs";

//#region src/math/poly/steps.ts
/**
...
const lin = (r, x) => r === 0 ? x : `(${x} ${r < 0 ? "+" : "-"} ${n2(Math.abs(r))})`;
/** coefficient × x-power, dropping a leading 1 (so "x^2", not "1x^2"). */
const cx = (c, xp) => Math.abs(c) === 1 && xp ? c < 0 ? `-${xp}` : xp : `${n2(c)}${xp}`;
/** Integer pair (u, v) with u·v = prod and u + v = sum, or null. */
function factorPair(prod, sum) {
	if (!Number.isInteger(prod) || !Number.isInteger(sum)) return null;
	const lim = Math.min(2e4, Math.abs(prod) + 1);
	for (let u = -lim; u <= lim; u++) {
		if (u === 0) {
			if (prod === 0 && v0(sum) === 0) return [0, sum];
			continue;
		}
		if (prod % u === 0) {
			const v = prod / u;
			if (u + v === sum) return [u, v];
		}
	}
	return null;
}
const v0 = (s) => s;
function quadratic(p, x) {
	const a = p[2], b = p[1], c = p[0];
	const w = calc().step(`${polyTex(p, x)} \\;=\\; a${x}^2 + b${x} + c`, `compare to ax² + bx + c: a = ${n2(a)} (the x² coefficient), b = ${n2(b)} (the middle), c = ${n2(c)} (the constant)`);
	const ac = a * c;
	...
	if (pair && [
		...
		const [u, v] = pair;
		w.step(`a \\times c = (${n2(a)})(${n2(c)}) = ${n2(ac)}`, "multiply the x² coefficient by the constant").step(`\\text{find } u, v:\\ \\ u \\cdot v = ${n2(ac)}\\ \\text{ and }\\ u + v = ${n2(b)} \\;\\Rightarrow\\; u = ${n2(u)},\\ v = ${n2(v)}`, `two numbers that multiply to a·c (${n2(ac)}) and add to b (${n2(b)})`).step(`${cx(a, `${x}^2`)} ${u < 0 ? "-" : "+"} ${n2(Math.abs(u))}${x} ${v < 0 ? "-" : "+"} ${n2(Math.abs(v))}${x} ${c < 0 ? "-" : "+"} ${n2(Math.abs(c))}`, `split the middle term ${n2(b)}${x} into ${n2(u)}${x} + ${n2(v)}${x}`).step(`= ${factorTex(p, x)}`, "group in pairs and take out the common factor");
		return w.done(0);
	}
	const disc = b * b - 4 * a * c;
	const sol = solve(p);
	w.step(`\\Delta = b^2 - 4ac = (${n2(b)})^2 - 4(${n2(a)})(${n2(c)}) = ${n2(disc)}`, disc < 0 ? "the discriminant is negative, so the roots are complex" : "the discriminant b² − 4ac (no whole-number pair exists, so use the formula)").step(`x = \\dfrac{-b \\pm \\sqrt{\\Delta}}{2a} = \\dfrac{${n2(-b)} \\pm \\sqrt{${n2(disc)}}}{${n2(2 * a)}}`, "substitute a, b and Δ into the quadratic formula").step(sol.roots.length === 2 ? `x = ${toStr(sol.roots[0])} \\text{ or } ${toStr(sol.roots[1])}` : `x = ${toStr(sol.roots[0])}`, "the two roots").step(`= ${factorTex(p, x)}`, "factored form");
	return w.done(0);
}
/** Step-by-step FACTORISATION of a polynomial. */
function factorSteps(p0, x = "x") {
	const p = trim(p0);
	const d = degree(p);
...
function solveSteps(p0, x = "x") {
	...
	if (degree(p) === 2) {
		...
		for (const s of inner.steps.slice(1)) w.step(s.tex, s.note);
	} else w.step(`${factorTex(p, x)} = 0`, "factor it (factor theorem)").step("\\text{each factor } = 0", "a product is 0 when a factor is 0");
