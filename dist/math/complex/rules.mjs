import { iPow, toTex } from "./core.mjs";
import { calc, texNum } from "../../kit/calc.mjs";

//#region src/math/complex/rules.ts
/**
...
*/
const sq = (x) => x < 0 ? `(${x})^2` : `${x}^2`;
/** |z| = √(a²+b²), with the working. */
function explainModulus(a, b) {
	const v = Math.hypot(a, b);
	return calc().step(`|z| = \\sqrt{a^2 + b^2} = \\sqrt{${sq(a)} + ${sq(b)}}`, "the distance of z from the origin").step(`= \\sqrt{${a * a + b * b}}`).step(`= ${texNum(Math.round(v * 100) / 100)}`).done(v);
}
/** iⁿ via the exact 4-cycle, with the working. */
function explainIPow(n) {
	const m = (Math.trunc(n) % 4 + 4) % 4;
	const z = iPow(n);
	return calc().step(`i^{${n}} = i^{(${n} \\bmod 4)} = i^{${m}}`, "powers of i repeat every 4").step(`= ${toTex(z)}`, [
		"1",
		"i",
		"-1",
		"-i"
...
const I_POWER_RULE = {
	...
	derivation: [
		...
		{
			tex: "i^2 = \\text{rotate } 180^\\circ = -1",
			note: "two quarter-turns"
		},
		{
			tex: "i^3 = -i, \\quad i^4 = 1",
			note: "three and four turns"
		},
		{
