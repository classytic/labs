...
//#region src/chem/gas-box/preset.tsx
/**
...
const H = 6, MAXV = 10, MINV = 2.5, R = .13, MAXN = 320;
const T0 = 300, SPEED0 = 5.5, TMIN = 80, TMAX = 600;
const PDISP = 1.5;
function GasBoxLab({ holdConstant = "none", particleCount = 180, temperature = 300, volume = 7, showGauge = true, title = "Gas in a box: watch pressure being made", prompt = "Drag the piston, heat it up, add molecules. Pressure is the drumbeat of hits on the walls.", height = 300, objectives }) {
	...
	useFrameTick(gate.running && !reduce, (f) => {
		const dt = Math.min(.032, f.dtMs / 1e3);
		const right = vol;
		let imp = 0;
		const ps = particles.current;
		for (let i = 0; i < n; i++) {
			const p = ps[i];
			const k = targetSpeed / (Math.hypot(p.vx, p.vy) || 1);
			p.vx *= k;
			p.vy *= k;
			p.x += p.vx * dt;
			p.y += p.vy * dt;
		...
		pressure.current = pressure.current * .9 + pInst * .1;
		if (mode === "pressure" && pTarget.current > 0) setVol((v) => clamp(v + clamp((pressure.current - pTarget.current) * .04, -3, 3) * dt, MINV, MAXV));
	...
	const pShow = mounted && reduce ? .0117 * n * temp / vol : pressure.current * PDISP;
	const draw = (ctx, c) => {
		...
		ctx.fillStyle = `hsl(${(232 - clamp((temp - TMIN) / (TMAX - TMIN), 0, 1) * 210).toFixed(0)} 78% 55%)`;
	...
	const onPiston = useMemo(() => mode === "none" || mode === "temperature" ? (m) => setVol(clamp(m[0], MINV, MAXV)) : void 0, [mode]);
	const pickMode = (next) => {
		if (next === "pressure") pTarget.current = pShow / PDISP;
		setMode(next);
	};
	const view = useMemo(() => ({
		xMin: -.3,
			...
			children: [
				...
				showGauge && /* @__PURE__ */ jsxs("div", {
					...
					children: [/* @__PURE__ */ jsxs("svg", {
						...
						children: [
							...
							(() => {
								...
								return /* @__PURE__ */ jsx("line", {
									x1: 42,
									y1: 46,
									x2: fmt(42 + Math.cos(a) * 30),
									y2: fmt(46 - Math.sin(a) * 30),
						...
						children: ["P ", pShow.toFixed(0)]
					})]
				}),
				/* @__PURE__ */ jsx(LiveRegion, { children: `Pressure ${pShow.toFixed(0)}, volume ${vol.toFixed(1)}, temperature ${Math.round(temp)} kelvin, ${n} molecules.` })
			]
		})
	});
	return /* @__PURE__ */ jsx(LabFrame, {
		...
		controls: /* @__PURE__ */ jsxs(ControlBar, { children: [
				...
				children: /* @__PURE__ */ jsxs("span", {
					...
					children: [
						...
						/* @__PURE__ */ jsx(Chip, {
							...
							onClick: () => pickMode("temperature"),
							children: "T (Boyle)"
						}),
						/* @__PURE__ */ jsx(Chip, {
							selected: mode === "volume",
							onClick: () => pickMode("volume"),
							children: "V (Gay-Lussac)"
						}),
						/* @__PURE__ */ jsx(Chip, {
							selected: mode === "pressure",
							onClick: () => pickMode("pressure"),
							children: "P (Charles)"
						})
					]
				})
			...
			/* @__PURE__ */ jsx(Field, {
				...
				children: /* @__PURE__ */ jsx(Slider, {
					...
					step: 10,
					onChange: setN,
					ariaLabel: "number of molecules"
				})
			}),
			/* @__PURE__ */ jsx(Field, {
				label: "V",
				value: `${vol.toFixed(1)} L`,
		...
		footer: /* @__PURE__ */ jsxs(Fragment$1, { children: [/* @__PURE__ */ jsx("p", {
			...
			children: mode === "volume" ? "piston locked" : mode === "pressure" ? "piston floats to hold P" : "drag the piston to set V"
		}), /* @__PURE__ */ jsx(ChallengeCard, {
			questions: GASBOX_CHALLENGE,
			state: challenge,
			title: "Predict the gas laws"
		})] }),
		children: figure
	});
}
