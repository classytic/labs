...
*/
const MAX_FLIGHT = 28;
function GaltonBoardLab({ rows = 12, seed = 7, showCurve = true, title = "Galton board", prompt, objectives, hints: hintList, controlId, height = 360 }) {
	const R = Math.max(4, Math.min(16, rows));
	const hints = useHints(hintList);
	const rng = useRef(mulberry32(seed));
	const counts = useRef(new Array(R + 1).fill(0));
	const balls = useRef([]);
	...
	const drop = useCallback((n) => {
		target.current = landed() + balls.current.length + n;
		setRunning(true);
	}, []);
	const landed = () => counts.current.reduce((a, b) => a + b, 0);
	useFrameLoop((frame) => {
		const dt = Math.min(.05, frame.dtMs / 1e3);
	...
	const draw = useCallback((ctx, _c) => {
		...
		tok("--stage-fg", "#222");
		const grid = tok("--stage-grid", "rgba(125,125,125,.3)"), muted = tok("--stage-muted", "#888"), accent = tok("--stage-accent", "#1c7ed6"), good = tok("--stage-good", "#2f9e44"), gold = tok("--stage-warn", "#e8a020");
		const W = ctx.canvas.clientWidth || 640, H = height;
		ctx.clearRect(0, 0, W, H);
		const dx = Math.min((W - 48) / (R + 1), 28);
		const cxC = W / 2;
		const pegTop = 26, binsTop = pegTop + H * .42, binsBottom = H - 22;
		const slotX = (r, slot) => cxC + (slot - r / 2) * dx;
		const yOf = (f) => pegTop + f / R * (binsTop - pegTop);
		ctx.fillStyle = muted;
		for (let r = 0; r < R; r++) for (let s = 0; s <= r; s++) {
			ctx.beginPath();
			ctx.arc(slotX(r, s) + dx / 2 * 0, pegTop + (r + .5) / R * (binsTop - pegTop), 1.8, 0, Math.PI * 2);
			ctx.fill();
		}
		...
		ctx.lineWidth = 1;
		ctx.beginPath();
		ctx.moveTo(slotX(R, 0) - barW, binsBottom + .5);
		ctx.lineTo(slotX(R, R) + barW, binsBottom + .5);
		ctx.stroke();
		if (curve && total > 0) {
			...
			for (let b = 0; b <= R; b++) {
				...
				const x = slotX(R, b), y = binsBottom - h;
		...
		for (const ball of balls.current) {
			...
			const x = x0 + (slotX(r + 1, ball.cum[r + 1]) - x0) * frac, y = yOf(ball.f);
			ctx.beginPath();
	...
	const total = counts.current.reduce((a, b) => a + b, 0);
	const mu = total ? mean(samples) : R / 2;
		...
		children: [/* @__PURE__ */ jsx("div", {
			...
			children: [
				/* @__PURE__ */ jsxs("span", {
					...
					children: ["balls: ", total.toLocaleString()]
				}),
				/* @__PURE__ */ jsxs("span", {
					style: {
					...
					children: [
						"mean ",
						/* @__PURE__ */ jsx(Tex$1, { tex: `\\approx ${mu.toFixed(2)}` }),
						" ",
						/* @__PURE__ */ jsxs("span", {
							style: {
							...
							children: [
								"(",
								/* @__PURE__ */ jsx(Tex$1, { tex: `\\to ${(R / 2).toFixed(1)}` }),
								")"
							]
						})
					]
				...
				/* @__PURE__ */ jsxs("span", {
					...
					children: [
						/* @__PURE__ */ jsx(Tex$1, { tex: `\\sigma \\approx ${sd.toFixed(2)}` }),
						" ",
						/* @__PURE__ */ jsxs("span", {
							style: {
								color: "var(--stage-muted)",
							...
							children: [
								"(",
								/* @__PURE__ */ jsx(Tex$1, { tex: `\\to ${(Math.sqrt(R) / 2).toFixed(2)}` }),
								")"
							]
						})
					]
