...
export function registerBuiltinTemplates(): void {
  registerLabTemplate({
    ...
    category: 'algebra',
    description:
      'Add unit weights until a balance is level, discover the hidden weight (the unknown) with no symbols. The essentials opener.',
    paramsSchema: z.object({ bucketWeight: z.number(), maxWeights: z.number().optional() }),
    defaultParams: { bucketWeight: 5, maxWeights: 10 },
    factory: (p) =>
      mysteryBucketDoc({
        bucketWeight: (p.bucketWeight as number) ?? 5,
        count: 0,
        maxWeights: p.maxWeights as number | undefined,
      }),
  });
