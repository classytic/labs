...
export function CircleLab({
  ...
  snap = 1,
  title = 'The circle',
  prompt = 'Drag the centre to move it; drag the rim to resize. (x − a)² + (y − b)² = r².',
  ask,
  activity = 'circle',
      ...
      {showTangent && (
        <>
          ...
          {tangent.vertical !== undefined ? (
            <Line
              from={{ x: tangent.vertical, y: 0 }}
              to={{ x: tangent.vertical, y: 1 }}
              color={C_TAN}
              weight={2.5}
            />
          ) : (
            <Line
              from={{ x: 0, y: tangent.c }}
              to={{ x: 1, y: tangent.c + tangent.m }}
              color={C_TAN}
              weight={2.5}
            />
          )}
          <RightAngleMark
            at={T}
            u={{ x: T.x - a, y: T.y - b }}
            v={tangent.vertical !== undefined ? { x: 0, y: 1 } : { x: 1, y: tangent.m }}
          />
          <MovableDot
            value={T}
  ...
  const aside = (
    <StatList>
      {readouts.map((rr, i) => (
        <Stat key={i} label={rr.label} value={rr.value} />
      ))}
    </StatList>
  );
  ...
  return (
    <Activity.Root>
      ...
      <Activity.Status>
        <strong>Circle model</strong>
        <span>{circleTex(a, b, r)}</span>
      </Activity.Status>
      <Activity.Workspace>
        <Activity.Canvas label="Interactive circle graph">{figure}</Activity.Canvas>
        <Activity.Inspector label="Circle measurements">{aside}</Activity.Inspector>
      </Activity.Workspace>
      <Activity.Feedback>
        <span>Observe</span>
        <div>Move the centre or radius handle and watch the equation preserve the same circle.</div>
      </Activity.Feedback>
      {footer ? (
        <section className="lab-authored-task" aria-label="Circle question">
          {footer}
        </section>
      ) : null}
      <Activity.LiveRegion>
        The circle has centre {num(a)}, {num(b)} and radius {num(r)}.
      </Activity.LiveRegion>
      <Activity.Transport>
        <div className="lab-transport-state">
          <strong>Explore the circle</strong>
          <span>{circleTex(a, b, r)}</span>
        </div>
      </Activity.Transport>
    </Activity.Root>
