...
export function PercentBarLab(props: PercentBarProps = {}): ReactNode {
  ...
  }, [start, snapPct]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const figure = (
    <Stage
      view={{ xMin: -7, xMax: 114, yMin: -2.4, yMax: segments?.length || referenceLabel ? 2.6 : 1.2 }}
      height={height}
    ...
    >
          ...
          {segSpans.length > 0 ? (
            segSpans.map((s, i) => (
              <g key={i}>
                ...
                />
                {s.label && s.x1 - s.x0 > 9 && (
                  <Label x={s.mid} y={Y_REF} text={s.label} color="var(--stage-fg)" size={11} weight={700} />
                )}
              </g>
            ))
          ) : (
      ...
      <Segment
        from={{ x: 0, y: Y_BAR }}
        to={{ x: Math.max(0.0001, pct), y: Y_BAR }}
        color={fillColor}
        weight={BARW}
      />
      ...
      {ticks.map((p) => (
        <Segment
          key={p}
          from={{ x: p, y: Y_BAR - 0.62 }}
          to={{ x: p, y: Y_BAR - (p % 25 === 0 ? 0.92 : 0.78) }}
          color="var(--stage-fg)"
          opacity={p % 25 === 0 ? 0.5 : 0.2}
          weight={p % 25 === 0 ? 1.5 : 1}
        />
      ))}
      ...
      {target != null && (
        <>
          <Segment
            from={{ x: target, y: Y_BAR - 0.95 }}
            to={{ x: target, y: Y_BAR + 0.95 }}
            color="var(--stage-good)"
            weight={1.75}
            opacity={solved ? 0.9 : 0.55}
            dashed
          />
  ...
  const controls = (
    <div className="lab-activity-fields">
      <span className="lab-numeric-primary">{pct}%</span>
      {showValue && (
  ...
  const twin =
    scene && scene !== 'none'
      ...
      : null;
  return (
    <Activity.Root className="math-percent-bar-activity" focusLayout="compact">
      <Activity.Header>
        <Activity.Heading eyebrow="Percentage model" title={title} description={prompt} />
      ...
      <Activity.Status>
        ...
        <span>whole {fmtVal(whole)}</span>
      </Activity.Status>
      <Activity.Workspace>
        <Activity.Canvas label="Interactive percentage model">
          <LinkedViews
            primary={figure}
            representations={twin ? <div className="lab-rep">{twin}</div> : undefined}
          />
        </Activity.Canvas>
        <Activity.Inspector label="Percentage and amount">{controls}</Activity.Inspector>
      </Activity.Workspace>
      <Activity.Feedback>
        <span>Observe</span>
        <div>
          The fill fraction and concrete amount stay synchronized: percentage ÷ 100 × whole gives the
          represented value.
        </div>
      </Activity.Feedback>
      <Activity.LiveRegion>
        {pct} percent represents {fmtVal(value)}
        {unit ? ` ${unit}` : ''} of {fmtVal(whole)}.{solved ? ' Target reached.' : ''}
      </Activity.LiveRegion>
      <Activity.Transport>
        <div className="lab-transport-state">
          <strong>{solved ? 'Target reached' : 'Move the fill handle'}</strong>
          <span>
            {pct}% of {fmtVal(whole)}
          </span>
        </div>
