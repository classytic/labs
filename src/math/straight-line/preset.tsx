...
import { RightAngleMark } from '../../kit/diagram/annotations.js';
import {
  CoordPlane,
  dotZone,
  clearOfLabel,
...
const PROMPTS: Record<StraightLineMode, string> = {
  ...
  perpendicular:
    'Drag P. The new line is perpendicular to the given one: its gradient is −1/m, so m₁·m₂ = −1.',
};
...
export function StraightLineLab(props: StraightLineProps = {}): ReactNode {
  ...
  let line: Lin;
  let scene: ReactNode;
  // Everything drawn that can sit on an axis, so the axis numbers step aside for it.
  let clear: ClearZone[] = [];
  ...
  if (mode === 'parallel' || mode === 'perpendicular') {
    ...
    clear = [
      dotZone(P),
      ...(cross ? [dotZone(cross, 8)] : []),
      clearOfLabel(P.x, P.y, `P (${num(P.x)}, ${num(P.y)})`, { size: 12, dx: 12, dy: -8, anchor: 'start' }),
    ];
    scene = (
      <>
        <Line
          from={{ x: 0, y: givenLine.c }}
          to={{ x: 1, y: givenLine.c + givenLine.m }}
          color={C_GIVEN}
          weight={2.5}
        />
        ...
        {cross && mode === 'perpendicular' && (
          <RightAngleMark
            at={cross}
            u={{ x: 1, y: givenLine.m }}
            v={line.vertical !== undefined ? { x: 0, y: 1 } : { x: 1, y: line.m }}
          />
        )}
    ...
    // Below the x-intercept the line occupies one side and leaves the other empty: rising to the
    // right, it is below the axis on the LEFT, so the label goes right, and the other way round.
    const aRight = line.m > 0;
    clear = [
      dotZone(A),
      dotZone(B),
      clearOfLabel(a, 0, `a = ${num(a)}`, {
        size: 12,
        dy: 16,
        dx: aRight ? 6 : -6,
        anchor: aRight ? 'start' : 'end',
      }),
      clearOfLabel(0, b, `b = ${num(b)}`, { size: 12, dx: 10, anchor: 'start' }),
    ];
    scene = (
      <>
        ...
        <MovableDot
          value={A}
          onMove={(p) => setA({ x: snapPoint(p, snap).x || 0.001, y: 0 })}
          snap={snap}
        ...
        <MovableDot
          value={B}
          onMove={(p) => setB({ x: 0, y: snapPoint(p, snap).y || 0.001 })}
          snap={snap}
          constrain="vertical"
          color={C_B}
  ...
  const figure = (
    <CoordPlane view={view} height={height} keepClear={clear} ariaLabel={`${title}: ${lineTex(line)}`}>
      {scene}
    </CoordPlane>
  );
  ...
  return (
    <Activity.Root className="math-straight-line-activity">
      <Activity.Header>
        <Activity.Heading eyebrow="Coordinate geometry" title={title} description={prompt} />
        <Activity.FocusButton />
      </Activity.Header>
      <Activity.Status>
        <strong>{mode.replaceAll('-', ' ')}</strong>
        <span>{lineTex(line)}</span>
      </Activity.Status>
      <Activity.Workspace>
        <Activity.Canvas label="Interactive straight-line graph">{figure}</Activity.Canvas>
        <Activity.Inspector label="Line measurements">{aside}</Activity.Inspector>
      </Activity.Workspace>
      <Activity.Feedback>
        <span>Observe</span>
        <div>
          Drag the marked geometry and compare which quantities change and which relationship is preserved.
        </div>
      </Activity.Feedback>
      {footer ? (
        <section className="lab-authored-task" aria-label="Straight-line question">
          {footer}
        </section>
      ) : null}
      <Activity.LiveRegion>The current line is {lineTex(line)}.</Activity.LiveRegion>
      <Activity.Transport>
        <div className="lab-transport-state">
          <strong>Explore the line</strong>
          <span>{lineTex(line)}</span>
        </div>
      </Activity.Transport>
    </Activity.Root>
