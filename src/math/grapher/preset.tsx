...
export function Grapher({
  ...
  const [dispMin, dispMax] =
    yRange === 'auto'
      ? autoRange(curves, values, xMin, xMax, disp)
      : isLog
        ? ([Math.log10(Math.max(1e-300, yRange[0])), Math.log10(Math.max(1e-300, yRange[1]))] as [
            number,
            number,
          ])
        : yRange;
  ...
  const spanX = xMax - xMin,
    spanD = dispMax - dispMin;
  const view = {
    xMin: xMin - spanX * 0.13,
    xMax: xMax + spanX * 0.02,
    yMin: dispMin - spanD * 0.15,
    yMax: dispMax + spanD * 0.05,
  };
  const xTicks = niceTicks(xMin, xMax, 6);
  const yTicks = isLog ? decadeTicks(dispMin, dispMax) : niceTicks(dispMin, dispMax, 5);
  const yLabel = (d: number): string =>
    isLog ? (Number.isInteger(d) ? `1e${d}` : `1e${d.toFixed(1)}`) : fmtNum(d);
  ...
  const figure = (
    <>
      ...
      >
        ...
        {curves.map((cu, i) =>
          cu.compiled ? (
            <Plot.OfX
              key={i}
              y={(x) => disp(cu.compiled!.fn({ ...values, x }))}
              domain={[xMin, xMax]}
              color={cu.color}
              weight={3}
            />
      ...
      {subtitle && <p className="math-grapher-subtitle">{subtitle}</p>}
    </>
  );
      ...
      <div className="lab-activity-fields">
        {params.map((p) => (
          <Field
            key={p.name}
            label={p.name}
            value={<strong className="lab-tabular">{(values[p.name] ?? p.value).toFixed(2)}</strong>}
          >
            <Slider
  ...
  const footer = (
    <>
      ...
      {errors.length > 0 && (
        <div className="math-grapher-errors">
          {errors.map((e, i) => (
            <div key={i}>
              “{e.expr}”, {e.error}
            </div>
  ...
  return (
    <Activity.Root className="math-grapher-activity">
      <Activity.Header>
        <Activity.Heading eyebrow="Function explorer" title={title || 'Graph'} />
        <Activity.FocusButton />
      </Activity.Header>
      <Activity.Status>
        <strong>{isLog ? 'Logarithmic scale' : 'Linear scale'}</strong>
        <span>
          {curves.length} {curves.length === 1 ? 'curve' : 'curves'}
        </span>
        <span>
          x: {fmtNum(xMin)} to {fmtNum(xMax)}
        </span>
      </Activity.Status>
      <Activity.Workspace>
        <Activity.Canvas label="Function graph">{figure}</Activity.Canvas>
        {controls ? <Activity.Inspector label="Parameters">{controls}</Activity.Inspector> : null}
      </Activity.Workspace>
      <Activity.Feedback>
        <span>Functions</span>
        <div>{footer}</div>
      ...
      <Activity.LiveRegion>
        {curves.length} functions shown from x {fmtNum(xMin)} to {fmtNum(xMax)}.
      </Activity.LiveRegion>
      <Activity.Transport>
        <div className="lab-transport-state">
          <strong>Explore the functions</strong>
          <span>{isLog ? 'logarithmic y-axis' : 'linear y-axis'}</span>
        </div>
      </Activity.Transport>
    </Activity.Root>
  );
