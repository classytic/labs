...
const SIGN_COLOR = (s: number): string =>
  s > 0 ? 'var(--stage-good)' : s < 0 ? 'var(--stage-danger)' : 'var(--stage-muted)';
...
const SignChip = ({ s }: { s: number }): ReactNode => (
  <span
    className="math-sign-chip"
    data-sign={s > 0 ? 'positive' : s < 0 ? 'negative' : 'zero'}
...
const CAST = [
  { q: 1, x: 0.45, y: 0.45, c: 'A' },
  { q: 2, x: -0.45, y: 0.45, c: 'S' },
  { q: 3, x: -0.45, y: -0.45, c: 'T' },
  { q: 4, x: 0.45, y: -0.45, c: 'C' },
] as const;
...
export function TrigSignsLab({
  ...
  useEffect(() => {
    setDeg(T.normDeg(snap(startDeg)));
  }, [startDeg]); // eslint-disable-line react-hooks/exhaustive-deps

  const rad = toRad(deg);
  const P: Vec2 = { x: Math.cos(rad), y: Math.sin(rad) };
  const q = T.quadrant(deg);
  const sSin = T.sign('sin', deg),
    sCos = T.sign('cos', deg),
    sTan = T.sign('tan', deg);
  ...
  const figure = (
    <Stage
      view={{ xMin: -1.5, xMax: 1.5, yMin: -1.45, yMax: 1.45 }}
      height={height}
  ...
  const Row = ({ fn, s }: { fn: T.TrigFn; s: number }): ReactNode => (
    <span
      className="lab-trig-value math-signed-value"
      data-sign={s > 0 ? 'positive' : s < 0 ? 'negative' : 'zero'}
    >
  ...
  const controls = (
    <div className="lab-activity-fields">
      <span className="lab-tabular">
        <Tex tex={`\\theta = ${deg}^\\circ = ${T.radTex(deg)}`} />
      </span>
      <Row fn="sin" s={sSin} />
      <Row fn="cos" s={sCos} />
      <Row fn="tan" s={sTan} />
      <span className="lab-muted-copy">
        {q ? `Q${q}: ${T.castLetter(deg)} positive` : 'on an axis'}
        {T.isSpecial(deg) ? ` · ref ${T.referenceAngleDeg(deg)}°` : ''}
      </span>
      {targetDeg != null && (
        <StatusPill ok={solved}>{solved ? `✓ ${targetDeg}°` : `target ${targetDeg}°`}</StatusPill>
      )}
    </div>
  );

  return (
    <Activity.Root className="math-trig-signs-activity">
      <Activity.Header>
        <Activity.Heading eyebrow="Trigonometric signs" title={title} description={prompt} />
        <Activity.FocusButton />
      </Activity.Header>
      <Activity.Status>
        <strong>{q ? `Quadrant ${q}` : 'On an axis'}</strong>
        <span>θ {deg}°</span>
        <span>{q ? `${T.castLetter(deg)} positive` : 'axis value'}</span>
      </Activity.Status>
      <Activity.Workspace>
        <Activity.Canvas label="Interactive CAST unit circle">{figure}</Activity.Canvas>
        <Activity.Inspector label="Angle and function signs">{controls}</Activity.Inspector>
      </Activity.Workspace>
      <Activity.Feedback>
        <span>Observe</span>
        <div>
          Cosine follows the horizontal coordinate and sine the vertical coordinate, so their signs come
          directly from the point’s quadrant.
        </div>
      </Activity.Feedback>
      <Activity.LiveRegion>
        Angle {deg} degrees. Sine {signTxt(sSin)}, cosine {signTxt(sCos)}, tangent {signTxt(sTan)}.
        {solved ? ' Target reached.' : ''}
      </Activity.LiveRegion>
      <Activity.Transport>
        <div className="lab-transport-state">
          <strong>{solved ? 'Target angle reached' : 'Drag around the circle'}</strong>
          <span>{T.isSpecial(deg) ? `reference angle ${T.referenceAngleDeg(deg)}°` : `θ ${deg}°`}</span>
        </div>
      </Activity.Transport>
    </Activity.Root>
  );
