...
const CAST = [
  { q: 1, x: 0.55, y: 0.55, c: 'A' },
  { q: 2, x: -0.55, y: 0.55, c: 'S' },
  { q: 3, x: -0.55, y: -0.55, c: 'T' },
  { q: 4, x: 0.55, y: -0.55, c: 'C' },
] as const;
...
export function UnitCircleMini({
  deg,
  showLegs = true,
  showValue = false,
  showCast = false,
  showHyp = false,
  size = 230,
}: UnitCircleMiniProps): ReactNode {
  const rad = toRad(deg);
  const P: Vec2 = { x: Math.cos(rad), y: Math.sin(rad) };
  const q = T.quadrant(deg);
  const sCos = T.sign('cos', deg),
    sSin = T.sign('sin', deg);
  const valOf = (fn: T.TrigFn): string =>
    T.exactTex(fn, deg) || (Math.round(T.evalTrig(fn, deg) * 100) / 100).toString().replace(/^-/, '−');
  const angleArc = arc(rad, 0.34);
  const sector = [{ x: 0, y: 0 }, ...angleArc, { x: 0, y: 0 }];
  const labelAngle = rad / 2;

  return (
    <div style={{ width: '100%', maxWidth: size }}>
      <Stage
        view={{ xMin: -1.35, xMax: 1.35, yMin: -1.3, yMax: 1.3 }}
        height={size}
      ...
      >
        ...
        <Polygon
          ...
          weight={0.75}
        />
        <Polyline points={angleArc} color="var(--stage-accent)" opacity={0.75} weight={2} />
        <Label
          x={0.5 * Math.cos(labelAngle)}
          y={0.5 * Math.sin(labelAngle)}
          text={`${T.normDeg(deg)}°`}
          color="var(--stage-accent)"
          size={10.5}
          weight={700}
        ...
        <Vector tail={{ x: 0, y: 0 }} tip={P} color="var(--stage-accent)" weight={2.25} />
        {showHyp && <Label x={P.x / 2} y={P.y / 2} text="1" color="var(--stage-fg)" size={11} dx={-8} />}
        <Dot x={P.x} y={P.y} r={5} color="var(--stage-accent)" />
      </Stage>
      {showValue && (
        <div className="math-trig-values">
          <span
            className="math-signed-value"
            data-sign={sCos > 0 ? 'positive' : sCos < 0 ? 'negative' : 'zero'}
          >
            cos = <Tex tex={valOf('cos')} />
          </span>
          <span
            className="math-signed-value"
            data-sign={sSin > 0 ? 'positive' : sSin < 0 ? 'negative' : 'zero'}
          >
            sin = <Tex tex={valOf('sin')} />
          </span>
        </div>
      )}
    </div>
...
export function SpecialTriangles({ size = 300 }: { size?: number }): ReactNode {
  return (
    <div style={{ width: '100%', maxWidth: size }}>
      <Stage
        view={{ xMin: -0.2, xMax: 4.2, yMin: -0.4, yMax: 2.2 }}
        height={size * 0.55}
