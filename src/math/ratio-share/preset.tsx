...
export function RatioShareLab(props: RatioShareProps = {}): ReactNode {
  ...
  const figure = (
    <Stage
      view={{ xMin: -total * 0.04, xMax: total * 1.12, yMin: -1.7, yMax: 1.5 }}
      height={height}
    ...
    >
      ...
      {split > total * 0.06 && (
        <Label x={split / 2} y={0} text={`${round(split)}`} color="var(--stage-fg)" size={13} weight={700} />
      )}
      {rest > total * 0.06 && (
        <Label
          x={(split + total) / 2}
      ...
      {/* target divider */}
      <Segment
        from={{ x: correct, y: -0.85 }}
        to={{ x: correct, y: 0.85 }}
        color="var(--stage-good)"
        weight={1.75}
        opacity={solved ? 0.9 : 0.5}
        dashed
      />
      {/* draggable divider */}
      <MovableDot
  ...
  const controls = (
    <div className="lab-activity-fields">
      <span className="lab-numeric-primary">
        {round(split)} : {round(rest)}
      </span>
      {split > 0 && rest > 0 && (
        <span className="lab-numeric-tertiary">
          = {round(split / sg)} : {round(rest / sg)}
        </span>
      )}
  ...
  const twin =
    scene && scene !== 'none'
      ...
      : null;
  return (
    <Activity.Root className="math-ratio-share-activity" focusLayout="compact">
      <Activity.Header>
        <Activity.Heading eyebrow="Proportional sharing" title={title} description={prompt} />
      ...
      <Activity.Status>
        ...
        <span>
          total {round(total)} {unit}
        </span>
      </Activity.Status>
      <Activity.Workspace>
        <Activity.Canvas label="Interactive ratio-sharing model">
          <LinkedViews
            primary={figure}
            representations={twin ? <div className="lab-rep">{twin}</div> : undefined}
          />
        </Activity.Canvas>
        <Activity.Inspector label="Ratio evidence">{controls}</Activity.Inspector>
      </Activity.Workspace>
      <Activity.Feedback>
        <span>Observe</span>
        <div>
          A valid split preserves both the whole and the relative number of equal parts assigned to each
          share.
        </div>
      </Activity.Feedback>
      <Activity.LiveRegion>
        {labelA} has {round(split)} and {labelB} has {round(rest)}; simplified ratio{' '}
        {split > 0 && rest > 0 ? `${round(split / sg)} to ${round(rest / sg)}` : 'not yet defined'}.
        {solved ? ' Target reached.' : ''}
      </Activity.LiveRegion>
      <Activity.Transport>
        <div className="lab-transport-state">
          <strong>{solved ? 'Ratio matched' : 'Move the divider'}</strong>
          <span>
            {round(split)} + {round(rest)} = {round(total)}
          </span>
        </div>
      </Activity.Transport>
    </Activity.Root>
