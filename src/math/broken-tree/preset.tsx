...
export function BrokenTreeLab({
  ...
  const lf = 0.34;
  const Lmid: Vec2 = {
    x: brk.x + (tip.x - brk.x) * lf + nx * 0.9,
    y: brk.y + (tip.y - brk.y) * lf + ny * 0.9,
  };
  ...
  const figure = (
    <Stage
      ...
      height={height}
      preserveAspect
      ariaLabel={`Broken tree: original height ${fmt(H)}, broke at ${fmt(h)}, top reaches ${fmt(d)} along the ground`}
    >
      ...
      <AngleArc
        at={tip}
        from={{ x: -1, y: 0 }}
        to={{ x: brk.x - tip.x, y: brk.y - tip.y }}
        rPx={30}
      ...
      <MovableDot
        value={brk}
        onMove={(p) => setBreakH(clamp(p.y, 0.5, H / 2 - 0.05))}
        color="var(--stage-accent)"
  ...
  const aside = (
    <div className="math-aside-stack">
      {hasGoal && (
        <StatusPill ok={reached} className="math-status-start" role="status">
          {reached
            ? '✓ The top reaches the house!'
            : `the top lands ${fmt(d)} m out, ${d > target ? 'break it higher' : 'break it lower'}`}
        </StatusPill>
      )}
      <StatList>
        <Stat label="original height H" value={fmt(H)} />
        <Stat label="broke at h" value={<span className="math-trunk-value">{fmt(h)}</span>} />
        <Stat label="fell top L" value={fmt(L)} />
        <Stat label="reaches d" value={fmt(d)} />
        <Stat label="angle θ" value={`${Math.round(deg)}°`} />
        <span className="math-explain">tan θ = h/d · L = H − h · d = √(L² − h²)</span>
      </StatList>
    </div>
  );

  return (
    <Activity.Root className="math-broken-tree-activity">
      <Activity.Header>
        <Activity.Heading eyebrow="Trigonometry" title={title} description={prompt} />
        <Activity.FocusButton />
      </Activity.Header>
      <Activity.Status>
        <strong>{reached ? 'Target reached' : 'Explore the break point'}</strong>
        <span>h {fmt(h)} m</span>
        <span>d {fmt(d)} m</span>
      </Activity.Status>
      <Activity.Workspace>
        <Activity.Canvas label="Broken-tree triangle model">{figure}</Activity.Canvas>
        <Activity.Inspector label="Triangle measurements">{aside}</Activity.Inspector>
      </Activity.Workspace>
      <Activity.Feedback>
        <span>Observe</span>
        <div>
          The stump, fallen top and ground form a right triangle. Moving the break changes every dependent
          length together.
        </div>
      </Activity.Feedback>
      <Activity.LiveRegion>
        The tree breaks at {fmt(h)} metres and lands {fmt(d)} metres from its base.
        {reached ? ' It reaches the house.' : ''}
      </Activity.LiveRegion>
      <Activity.Transport>
        <div className="lab-transport-state">
          <strong>{reached ? 'House reached' : 'Drag the break point'}</strong>
          <span>θ {Math.round(deg)}°</span>
        </div>
      </Activity.Transport>
    </Activity.Root>
  );
