...
export function TriangleTrig({
  ...
  const figure = (
    <Stage
      view={view}
      height={height}
      ariaLabel={`Right triangle: ${lab.angle} ${deg} degrees, ${lab.opposite} ${fmt(O)}, ${lab.adjacent} ${fmt(A)}, hypotenuse ${fmt(H)}`}
    >
      {/* sides */}
      <Segment
        from={Oc}
      ...
      {mode === 'depression' ? (
        <>
          ...
          <AngleArc
            at={T}
            from={{ x: horiz.x - T.x, y: 0 }}
            to={{ x: Oc.x - T.x, y: Oc.y - T.y }}
            rPx={30}
            label={`${lab.angle}=${deg}°`}
          />
        </>
      ...
      {sideLabel({ x: A / 2, y: 0 }, lab.adjacent, A, !givenIsOpp ? C_GIVEN : C_CALC, !givenIsOpp, {
        dy: 16,
      })}
  ...
  const aside = (
    <StatList>
      <Stat label={lab.angle} value={`${deg}°`} />
      <Stat label={lab.opposite} value={fmt(O)} />
      <Stat label={lab.adjacent} value={fmt(A)} />
      <Stat label="hypotenuse" value={fmt(H)} />
      <span className="math-explain">
        tan {lab.angle} = {fmt(O)}/{fmt(A)} = {fmt(O / A)}
      </span>
    </StatList>
  );

  const sliders: ReactNode[] = [];
  if (drive.includes('angle'))
    sliders.push(
      <Field key="a" label={lab.angle} value={`${deg}°`}>
        <Slider value={deg} min={5} max={85} step={1} onChange={setDeg} ariaLabel="angle in degrees" />
      </Field>,
    );
  if (drive.includes('leg'))
    sliders.push(
      <Field key="l" label={legKind} value={fmt(len)}>
        <Slider
          value={len}
          min={legMin}
          max={maxLeg}
  ...
  const controls = sliders.length ? <div className="lab-activity-fields">{sliders}</div> : undefined;
  ...
  return (
    <Activity.Root className="math-triangle-trig-activity">
      ...
      <Activity.Workspace>
        ...
        <Activity.Inspector label="Triangle measurements">
          {aside}
          {controls}
        </Activity.Inspector>
      </Activity.Workspace>
      <Activity.Feedback>
        <span>Observe</span>
        <div>
          The side opposite θ and the adjacent side determine tan θ; all three lengths remain linked as the
          inputs change.
        </div>
      </Activity.Feedback>
      {challenge && <section aria-label="Triangle question">{challenge}</section>}
      <Activity.LiveRegion>
        Angle {deg} degrees; opposite {fmt(O)}; adjacent {fmt(A)}; hypotenuse {fmt(H)}.
      </Activity.LiveRegion>
      <Activity.Transport>
        <div className="lab-transport-state">
          <strong>{drive.length ? 'Explore the triangle' : 'Authored scenario'}</strong>
          <span>tan θ = {fmt(O / A)}</span>
        </div>
      </Activity.Transport>
    </Activity.Root>
  );
