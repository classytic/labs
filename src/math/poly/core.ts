...
export function evalC(p: Poly, z: C.Complex): C.Complex {
  let s = C.ZERO;
  for (let i = p.length - 1; i >= 0; i--) s = C.add(C.mul(s, z), C.cx(p[i] ?? 0));
  return s;
}
...
function hasVar(node: Node, x: string): boolean {
  switch (node.type) {
    case 'var':
      return node.name === x;
    case 'neg':
      return hasVar(node.arg, x);
    case 'binary':
      return hasVar(node.left, x) || hasVar(node.right, x);
    case 'call':
      return node.args.some((a) => hasVar(a, x));
    default:
      return false;
  }
}
...
const snap = (z: C.Complex): C.Complex =>
  Math.abs(z.im) < 1e-7 ? { re: snapReal(z.re), im: 0 } : { re: snapReal(z.re), im: snapReal(z.im) };
...
export function allRoots(p0: Poly): C.Complex[] {
  ...
  let z: C.Complex[] = Array.from({ length: n }, (_, k) =>
    C.fromPolar(1 + (0.1 * k) / n, (2 * Math.PI * k) / n + 0.6),
  );
  ...
  roots.sort((a, b) =>
    a.im === 0 && b.im === 0 ? a.re - b.re : a.im === 0 ? -1 : b.im === 0 ? 1 : a.re - b.re || a.im - b.im,
  );
  return roots;
}
...
const linTex = (r: number, x: string): string =>
  r === 0 ? x : `(${x} ${r < 0 ? '+' : '−'} ${fnum(Math.abs(r))})`;
...
export function factorTex(p0: Poly, x = 'x'): string {
  ...
  for (let i = 0; i < sol.roots.length; i++) {
    ...
    used[i] = true;
    const j = sol.roots.findIndex(
      (w, k) => !used[k] && Math.abs(w.re - z.re) < 1e-6 && Math.abs(w.im + z.im) < 1e-6,
    );
    if (j >= 0) used[j] = true;
    const b2 = z.re * z.re + z.im * z.im,
      two = 2 * z.re;
