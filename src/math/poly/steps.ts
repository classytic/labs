...
const lin = (r: number, x: string): string =>
  r === 0 ? x : `(${x} ${r < 0 ? '+' : '-'} ${n2(Math.abs(r))})`;
/** coefficient × x-power, dropping a leading 1 (so "x^2", not "1x^2"). */
const cx = (c: number, xp: string): string =>
  Math.abs(c) === 1 && xp ? (c < 0 ? `-${xp}` : xp) : `${n2(c)}${xp}`;
...
function quadratic(p: Poly, x: string): Worked {
  ...
  if (pair && [a, b, c].every(Number.isInteger)) {
    ...
    w.step(`a \\times c = (${n2(a)})(${n2(c)}) = ${n2(ac)}`, 'multiply the x² coefficient by the constant')
      .step(
        `\\text{find } u, v:\\ \\ u \\cdot v = ${n2(ac)}\\ \\text{ and }\\ u + v = ${n2(b)} \\;\\Rightarrow\\; u = ${n2(u)},\\ v = ${n2(v)}`,
        `two numbers that multiply to a·c (${n2(ac)}) and add to b (${n2(b)})`,
      )
      .step(
        `${cx(a, `${x}^2`)} ${u < 0 ? '-' : '+'} ${n2(Math.abs(u))}${x} ${v < 0 ? '-' : '+'} ${n2(Math.abs(v))}${x} ${c < 0 ? '-' : '+'} ${n2(Math.abs(c))}`,
        `split the middle term ${n2(b)}${x} into ${n2(u)}${x} + ${n2(v)}${x}`,
      )
  ...
  const sol = solve(p)!;
  w.step(
    `\\Delta = b^2 - 4ac = (${n2(b)})^2 - 4(${n2(a)})(${n2(c)}) = ${n2(disc)}`,
    disc < 0
      ? 'the discriminant is negative, so the roots are complex'
      : 'the discriminant b² − 4ac (no whole-number pair exists, so use the formula)',
  )
    .step(
      `x = \\dfrac{-b \\pm \\sqrt{\\Delta}}{2a} = \\dfrac{${n2(-b)} \\pm \\sqrt{${n2(disc)}}}{${n2(2 * a)}}`,
      'substitute a, b and Δ into the quadratic formula',
    )
    .step(
      sol.roots.length === 2
        ? `x = ${toStr(sol.roots[0]!)} \\text{ or } ${toStr(sol.roots[1]!)}`
        : `x = ${toStr(sol.roots[0]!)}`,
      'the two roots',
    )
...
export function factorSteps(p0: Poly, x = 'x'): Worked {
  ...
  if (d <= 1)
    return calc()
      .step(polyTex(p, x), d === 1 ? 'already linear (a factor itself)' : 'a constant')
      .done(0);
  ...
  if (degree(cur) === 2)
    w.step(`${polyTex(cur, x)} \\Rightarrow \\text{factor the quadratic}`, 'split the middle term / formula');
...
export function solveSteps(p0: Poly, x = 'x'): Worked {
  ...
  if (degree(p) === 2) {
    ...
    w.step(`${factorTex(p, x)} = 0`, 'factor it (factor theorem)').step(
      '\\text{each factor } = 0',
      'a product is 0 when a factor is 0',
    );
  }
  const roots = sol.roots.map((r) => `${x} = ${toStr(r)}`).join(' \\text{ or } ');
  w.step(
    roots || '\\text{no roots}',
    sol.realRoots.length < sol.roots.length ? 'real and complex roots' : 'the roots',
  );
