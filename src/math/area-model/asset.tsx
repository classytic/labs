...
function resolver({ params, bound }: AssetResolveArgs): AssetGeometry {
  ...
  for (let k = 1; k < a; k++)
    grid.push([
      { x: o.x + xv + k * u, y: o.y },
      { x: o.x + xv + k * u, y: o.y + xv + b * u },
    ]);
  for (let k = 1; k < b; k++)
    grid.push([
      { x: o.x, y: o.y + xv + k * u },
      { x: o.x + xv + a * u, y: o.y + xv + k * u },
    ]);

  const w = xv + a * u;
  const h = xv + b * u;
  const sideBottom: SideLabel = {
    at: { x: o.x + w / 2, y: o.y },
    text: revealed ? `x${a ? ` + ${a}` : ''}` : 'x + ?',
  };
  const sideLeft: SideLabel = {
    at: { x: o.x, y: o.y + h / 2 },
  ...
  return {
    ...
    meta: {
      ...
      solved,
      revealed,
      expanded: { sq: 1, lin: a + b, con: a * b },
    },
  };
}

// Soft, harmonious tile palette (calm pastel fill + a matching, slightly stronger
// border), rather than the loud saturated brand gradients. Dark text reads cleanly
// on the pastels, and the three roles stay clearly distinct: the square (x²) indigo,
// the x-strips amber, the constant block a neutral slate.
const ROLE_STYLE: Record<Role, { fill: string; stroke: string }> = {
  x2: {
    fill: 'color-mix(in oklab, var(--stage-accent) 15%, var(--stage-bg))',
    stroke: 'color-mix(in oklab, var(--stage-accent) 42%, var(--stage-bg))',
  },
  xterm: {
    fill: 'color-mix(in oklab, var(--stage-accent-2) 20%, var(--stage-bg))',
    stroke: 'color-mix(in oklab, var(--stage-accent-2) 46%, var(--stage-bg))',
  },
  const: {
    fill: 'color-mix(in oklab, var(--stage-muted) 14%, var(--stage-bg))',
    stroke: 'color-mix(in oklab, var(--stage-muted) 38%, var(--stage-bg))',
  },
};
...
function Component({ geom }: { geom: AssetGeometry; style?: ElementStyle; label?: string }): ReactNode {
  ...
  const m = (geom.meta ?? {}) as {
    regions: Region[];
    grid: Array<[Vec2, Vec2]>;
    sideBottom: SideLabel;
    sideLeft: SideLabel;
    solved: boolean;
  };
  const P = (v: Vec2): [number, number] => c.toPx(v.x, v.y);
  const good = m.solved ? 'var(--stage-good)' : 'color-mix(in oklab, var(--stage-fg) 26%, transparent)';
    ...
    return (
      <g key={`r${i}`}>
        <rect
          ...
          width={wpx}
          height={hpx}
          rx={Math.min(8, Math.min(wpx, hpx) * 0.14)}
          fill={st.fill}
          stroke={st.stroke}
          strokeWidth={1.5}
        />
        <text
          className="math-svg-passive"
          x={lx + wpx / 2}
          y={ty + hpx / 2}
          fill="var(--stage-fg)"
          fontSize={fs}
        ...
        {(m.grid ?? []).map(([g0, g1], i) => {
          ...
          return (
            <line
              ...
              x2={x2}
              y2={y2}
              stroke="color-mix(in oklab, var(--stage-fg) 9%, transparent)"
              strokeWidth={1}
            />
          );
        })}
        {(['split', 'split2'] as const).map((key) => {
          ...
          return (
            <line
              ...
              x2={x2}
              y2={y2}
              stroke="color-mix(in oklab, var(--stage-fg) 20%, transparent)"
              strokeWidth={1.5}
            />
          );
        })}
