...
export function TrigExplorer({ functions, startDeg = 30 }: TrigExplorerProps = {}): ReactNode {
  ...
  useFrameLoop(
    (f) => {
      setTheta((prev) => (prev + (f.dtMs / 1000) * 0.9) % TWO_PI);
    },
  ...
  const figure = (
    <div ref={viewRef} className="lab-playwrap">
      ...
      >
        ...
        {fns.includes('cos') && (
          <Segment from={{ x: 0, y: 0 }} to={{ x: cosT, y: 0 }} color={FN_COLOR.cos} weight={2.5} />
        )}
        {fns.includes('sin') && (
          <Segment
            from={{ x: cosT, y: sinT }}
        ...
        {fns.map((fn) => (
          <Polyline key={fn} points={waveOf(fn)} color={FN_COLOR[fn]} weight={2.5} />
        ))}
        {fns.map((fn) => (
          <Dot key={`d-${fn}`} x={waveX(theta)} y={FN_EVAL[fn](theta)} r={3.5} color={FN_COLOR[fn]} />
        ))}
        {/* connector from circle to the lead wave front */}
        <Segment
          from={{ x: cosT, y: lead === 'cos' ? 0 : sinT }}
          to={{ x: waveX(theta), y: FN_EVAL[lead](theta) }}
          color="var(--stage-fg)"
          opacity={0.4}
          weight={1}
          dashed
  ...
  const controls = (
    <div className="lab-activity-fields">
      ...
      <span className="lab-trig-readouts">
        <span>
          <Tex tex={`\\theta\\ ${deg.toFixed(0)}^\\circ`} />
        </span>
        {fns.map((fn) => (
          <span key={fn} className="lab-trig-function" data-function={fn}>
            <Tex tex={`\\${fn}\\theta\\ ${FN_EVAL[fn](theta).toFixed(2)}`} />
          </span>
        ))}
      </span>
    </div>
  ...
  return (
    <Activity.Root className="math-trig-explorer-activity">
      <Activity.Header>
        <Activity.Heading
          eyebrow="Trigonometric motion"
          title="Trig Explorer"
          description="Drag the angle on the circle—or play the motion—to see sin and cos as its projections."
        />
        <Activity.FocusButton />
      </Activity.Header>
      <Activity.Status>
        <strong>{playing ? 'Tracing the circle' : 'Paused'}</strong>
        <span>θ {deg.toFixed(0)}°</span>
        <span>{fns.join(' + ')}</span>
      </Activity.Status>
      <Activity.Workspace>
        <Activity.Canvas label="Unit circle and unrolled waves">{figure}</Activity.Canvas>
      </Activity.Workspace>
      <Activity.Feedback>
        <span>Observe</span>
        <div>
          The horizontal projection is cosine and the vertical projection is sine; their wave traces come from
          the same rotating point.
        </div>
      </Activity.Feedback>
      <Activity.LiveRegion>
        Angle {deg.toFixed(0)} degrees. {fns.map((fn) => `${fn} ${FN_EVAL[fn](theta).toFixed(2)}`).join(', ')}
        .
      </Activity.LiveRegion>
      <Activity.Transport>{controls}</Activity.Transport>
    </Activity.Root>
  );
}
