...
export function ComplexPlaneLab(props: ComplexPlaneProps = {}): ReactNode {
  const {
    ...
    prompt = mode === 'roots'
      ? 'The nth-roots of unity, equally spaced on the unit circle.'
      : mode === 'multiply'
        ? 'Drag z. Multiplying by i rotates it 90°, do it twice and you land on −z.'
  ...
  useEffect(() => {
    setZ(start0());
  }, [props.start?.re, props.start?.im, mode, range]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const powers = useMemo(
    () => Array.from({ length: nPow }, (_, k) => C.powInt(Z, k + 1)),
    [Z.re, Z.im, nPow],
  ...
  const figure = (
    <CoordPlane
      view={view}
      height={height}
      step={mode === 'roots' || mode === 'multiply' ? 1 : undefined}
      ariaLabel={`Argand plane, z = ${C.toStr(Z)}`}
    >
      ...
      {mode === 'power' && (
        <>
          <Polyline
            points={[{ x: 0, y: 0 }, ...powers.filter(C.isFiniteC).map((p) => ({ x: p.re, y: p.im }))]}
          ...
          {powers.map(
            (p, k) =>
              C.isFiniteC(p) && (
                <g key={k}>
                  <Dot x={p.re} y={p.im} r={4} color="var(--stage-accent-2)" />
                  <Label
                    x={p.re}
                    y={p.im}
                    text={`z${['', '²', '³', '⁴', '⁵', '⁶'][k]}`}
      ...
      {mode !== 'roots' && (z.x !== 0 || z.y !== 0) && (
        <>
          ...
          <Polyline
            points={arc(thetaRad, Math.min(0.9, r * 0.45) + 0.4)}
      ...
      {mode !== 'roots' && target != null && !solved && (
        <Dot x={target.re} y={target.im} r={9} color="var(--stage-good)" opacity={0.35} />
      )}
  ...
  const controls =
    mode === 'roots' ? (
      <div className="lab-activity-fields">
        <span className="math-control-heading">
          z<sup>{nRoots}</sup> = 1 has {nRoots} roots
        </span>
        <span className="lab-numeric-tertiary">spaced {fmt(360 / nRoots, 1)}° apart on the unit circle</span>
      </div>
    ) : (
      <div className="lab-activity-fields">
        <span className="math-control-heading lab-tabular">z = {C.toStr(Z)}</span>
        <span className="lab-numeric-tertiary">|z| = {fmt(r)}</span>
        <span className="lab-numeric-tertiary">
          arg = {fmt(thetaDeg, 1)}° = {fmt(thetaRad)} rad
        </span>
        {mode === 'multiply' && <span className="math-explain">i·z = {C.toStr(iz)}</span>}
        {target != null && (
          <StatusPill ok={solved}>
            {solved
              ? `✓ ${C.toStr({ re: target.re, im: target.im })}`
              : `target ${C.toStr({ re: target.re, im: target.im })}`}
          </StatusPill>
        )}
      </div>
    );
  ...
  const observation =
    mode === 'roots'
      ? `${nRoots} roots divide the unit circle into equal ${fmt(360 / nRoots, 1)}° turns.`
      : mode === 'multiply'
        ? 'Multiplying by i preserves the modulus and adds a 90° anticlockwise rotation.'
        : mode === 'power'
          ? 'Successive powers multiply moduli and add arguments, producing a visible spiral.'
          : 'The real and imaginary components form a right triangle whose hypotenuse is the modulus.';

  return (
    <Activity.Root className="math-complex-plane-activity">
      <Activity.Header>
        <Activity.Heading eyebrow="Complex numbers" title={title} description={prompt} />
        <Activity.FocusButton />
      </Activity.Header>
      <Activity.Status>
        <strong>{mode === 'roots' ? `${nRoots} roots of unity` : `z = ${C.toStr(Z)}`}</strong>
        {mode !== 'roots' ? (
          <>
            <span>|z| {fmt(r)}</span>
            <span>arg {fmt(thetaDeg, 1)}°</span>
          </>
        ) : null}
      </Activity.Status>
      <Activity.Workspace>
        <Activity.Canvas label="Interactive Argand diagram">{figure}</Activity.Canvas>
        <Activity.Inspector label="Complex-number measurements">{controls}</Activity.Inspector>
      </Activity.Workspace>
      <Activity.Feedback>
        <span>Observe</span>
        <div>{observation}</div>
      ...
      <Activity.LiveRegion>
        {mode === 'roots'
          ? `${nRoots} roots equally spaced on the unit circle.`
          : `z is ${C.toStr(Z)}, modulus ${fmt(r)}, argument ${fmt(thetaDeg, 1)} degrees.`}
      </Activity.LiveRegion>
      <Activity.Transport>
        <div className="lab-transport-state">
          <strong>{solved ? 'Target reached' : mode === 'roots' ? 'Compare the rotations' : 'Move z'}</strong>
