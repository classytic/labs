...
function resolver({ params }: AssetResolveArgs): AssetGeometry {
  ...
  const pedestal: Vec2[] = [
    { x: pivot.x - 0.12, y: pivot.y - 0.05 },
    { x: pivot.x + 0.12, y: pivot.y - 0.05 },
    { x: pivot.x + 0.3, y: baseY },
    { x: pivot.x - 0.3, y: baseY },
  ];
  const foot: Vec2[] = [
    { x: pivot.x - 0.55, y: baseY },
    { x: pivot.x + 0.55, y: baseY },
  ];
...
function Component({ geom }: { geom: AssetGeometry; style?: ElementStyle; label?: string }): ReactNode {
  ...
  const toGlyph = (
    b: BucketBox | Coin,
  ): { cx: number; top: number; wpx: number; hpx: number; label: string } => {
  ...
  return (
    <>
      ...
      {(m.buckets ?? []).map((b, i) => (
        <BucketGlyph key={`b${i}`} {...toGlyph(b)} />
      ))}
      {(m.coins ?? []).map((coin, i) => (
        <WeightGlyph key={`c${i}`} {...toGlyph(coin)} />
      ))}
    </>
  );
}
