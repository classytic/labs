...
import { Stage, Grid, Plot, Vector, Segment, Dot, Label, type Vec2 } from '@classytic/stage';
...
import { MathActivity, MathResetTransport } from '../activity.js';
...
export function HarmonicFormLab({
  ...
  const phasor = (
    <Stage
      view={{ xMin: -PHV, xMax: PHV, yMin: -PHV, yMax: PHV }}
      height={280}
    ...
    >
      ...
      <PlaneAxes
        labels
        keepClear={showComp && a !== 0 ? [clearOfLabel(a / 2, 0, 'a', { size: 13, dy: 16 })] : []}
      />
  ...
  const waves = (
    <Stage
      view={{ xMin: -Math.PI, xMax: 2 * Math.PI, yMin: -WAV, yMax: WAV }}
      height={280}
      preserveAspect={false}
      ariaLabel="The cosine term, the sine term, and their combined single wave"
    >
      <Grid />
      <PlaneAxes labels keepClear={[clearOfLabel(peakX, 0, 'x = −α', { size: 11, dy: 16 })]} />
  ...
  const figure = (
    <div className="math-harmonic-layout">
      <div className="math-harmonic-phasor">{phasor}</div>
      <div className="math-harmonic-waves">{waves}</div>
    </div>
  );
  ...
  const aside = (
    <StatList>
      ...
      <Stat label="R = √(a² + b²)" value={R.toFixed(3)} />
      <Stat label="α" value={`${alpha.toFixed(3)} rad = ${((alpha * 180) / Math.PI).toFixed(1)}°`} />
    </StatList>
  );

  const controls = (
    <>
      <Field label="a (cos x)" value={a}>
        <Slider value={a} min={-LIM} max={LIM} step={1} onChange={setA} ariaLabel="coefficient of cosine" />
      </Field>
      <Field label="b (sin x)" value={b}>
        <Slider value={b} min={-LIM} max={LIM} step={1} onChange={setB} ariaLabel="coefficient of sine" />
      </Field>
      <Chip selected={showComp} onClick={() => setShowComp((v) => !v)}>
        show components
      </Chip>
    </>
  );
  ...
  return (
    <MathActivity
      ...
      status={
        <>
          <strong>One resultant wave</strong>
          <span>R {R.toFixed(3)}</span>
          <span>α {((alpha * 180) / Math.PI).toFixed(1)}°</span>
        </>
      }
      figure={figure}
      measurements={aside}
      ...
      conclusion={
        <>
          The two component waves combine into one wave with amplitude <TexHtml tex="R=\sqrt{a^2+b^2}" /> and
          phase shift <TexHtml tex="-\alpha" />.
        </>
      }
      transport={
        <MathResetTransport
          ...
          state="Drag either coefficient"
          detail={`R ${R.toFixed(3)} · α ${((alpha * 180) / Math.PI).toFixed(1)}°`}
        />
      }
      details={details}
      graphLabel="Linked phasor and component wave diagrams"
      inspectorLabel="Resultant amplitude and phase"
    />
  );
}
