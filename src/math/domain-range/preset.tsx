...
export function DomainRangeLab({
  ...
  const domain = coveredIntervals(
    pts.filter(([, y]) => Math.abs(y) <= yCap).map(([x]) => x),
    dx * 2.5,
  );

  // RANGE: a continuous domain piece covers [min, max] of its visible outputs (IVT), union the pieces.
  const range = mergeIntervals(
    domain.map(([a, b]): [number, number] | null => {
      let mn = Infinity,
        mx = -Infinity;
      for (const [x, y] of pts)
        if (x >= a - 1e-9 && x <= b + 1e-9 && y >= yMin && y <= yMax) {
          if (y < mn) mn = y;
          if (y > mx) mx = y;
        }
  ...
  const figure = (
    <CoordPlane
      view={{ xMin: x0, xMax: x1, yMin: yMin, yMax: yMax }}
      height={height}
    ...
    >
      ...
      {domain.map(([a, b], i) => (
        <Segment key={`d${i}`} from={{ x: a, y: 0 }} to={{ x: b, y: 0 }} color={C_DOMAIN} weight={7} />
      ))}
      {domain.length > 0 && (
        <Label
          x={domain[0]![0]}
      ...
      {range.map(([a, b], i) => (
        <Segment key={`r${i}`} from={{ x: 0, y: a }} to={{ x: 0, y: b }} color={C_RANGE} weight={7} />
      ))}
      {range.length > 0 && (
        <Label
          x={0}
  ...
  const aside = (
    <div className="math-aside-stack">
      <StatusPill ok={accepted} className="math-status-start" role="status">
      ...
      <StatList>
        ...
        />
        <Stat
          label={<span className="math-range-label">range</span>}
          value={fmtIntervals(range, (yMax - yMin) * 0.05)}
        />
        {restrict && (
          <span className="math-explain">
            restricted to [{num(restrict[0])}, {num(restrict[1])}]
          </span>
        )}
      </StatList>
    </div>
  ...
  const probeState = accepted ? `f(${num(pX)}) = ${num(pY)}` : `x = ${num(pX)} is undefined`;
  ...
  return (
    <Activity.Root>
      ...
      <Activity.Status>
        <strong>{accepted ? 'Input accepted' : 'Undefined input'}</strong>
        <span>{probeState}</span>
      </Activity.Status>
      <Activity.Workspace>
        <Activity.Canvas label="Domain and range graph">{figure}</Activity.Canvas>
        <Activity.Inspector label="Function evidence">{aside}</Activity.Inspector>
      </Activity.Workspace>
      <Activity.Feedback>
        <span>Observe</span>
        <div>The horizontal shadow records accepted inputs; the vertical shadow records their outputs.</div>
      </Activity.Feedback>
      {footer ? (
        <section className="lab-authored-task" aria-label="Domain and range question">
          {footer}
        </section>
      ) : null}
      <Activity.LiveRegion>
        {probeState}. Domain {fmtIntervals(domain, dx * 3)}. Range {fmtIntervals(range, (yMax - yMin) * 0.05)}
        .
      </Activity.LiveRegion>
      <Activity.Transport>
        <div className="lab-transport-state">
          <strong>{swept ? 'Range explored' : 'Sweep the input'}</strong>
          <span>{probeState}</span>
        </div>
      </Activity.Transport>
    </Activity.Root>
