...
export function applyTf(p: Vec2, t: Transform): Vec2 {
  ...
  switch (t.kind) {
    case 'translate':
      return { x: p.x + (t.by?.x ?? 0), y: p.y + (t.by?.y ?? 0) };
    case 'reflect':
      switch (t.axis ?? 'y') {
        case 'x':
          return { x: p.x, y: -p.y };
      ...
      return p;
    case 'rotate':
      return vec.rotateAbout(p, o, ((t.deg ?? 0) * Math.PI) / 180);
    case 'enlarge':
      return { x: o.x + (p.x - o.x) * (t.k ?? 1), y: o.y + (p.y - o.y) * (t.k ?? 1) };
  }
}

const AXIS_LABEL: Record<ReflectAxis, string> = {
  x: 'x-axis',
  y: 'y-axis',
  'y=x': 'y = x',
  'y=-x': 'y = −x',
...
export function TransformLab(props: TransformProps = {}): ReactNode {
  ...
  const {
    ...
    view = { xMin: -6, xMax: 6, yMin: -4, yMax: 6 },
    height = 380,
    title = 'Transformations',
    prompt = PROMPTS[kind],
    activity = `transform-${kind}`,
  } = props;
  ...
  if (kind === 'translate') {
    ...
    const lo = Math.min(-3, bx - 2, by - 2),
      hi = Math.max(3, bx + 2, by + 2);
    ...
    /**
     * The tiles are BUILT FROM THE ANSWER, never from a fixed list.
     *
     * Both of these used to offer a hard-coded set (90/180/270, and 2/3/4) while the schema accepts
     * any number, so an author could set k=5 or deg=45, pass every check, and ship an activity
     * whose correct tile was not on screen: unsolvable, with nothing to say so. The answer is always
     * in the pool now, and the usual choices join it as distractors.
     */
  } else if (kind === 'rotate') {
    const d = tf.deg ?? 90;
    slots = [{ id: 'deg', answer: `${d}°` }];
    tiles = [...new Set([d, 90, 180, 270, ...(props.distractors ?? [])])]
      .sort((a, b) => a - b)
      .map((v) => `${v}°`);
  } else {
    const k = tf.k ?? 2;
    slots = [{ id: 'k', answer: String(k) }];
    tiles = [...new Set([k, 2, 3, 4, ...(props.distractors ?? [])])]
      .filter((v) => v > 0)
      .sort((a, b) => a - b)
      .map(String);
  }
  ...
  const figure = (
    <CoordPlane view={view} height={height} stepX={1} stepY={1} ariaLabel={`${title}: ${kind}`}>
      ...
      {targets.map((t, i) => (
        <Circle key={`t${i}`} center={t} r={0.26} color={C_GHOST} weight={2} fill="none" />
      ))}
      ...
      {current.map((p, i) => (
        <Point key={`p${i}`} x={p.x} y={p.y} r={6} color={solved ? C_IMG : C_SRC} />
      ))}
  ...
  const instruction = (
    <span className="math-transform-instruction">
      {kind === 'translate' && (
        <>
          translate by ( <Blank fill={fill} id="dx" /> , <Blank fill={fill} id="dy" /> )
        </>
      )}
      {kind === 'reflect' && (
        <>
          reflect in the <Blank fill={fill} id="axis" width={72} />
        </>
      )}
      {kind === 'rotate' && (
        <>
          rotate <Blank fill={fill} id="deg" /> anticlockwise about O
        </>
      )}
      {kind === 'enlarge' && (
        <>
          enlarge by scale factor <Blank fill={fill} id="k" /> , centre O
        </>
      )}
    </span>
  );

  // Shell anatomy (lab-design §Shell): status strip → the task (instruction + tray) → figure →
  // ONE callout, only once there is something to say. The state used to be repeated in the
  // status strip, the transport row, the callout and the task ("Transformation complete" ×4).
  const state = solved ? 'Landed on the targets' : `Identify the ${kind}`;
  return (
    <Activity.Root className="lab-authored-activity" data-inspector-layout="stacked">
      <Activity.Header>
        <Activity.Heading eyebrow="Coordinate geometry" title={title} description={prompt} />
        <Activity.FocusButton />
      </Activity.Header>
      <Activity.Status>
        <strong>{solved ? 'complete' : 'predict'}</strong>
        <span>{state}</span>
      </Activity.Status>
      <section className="lab-authored-task" aria-label="Transformation instruction">
        <div className="math-transform-footer">
          {instruction}
          {!solved && <SlotTray fill={fill} />}
        </div>
      </section>
      <Activity.Workspace>
        <Activity.Canvas label="Transformation coordinate plane">{figure}</Activity.Canvas>
      </Activity.Workspace>
      {solved && (
        <Activity.Feedback>
          <span>Observe</span>
          <div>Every vertex lands on its ghost target, so the instruction describes the move exactly.</div>
        </Activity.Feedback>
      )}
      <Activity.LiveRegion>{state}.</Activity.LiveRegion>
    </Activity.Root>
  );
}
