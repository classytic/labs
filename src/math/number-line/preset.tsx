...
export function NumberLineLab({
  ...
  const figure = (
    <Stage
      view={{ xMin: min - 0.8, xMax: max + 0.8, yMin: -1.3, yMax: 1.3 }}
      height={height}
    ...
    >
      ...
      {ticks.map((t) => (
        <Segment
          key={`t${t}`}
          from={{ x: t, y: t === 0 ? -0.26 : -0.16 }}
          to={{ x: t, y: t === 0 ? 0.26 : 0.16 }}
          color="var(--stage-fg)"
  ...
  return (
    <Activity.Root className="math-number-line-activity" focusLayout="compact">
      <Activity.Header>
        <Activity.Heading eyebrow="Number sense" title={title} description={prompt} />
        <Activity.FocusButton />
      </Activity.Header>
      <Activity.Status>
        <strong>{solved ? 'Target reached' : 'Explore the line'}</strong>
        <span>value {val}</span>
      ...
      <Activity.Feedback>
        ...
        <div>
          {solved
            ? `The marker is exactly on ${target}.`
            : 'Values increase to the right and decrease to the left of zero.'}
        </div>
      </Activity.Feedback>
      <Activity.LiveRegion>{update}</Activity.LiveRegion>
      <Activity.Transport>
