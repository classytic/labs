...
export function DigitWheel({
  ...
  return (
    <svg
      ...
      onClick={onTap}
      className="ict-digit-glyph"
      data-interactive={onTap ? true : undefined}
    >
      <defs>
        <clipPath id={clipId}>
          <rect x={0} y={0} width={CELL_W} height={CELL_H} rx={7} />
        </clipPath>
      </defs>
      <rect
        x={0.75}
        ...
        strokeWidth={1.5}
      />
      <g className="ict-digit-content" clipPath={`url(#${clipId})`} fontWeight={700} textAnchor="middle">
        <g transform={`translate(0 ${offset})`}>
          ...
          <text x={cx} y={cy} fill={col} fontSize={26} dominantBaseline="central">
            {digitChar(digit)}
          </text>
          <text
            x={cx}
...
export function WheelRow({
  ...
  return (
    <div ref={viewRef} className="ict-wheel-row">
      {digits.map((d, i) => {
        const place = width - 1 - i;
        const weight = Math.pow(base, place);
        return (
          <div key={i} className="ict-wheel-place">
            {cells ? (
              <BitCell
                on={d === 1}
                onTap={onTapDigit ? () => onTapDigit(i) : undefined}
            ...
            {showWeights && (
              <div className="ict-place-weight">
                <div className="ict-place-power">
                  {base}
                  <sup>{place}</sup>
                </div>
                <div className="ict-place-value" data-active={d !== 0 || undefined}>
                  {weight}
                </div>
              </div>
            )}
