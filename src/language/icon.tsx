...
export function registerLabIcon(
  id: string,
  render: (p: { size: number; title?: string }) => ReactNode,
): void {
...
export function Icon({ icon, size = 36, className, style, decorative }: IconProps): ReactNode {
  ...
  if (ref.kind === 'image' && ref.src) {
    return (
      <img
        ...
        className={['lang-icon-image', className].filter(Boolean).join(' ')}
        width={size}
        height={size}
        loading="lazy"
        decoding="async"
  ...
  if (ref.kind === 'svg' && ref.id) {
    ...
    if (render)
      return (
        <span className={['lang-icon-svg', className].filter(Boolean).join(' ')} style={style} {...a11y}>
          {render({ size, title: labelled ? ref.alt : undefined })}
        </span>
      );
  ...
  return (
    <span
      className={className}
      style={{ ...(className ? null : { fontSize: size, lineHeight: 1 }), ...style }}
      {...a11y}
    >
      {char}
    </span>
