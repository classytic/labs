...
const GREEN = 'var(--stage-good)',
  ORANGE = 'var(--stage-warn)',
  POS = '#e03131',
  NEG = '#1c7ed6';

export function LorentzForceLab({
  charge = 1,
  fieldOut = true,
  B: B0 = 1.4,
  speed: v0 = 2,
  ...
  const sense = cyclotronSense(q, out ? 1 : -1);
  useFrameTick(gate.running && mounted && !reduce, (f) => {
    theta.current += sense * B * (f.dtMs / 1000) * 0.9;
  });

  const r = Math.max(0.7, Math.min(4.2, cyclotronRadius(1, v, q, B)));
  ...
  const aside = (
    <>
      <div className="physics-probe">
        <div className="physics-formula-lead">
          <Tex tex="F = q\,v \times B" />
        </div>
        <div className="physics-probe-verdict">curves {sense > 0 ? 'counter-clockwise' : 'clockwise'}</div>
        <div className="physics-probe-detail">
          radius <Tex tex="r = \tfrac{mv}{qB}" /> = {r.toFixed(2)}
        </div>
      </div>
      <p className="physics-explain">
        <b>Right-hand rule:</b> fingers point along <b className="physics-vector-velocity">v</b>, curl toward{' '}
        <b>B</b> ({out ? 'out ⊙' : 'in ⊗'}), thumb = <b className="physics-vector-force">F</b> (for +q;
        reverse for −q). F always ⟂ v, so it only turns the charge, the speed never changes.
      </p>
    </>
  );
  ...
  const controls = (
    <>
      ...
      <Field label="B strength" value={B.toFixed(1)}>
        <Slider value={B} min={0.6} max={3} step={0.1} onChange={setB} ariaLabel="field strength" />
      </Field>
  ...
  return (
    <FieldActivity
      ...
      status={
        <>
          <span>{q > 0 ? 'Positive charge' : 'Negative charge'}</span>
          <span>B {out ? 'out ⊙' : 'in ⊗'}</span>
          <span>r {r.toFixed(2)}</span>
          <span>{sense > 0 ? 'counter-clockwise' : 'clockwise'}</span>
        </>
      }
      figure={figure}
      evidence={aside}
      controls={controls}
      observation="F stays perpendicular to v, so the field turns the particle without changing its speed."
      objectives={objectives}
      support={<HintLadder hints={hints} />}
      transcript={
        <p>{`${q > 0 ? 'Positive' : 'Negative'} charge curves ${sense > 0 ? 'counter-clockwise' : 'clockwise'} with radius ${r.toFixed(2)} in a field pointing ${out ? 'out of' : 'into'} the page.`}</p>
      }
    />
  );
}
