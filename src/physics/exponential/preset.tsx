...
const EXPONENTIAL_CHALLENGE: ChallengeQuestion[] = [
  ...
  },
  {
    ...
    answer: 'start',
    explain:
      'The rate tracks the gap to the surroundings, biggest at the start, easing to zero as it nears room temperature.',
  },
];
...
export function DecayCoolingLab({
  ...
  const doc = useMemo<SceneDoc>(
    () => ({
      ...
      bindings: [],
      meta: {
        sims: [{ id: 'r', core: 'rate', params: { value0, target, tau, maxTime: 6 * tau }, drives: {} }],
      },
    }),
  ...
  const figure = (
    <SceneSurface className="physics-exponential-scene">
      <Scene
        key={`${mode}:${tau}:${resetN}`}
        doc={doc}
        interactive={false}
        ...
        onPlayingChange={gate.setPlaying}
  ...
  const halfLife = tau * Math.LN2;
  const instruments = (
    <>
      <div className="physics-probe">
        <span>{decay ? 'Half-life' : 'Time constant'}</span>
        <strong>{decay ? `${halfLife.toFixed(1)} s` : `${tau.toFixed(1)} s`}</strong>
        <small>{decay ? '100 → 50 → 25' : '90 °C → room temperature'}</small>
      </div>
      <p className="physics-explain">
        The change rate is proportional to the remaining gap, so the curve changes fastest at the start and
        approaches its target without a constant slope.
      </p>
    </>
  );
  return (
    <MechanicsActivity
      className="physics-exponential"
      eyebrow="Rates and change"
      title={title}
      prompt={prompt}
      status={
        <>
          <strong>{decay ? 'Radioactive decay' : 'Newton cooling'}</strong>
          <span>τ {tau.toFixed(1)} s</span>
          {decay ? <span>t½ {halfLife.toFixed(1)} s</span> : <span>target 20 °C</span>}
        </>
      }
      figure={figure}
      ...
      feedback={
        decay
          ? 'Each equal half-life removes half of what remains, not half of the original amount.'
          : 'Cooling slows as the drink approaches room temperature because the temperature gap shrinks.'
      }
      ...
      transport={
        <SimulationTransport
          ...
          state={gate.playing ? 'Changing' : 'Paused'}
          detail={decay ? `half-life ${halfLife.toFixed(1)} s` : `τ ${tau.toFixed(1)} s`}
          resetLabel="Reset exponential process"
        />
      }
      footer={<ChallengeCard questions={EXPONENTIAL_CHALLENGE} state={challenge} title="Predict" />}
      canvasLabel="Animated exponential decay or cooling curve"
      inspectorLabel="Rate measurement and process controls"
    />
  );
}
