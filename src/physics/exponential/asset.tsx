    ...
    <g>
      ...
      {(() => {
        ...
        return (
          <text x={x} y={y + 3} fontSize={11} fontWeight={700} fill="var(--stage-good)">
            {decay ? '0' : `${m.target.toFixed(0)}° room`}
          </text>
        );
      })()}
          ...
          return (
            <>
              <line
                ...
                strokeDasharray="3 4"
              />
              <text x={x + 4} y={y - 3} fontSize={9.5} fill="var(--stage-muted)">
                N₀/2
              </text>
            </>
          );
      ...
      {(() => {
        ...
        return (
          <text x={x} y={y} textAnchor="middle" fontSize={11} fill="var(--stage-muted)">
            {decay
              ? `half-life ≈ ${halfLife.toFixed(1)} s, each one halves what's left`
              : `cooling toward room temp · τ ≈ ${m.tau.toFixed(1)} s`}{' '}
            · t = {m.tSec.toFixed(1)} s
          </text>
        );
      })()}
    </g>
