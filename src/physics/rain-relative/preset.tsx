...
const RAIN_ACTIVITY: AuthoredActivity = {
  pattern: 'investigation',
  title: 'See rain from a moving frame',
  objectives: [
  ...
  steps: [
    {
      id: 'predict',
      phase: 'predict',
      title: 'Predict the apparent rain',
      lead: 'Decide how vertical rain will look from a moving car.',
    },
    {
      id: 'act',
      phase: 'act',
      title: 'Drive through the rain',
      lead: 'Play the scene and increase the car speed.',
      controls: true,
      reveal: ['model'],
      success: 'played',
    },
    {
      ...
      lead: 'Compare the ground-frame rain with the passenger-frame resultant.',
      controls: true,
    ...
    },
    {
      id: 'explain',
      phase: 'explain',
      title: 'Explain the slant',
      lead: 'Use velocity subtraction to explain the apparent angle.',
    },
    {
      id: 'transfer',
      phase: 'transfer',
      title: 'Change the observer speed',
      lead: 'Choose another speed and predict the new angle.',
      controls: true,
      reveal: ['model', 'evidence'],
  ...
  success: [
    {
      ...
      operator: 'eq',
      value: true,
      pendingLabel: 'Play the rain scene.',
    },
    {
      id: 'speed-changed',
      source: 'metric',
      ...
      operator: 'eq',
      value: true,
      pendingLabel: 'Change the car speed.',
    },
  ],
};
...
function Car({ baseY, wheelAngle }: { baseY: number; wheelAngle: number }): ReactNode {
  ...
  return (
    <g transform={`translate(${px} ${py})`}>
      ...
      <path
        d="M -88 -16 Q -96 -16 -96 -28 L -96 -33 Q -96 -44 -80 -45 L 78 -45 Q 94 -44 94 -31 L 94 -22 Q 94 -16 86 -16 Z"
        fill="url(#rr-car-body)"
        stroke={edge}
        strokeWidth={2}
        strokeLinejoin="round"
      />
      <path
        d="M -54 -45 Q -46 -69 -18 -69 L 26 -69 Q 48 -69 58 -45 Z"
        fill="url(#rr-car-body)"
        stroke={edge}
        strokeWidth={2}
        strokeLinejoin="round"
      ...
      {[-50, 54].map((wx) => (
        <g key={wx}>
          ...
          <g
            transform={`rotate(${wheelAngle} ${wx} ${-14})`}
            stroke="#8c8c8c"
            strokeWidth={2.4}
            strokeLinecap="round"
          >
...
export function RainRelativeLab({
  ...
  height = 360,
  objectives,
  activity = 'rain-relative',
}: RainRelativeProps): ReactNode {
  const activityId = typeof activity === 'string' ? activity : 'rain-relative';
  const authoredActivity = typeof activity === 'string' ? RAIN_ACTIVITY : activity;
  ...
  const draw = (ctx: CanvasRenderingContext2D, c: { width: number; height: number }): void => {
    ...
    if (drops.current.length === 0) {
      ...
      drops.current = Array.from({ length: DROPS }, () => ({
        x: rnd() * (W + 200) - 100,
        y: rnd() * H,
        len: 10 + rnd() * 10,
      }));
    ...
    const accent =
      (typeof getComputedStyle !== 'undefined' &&
        getComputedStyle(document.documentElement).getPropertyValue('--stage-accent').trim()) ||
      'rgb(120,160,235)';
    ...
    for (const d of drops.current) {
      ...
      if (d.y > H + 12) {
        d.y = -12;
        d.x = rng.current() * (W + 200) - 100;
      }
      if (d.x < -60) d.x += W + 120;
      else if (d.x > W + 60) d.x -= W + 120;
      ctx.moveTo(d.x, d.y);
  ...
  const figure = (
    <SceneSurface className="physics-rain-relative-scene">
      <CanvasLayer
        view={VIEW}
        height={height}
        draw={draw}
        ariaLabel={`rain slanting ${thetaDeg} degrees as the car speeds up`}
      />
      <div className="physics-canvas-overlay">
        <Stage
          view={VIEW}
          height={height}
          preserveAspect={false}
          background="transparent"
          ariaLabel="car and velocity triangle"
        >
          {/* road */}
          <Segment
            from={{ x: -6, y: cy }}
  ...
  const instruments = (
    <>
      <div className="physics-probe">
        <span>Apparent rain</span>
        <strong>{thetaDeg}° from vertical</strong>
        <small>relative horizontal component {(ratio * RAIN_FALL_PX).toFixed(0)} px/s</small>
      </div>
      <p className="physics-explain">
        The ground-frame rain remains vertical. Subtracting the car velocity produces the slanted velocity
  ...
  return (
    <AuthoredActivityRuntime
      ...
      status={
        <>
          <span>{speed === 0 ? 'Car stationary' : 'Car moving'}</span>
          <span>speed {speed.toFixed(1)}</span>
          <span>apparent angle {thetaDeg}°</span>
        </>
      }
      ...
      observation={
        speed === 0
          ? 'With no car velocity to subtract, the rain appears vertical.'
          : 'Increasing car speed adds a larger backward relative component, so the apparent rain leans farther from vertical.'
      }
      transcript={
        <p>{`At car speed ${speed.toFixed(1)}, the rain appears ${thetaDeg} degrees from vertical.`}</p>
      }
    >
      {({ complete }) => (
        <>
          <AuthoredMetricGate conditionId="played" met={gate.playing} complete={complete} />
          <AuthoredMetricGate conditionId="speed-changed" met={speed !== start} complete={complete} />
          {figure}
        </>
      )}
    </AuthoredActivityRuntime>
