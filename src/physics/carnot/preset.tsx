...
export function CarnotCycleLab({
  ...
  useFrameTick(gate.running, (f) => {
    uRef.current = (uRef.current + Math.min(0.05, f.dtMs / 1000) * 0.18) % 1;
  });
  ...
  const loop = c.legs
    .flatMap((l) => l.path)
    .map((p) => `${pvx(p.V).toFixed(1)},${pvy(p.P).toFixed(1)}`)
    .join(' ');
  ...
  const legColor = (i: number): string =>
    i === 0
  ...
  const figure = (
    <SceneSurface className="physics-carnot-scene">
      <svg
        viewBox={`0 0 ${W} ${H}`}
        width="100%"
        role="img"
      ...
      >
        {/* P–V */}
        <text x={AX} y={AY - 14} fontSize={15} fontWeight={700} fill="var(--stage-fg)">
          P–V diagram
        </text>
        ...
        <text x={AX - 6} y={CY + 6} textAnchor="end" fontSize={13} fill="var(--stage-muted)">
          P
        </text>
        <text x={BX} y={BY + 20} textAnchor="end" fontSize={13} fill="var(--stage-muted)">
          V →
        </text>
        ...
        {/* T–S */}
        <text x={CX} y={AY - 14} fontSize={15} fontWeight={700} fill="var(--stage-fg)">
          T–S diagram
        </text>
        ...
        <text x={CX - 6} y={CY + 6} textAnchor="end" fontSize={13} fill="var(--stage-muted)">
          T
        </text>
        <text x={DX} y={DY + 20} textAnchor="end" fontSize={13} fill="var(--stage-muted)">
          S →
        </text>
        {/* rectangle */}
        <rect
  ...
  const instruments = (
    <>
      <div className="physics-probe">
        ...
        <strong>η = {(c.efficiency * 100).toFixed(1)}%</strong>
        <small>
          Qh {J(c.Qh)} J → W {J(c.Wnet)} J + Qc {J(c.Qc)} J
        </small>
      </div>
      <div className="physics-thermal-model">
        <Tex tex={'\\eta = 1 - \\dfrac{T_c}{T_h} = \\dfrac{W}{Q_h}'} block />
        <span className="physics-explain">
          Heat enters at <strong className="physics-thermal-hot">Th = {Th} K</strong> and leaves at{' '}
          <strong className="physics-thermal-cold">Tc = {tcc} K</strong>. The wider the temperature gap, the
          more efficient, but you can never reach 100% without Tc = 0.
        </span>
        <span className="physics-explain">
          On the T–S rectangle, <Tex tex={'\\Delta S = Q_h/T_h = Q_c/T_c'} /> = {dS.toFixed(2)} J/K, so the
          gas comes back to the same entropy, <strong className="physics-emphasis">net ΔS = 0</strong> for a
          reversible cycle.
        </span>
      </div>
    </>
  ...
  const controls = (
    <>
      <>
        ...
        <Field label="cold Tc" value={`${tcc} K`}>
          <Slider
            value={Tc}
            min={250}
            max={600}
      ...
      <>
        <Field label="expansion V₂/V₁" value={`×${vr.toFixed(1)}`}>
          <Slider
            value={vr}
            min={1.3}
            max={3}
        ...
        <Field label="gas">
          <Segmented
            ...
            options={[
              { value: 'mono', label: 'monatomic' },
              { value: 'di', label: 'diatomic' },
            ]}
          />
        </Field>
      </>
  ...
  return (
    <ThermalActivity
      activity={activity}
      className="physics-carnot"
      title={title}
      prompt={prompt}
      status={
        <>
          <strong>Reversible cycle</strong>
          <span>η {(c.efficiency * 100).toFixed(1)}%</span>
          <span>Th {Th} K</span>
          <span>Tc {tcc} K</span>
        </>
      }
      ...
      controls={controls}
      feedback="The enclosed area is net work in both views. A wider hot–cold temperature gap raises the theoretical efficiency ceiling."
      ...
      transport={
        <SimulationTransport
          ...
          state={gate.playing ? 'Cycling' : 'Paused'}
          detail={`leg ${li + 1} of 4`}
          resetLabel="Reset Carnot cycle"
        />
      }
      canvasLabel="Carnot cycle on pressure-volume and temperature-entropy diagrams"
      inspectorLabel="Efficiency, energy flow, and temperatures"
    />
  );
}
