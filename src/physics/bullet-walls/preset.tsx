'use client';

/**
 * BulletWallsLab, "How many planks?", the classic penetration problem made
 * watchable and PREDICT-FIRST.
 *
 * A bullet fires into a stack of identical planks. Each plank pushes back with a
 * constant retarding force over its thickness, so it drains a FIXED chunk of
 * kinetic energy, equivalently a fixed Δ(v²) per plank. The bullet free-flies
 * between planks (constant v) and decelerates linearly in v² while inside one
 * (v²(x) = v²ₑ − (cost/width)·depth), the textbook v² = u² − 2as. It stops the
 * instant its kinetic energy runs out, embedding partway through a plank.
 *
 * The learner GUESSES how many planks it punches through, then fires and watches
 * it slow plank-by-plank while the KE bar drains, turning "N = u²/(2as)" from a
 * formula into a bet you can win.
 *
 * Tokenized SVG; time-dependent integrator here; honours reduced-motion.
 */

import { useRef, useState, type ReactNode } from 'react';
import type { AuthoredActivity } from '../../kit/activity-authoring.js';
import { useInView, useLearner } from '@classytic/stage';
import { Slider, Stepper, StatusPill } from '../../kit/controls.js';
import { Field, MeterBar, LiveRegion, type ControlConfig } from '../../kit/frame.js';
import { useReducedMotion, useFrameTick } from '../../kit/anim.js';
import { MechanicsActivity } from '../mechanics/activity.js';
import {
  MaterialBarrier,
  ProjectileGlyph,
  SceneSurface,
  SimulationTransport,
} from '../mechanics/presentation.js';
import { penetrationResult } from '../mechanics/core.js';
import { FigTag, FigText, Figure, Ground, HUE } from '../../kit/figure/index.js';

export interface BulletWallsProps {
  speed?: number;
  /** Energy a single plank drains, expressed as the v² it removes (m²/s²). */
  toughness?: number;
  planks?: number;
  mass?: number;
  title?: string;
  prompt?: string;
  objectives?: string[];
  activity?: string | AuthoredActivity;
  /** Lock/hide any knob, e.g. `{ lock: ['plank toughness'] }` to fix the toughness. */
  controlConfig?: ControlConfig;
}

const PLANK_W = 0.7; // world width of a plank
const GAP = 1.0; // gap between planks
const START_X = -4;
const STACK_X0 = 0; // x of the first plank's near face

export function BulletWallsLab({
  speed = 30,
  toughness = 160,
  planks = 6,
  mass = 0.02,
  title = 'How many planks?: bet, then fire',
  prompt = 'Each plank steals the same chunk of energy. Guess how many the bullet smashes through, then fire and watch the speed (and the energy bar) drain plank by plank.',
  objectives,
  controlConfig,
  activity,
}: BulletWallsProps): ReactNode {
  const [v0, setV0] = useState(speed);
  const [cost, setCost] = useState(toughness); // Δ(v²) per plank
  const [n, setN] = useState(planks);
  const [guess, setGuess] = useState(2);
  const [running, setRunning] = useState(false);
  const [fired, setFired] = useState(false);

  const xRef = useRef(START_X);
  const v2Ref = useRef(v0 * v0);
  const reduce = useReducedMotion();
  const learner = useLearner();
  const { ref: viewRef, inView } = useInView<HTMLDivElement>();

  // plank near-faces
  const faceX = (i: number): number => STACK_X0 + i * (PLANK_W + GAP);
  // analytic answer: full planks penetrated = floor(v0² / cost)
  const ke0 = 0.5 * mass * v0 * v0;
  const penetration = penetrationResult(v0, cost, n);
  const fullPenetrated = penetration.fullPenetrated;
  const stopsInsideStack = !penetration.exitsStack;
  const lodgeDepth = penetration.lodgeFraction * PLANK_W;

  const SPEED_SCALE = 0.25; // m/s → world units/s on screen

  const repaint = useFrameTick(running && inView, (f) => {
    const dt = Math.min(0.04, f.dtMs / 1000);
    let x = xRef.current;
    let v2 = v2Ref.current;
    const v = Math.sqrt(Math.max(0, v2));
    let dx = v * SPEED_SCALE * dt;
    // are we inside an intact plank?
    let inside = false;
    for (let i = 0; i < n; i++) {
      const a = faceX(i),
        b = a + PLANK_W;
      if (x >= a && x < b) {
        inside = true;
        break;
      }
    }
    if (inside) {
      // constant retarding force → v² falls linearly with depth travelled
      const dV2 = (cost / PLANK_W) * dx;
      if (dV2 >= v2) {
        // lodges within this plank: advance only the distance its energy allows
        const allowed = (v2 / cost) * PLANK_W;
        x += allowed;
        v2 = 0;
      } else {
        x += dx;
        v2 -= dV2;
      }
    } else {
      x += dx;
    }
    xRef.current = x;
    v2Ref.current = v2;
    const exited = x >= faceX(n - 1) + PLANK_W + 0.5;
    if (v2 <= 0 || exited) {
      setRunning(false);
      learner?.report({
        activity: 'bullet-walls',
        correct: guess === fullPenetrated,
        score: { raw: guess === fullPenetrated ? 1 : 0, max: 1 },
        response: String(guess),
        completion: true,
      });
    }
  });

  const fire = (): void => {
    xRef.current = START_X;
    v2Ref.current = v0 * v0;
    setFired(true);
    if (reduce) {
      // jump to the resting place
      xRef.current = stopsInsideStack ? faceX(fullPenetrated) + lodgeDepth : faceX(n - 1) + PLANK_W + 0.5;
      v2Ref.current = stopsInsideStack ? 0 : v0 * v0 - n * cost;
      repaint();
      return;
    }
    setRunning(true);
  };
  const onParam =
    (set: (x: number) => void) =>
    (x: number): void => {
      set(x);
      setRunning(false);
      setFired(false);
      xRef.current = START_X;
      v2Ref.current = (set === setV0 ? x : v0) ** 2;
    };

  // live state for render
  const bx = xRef.current;
  const vNow = Math.sqrt(Math.max(0, v2Ref.current));
  const keNow = 0.5 * mass * v2Ref.current;
  // how many planks has the bullet's nose cleared (passed the far face)?
  let broken = 0;
  for (let i = 0; i < n; i++) if (bx >= faceX(i) + PLANK_W) broken++;
  const settled = fired && !running;

  const stackEnd = faceX(n - 1) + PLANK_W;
  const sceneLeft = 42;
  const sceneRight = 684;
  const worldLeft = START_X - 1.3;
  const worldRight = stackEnd + 1.5;
  const toSceneX = (x: number): number =>
    sceneLeft + ((x - worldLeft) / (worldRight - worldLeft)) * (sceneRight - sceneLeft);
  const plankWidth = Math.max(10, toSceneX(STACK_X0 + PLANK_W) - toSceneX(STACK_X0));

  const figure = (
    <SceneSurface
      ref={viewRef}
      tone="grid"
      className="physics-bullet-wall-scene"
      data-state={running ? 'running' : settled ? 'settled' : 'ready'}
    >
      <Figure
        viewBox={[720, 260]}
        domain="physics"
        label={`A projectile at ${v0} metres per second approaches ${n} identical material samples. ${broken} have been penetrated.`}
      >
        <FigText x={sceneLeft} y={29} size="eyebrow" tone="soft">
          IMPACT TEST
        </FigText>
        <FigText x={sceneRight} y={29} size="note" tone="soft" anchor="end">
          each sample absorbs {(0.5 * mass * cost).toFixed(1)} J
        </FigText>
        <Ground x1={sceneLeft} x2={sceneRight} y={205} />
        {Array.from({ length: n }, (_, i) => {
          const a = faceX(i);
          const shattered = bx >= a + PLANK_W;
          const lodgedHere = settled && stopsInsideStack && i === fullPenetrated;
          return (
            <MaterialBarrier
              key={i}
              x={toSceneX(a)}
              y={78}
              width={plankWidth}
              height={127}
              index={i + 1}
              state={shattered ? 'broken' : lodgedHere ? 'lodged' : 'intact'}
            />
          );
        })}
        <ProjectileGlyph x={toSceneX(bx)} y={167} moving={running && vNow > 0.5} />
        <FigTag x={Math.max(sceneLeft + 35, toSceneX(bx) - 20)} y={126} anchor="middle" color={HUE[1]}>
          {`${vNow.toFixed(0)} m/s`}
        </FigTag>
        {settled ? (
          <FigTag
            x={Math.min(sceneRight - 68, Math.max(sceneLeft + 75, toSceneX(bx)))}
            y={54}
            anchor="middle"
            color={stopsInsideStack ? HUE.warn : HUE.good}
          >
            {stopsInsideStack ? 'energy exhausted' : 'energy remains'}
          </FigTag>
        ) : null}
      </Figure>
    </SceneSurface>
  );

  const verdict = settled
    ? guess === fullPenetrated
      ? `🎯 Spot on, ${fullPenetrated} planks`
      : `It broke ${fullPenetrated}, you guessed ${guess}`
    : null;

  const aside = (
    <>
      <div className="physics-dynamics-ledger">
        <div>
          <span>Starting KE · ½mv²</span>
          <strong>{ke0.toFixed(1)} J</strong>
        </div>
        <div>
          <span>Energy per plank</span>
          <strong>{(0.5 * mass * cost).toFixed(1)} J</strong>
        </div>
        <div data-highlight={settled}>
          <span>Broken so far</span>
          <strong>
            {broken} / {n}
          </strong>
        </div>
      </div>
      {/* KE drain bar */}
      <MeterBar
        label="kinetic energy"
        frac={ke0 > 0 ? keNow / ke0 : 0}
        color="var(--stage-accent)"
        value={`${keNow.toFixed(1)} J`}
      />
      {verdict && <StatusPill ok={guess === fullPenetrated}>{verdict}</StatusPill>}
      <p className="physics-explain">
        Predict with <strong>N = u² / (Δv² per plank)</strong>, then watch v² fall by the same step each plank
        (v² = u² − 2as). It lodges when the energy hits zero.
      </p>
      <LiveRegion>
        {settled ? `The bullet broke ${fullPenetrated} of ${n} planks. You guessed ${guess}.` : ''}
      </LiveRegion>
    </>
  );

  const controls = (
    <>
      <Field label="your guess">
        <Stepper value={guess} min={0} max={n} onChange={setGuess} label="planks you think it breaks" />
      </Field>
      <Field label="muzzle speed" value={`${v0} m/s`}>
        <Slider
          value={v0}
          min={10}
          max={60}
          step={1}
          onChange={onParam(setV0)}
          ariaLabel="muzzle speed (m/s)"
        />
      </Field>
      <Field label="plank toughness" value={`Δv² ${cost}`}>
        <Slider
          value={cost}
          min={40}
          max={400}
          step={10}
          onChange={onParam(setCost)}
          ariaLabel="energy each plank absorbs"
        />
      </Field>
      <Field label="planks" value={`${n}`}>
        <Slider
          value={n}
          min={1}
          max={10}
          step={1}
          onChange={(x) => {
            onParam(setN)(x);
            setGuess((g) => Math.min(g, x));
          }}
          ariaLabel="number of planks"
        />
      </Field>
    </>
  );

  const reset = (): void => {
    setRunning(false);
    setFired(false);
    xRef.current = START_X;
    v2Ref.current = v0 ** 2;
  };
  const transport = (
    <SimulationTransport
      running={running}
      onReset={reset}
      onToggle={running ? () => setRunning(false) : fire}
      state={running ? 'Penetrating' : settled ? (stopsInsideStack ? 'Lodged' : 'Exited') : 'Ready'}
      detail={`${broken} of ${n} planks · ${vNow.toFixed(0)} m/s`}
      startLabel="Fire"
      resetLabel="Reset bullet and planks"
    />
  );
  return (
    <MechanicsActivity
      activity={activity}
      className="physics-bullet-walls"
      title={title}
      prompt={prompt}
      status={
        <>
          <strong>{settled ? verdict : 'Predict before firing'}</strong>
          <span>KE {keNow.toFixed(1)} J</span>
          <span>
            broken {broken}/{n}
          </span>
        </>
      }
      figure={figure}
      instruments={aside}
      controls={controls}
      feedback={
        settled
          ? `Each plank removed the same energy step. The bullet ${
              stopsInsideStack
                ? `lodged ${Math.round(penetration.lodgeFraction * 100)}% into the next plank`
                : 'retained enough energy to exit the stack'
            }.`
          : 'Because every plank removes a fixed amount of v², penetration count scales with the square of muzzle speed.'
      }
      objectives={objectives}
      transport={transport}
      controlConfig={controlConfig}
      canvasLabel="Bullet penetration experiment"
      inspectorLabel="Energy, prediction, and barrier controls"
    />
  );
}
