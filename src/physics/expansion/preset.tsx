...
export function ThermalExpansionLab({
  ...
  if (mode === 'length') {
    ...
    aside = expansionAside(
      ...
      `ΔL = ${dLmm.toFixed(2)} mm`,
      <>
        A 1 m {METALS[material]!.label} rod warmed {dT} °C grows just{' '}
        <strong className="physics-emphasis">{dLmm.toFixed(2)} mm</strong>, tiny, but a 100 m bridge would
        grow {(dLmm * 100).toFixed(0)} mm, so it needs expansion joints.
      </>,
    );
  } else if (mode === 'area') {
    const cx = W / 2,
      cy = H / 2 + 6,
      s0 = 210; // centred; grows symmetrically about the centre
    ...
    figure = expansionWrap(
      ...
      >
        ...
        <text x={cx} y={cy + s1 / 2 + 20} textAnchor="middle" fontSize={15} fill="var(--stage-good)">
          ↔
        </text>
        <text x={cx - s1 / 2 - 16} y={cy + 5} textAnchor="middle" fontSize={15} fill="var(--stage-good)">
          ↕
        </text>
        <DiagramLabel
          x={cx}
    ...
    aside = expansionAside(
      ...
      `ΔA = ${dAcm2.toFixed(1)} cm²`,
      <>
        A plate expands along its width AND its length, so the area coefficient is{' '}
        <strong className="physics-emphasis">2α</strong>. A 1 m² {METALS[material]!.label} sheet warmed {dT}{' '}
        °C gains {dAcm2.toFixed(1)} cm².
      </>,
    );
  } else if (mode === 'volume') {
    const cx = W / 2,
    ...
    aside = expansionAside(
      ...
      `ΔV = ${dVcm3.toFixed(1)} cm³`,
      <>
        A solid swells in all three directions, so the volume coefficient is{' '}
        <strong className="physics-emphasis">3α</strong>. A 1 L {METALS[material]!.label} block warmed {dT} °C
        gains {dVcm3.toFixed(1)} cm³.
      </>,
    );
  } else {
    ...
    const off = (sign: number): string =>
      center
        .map((p) => {
          const al = Math.atan2(p.ty, p.tx);
          return `${(p.x + sign * (thick / 2) * Math.sin(al)).toFixed(1)},${(
            p.y -
            sign * (thick / 2) * Math.cos(al)
          ).toFixed(1)}`;
        })
        .join(' ');
    const tip = center[N]!;
    ...
    aside = expansionAside(
      ...
      on ? 'contact CLOSED' : 'contact open',
      <>
        Brass expands more than steel, so when heated it must take the{' '}
        <strong className="physics-emphasis">longer, outer</strong> edge of the curve, the strip bends toward
        the steel. Drag the temperature up until it closes the contact: that's how a{' '}
        <strong className="physics-emphasis">thermostat</strong> works.
      </>,
    );
  }
  ...
  return (
    <ThermalActivity
      activity={activity}
      className="physics-expansion"
      title={title}
      prompt={prompt}
      status={
        <>
          <strong>{mode === 'bimetallic' ? 'Thermostat strip' : `${mode} expansion`}</strong>
          <span>ΔT +{dT} °C</span>
          <span>{mode === 'bimetallic' ? 'brass + steel' : METALS[material]!.label}</span>
        </>
      }
      figure={figure}
      instruments={aside}
      controls={controls}
      feedback="The drawing magnifies a very small physical change. The numerical result remains the true expansion, so scale and direction stay explicit."
      ...
      transport={
        <ResetTransport
          ...
          state={mode}
          detail={`ΔT +${dT} °C`}
          resetLabel="Reset thermal expansion"
        />
      }
      controlConfig={controlConfig}
      canvasLabel="Magnified thermal expansion model"
      inspectorLabel="Expansion equation, result, and material controls"
    />
  );
}

function expansionWrap(svg: ReactNode): ReactNode {
  return <SceneSurface className="physics-expansion-scene">{svg}</SceneSurface>;
}
  ...
  return (
    <>
      <div className="physics-probe">
        <span>True expansion</span>
        <strong>{result}</strong>
        <small>Drawing magnified ×{MAG}</small>
      </div>
      <div className="physics-thermal-model">
        {formula}
        <span className="physics-explain">{note}</span>
      </div>
    </>
  );
}
