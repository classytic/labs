...
export function EntropyLab({
  ...
  if (mode === 'heat') {
      ...
      return (
        <g>
          <rect
            ...
            opacity={0.85}
          />
          <text x={x} y={270} textAnchor="middle" fontSize={14} fill="var(--stage-fg)">
            {label}
          </text>
          <text
            className="physics-svg-numeric"
            x={x}
            y={hpx >= 0 ? baseY - hpx - 7 : baseY + Math.abs(hpx) + 16}
            textAnchor="middle"
            fontSize={14}
            fontWeight={700}
            fill={color}
    ...
    figure = (
      <SceneSurface className="physics-entropy-scene">
        <svg
          viewBox={`0 0 ${W} ${H}`}
          width="100%"
          role="img"
        ...
        >
          ...
          <text className="physics-svg-on-fill" x={100} y={200} textAnchor="middle" fontSize={12}>
            {Th} K
          </text>
          ...
          <text className="physics-svg-on-fill" x={310} y={200} textAnchor="middle" fontSize={12}>
            {tc} K
          </text>
          ...
          <text x={205} y={164} textAnchor="middle" fontSize={11} fontWeight={700} fill="var(--stage-warn)">
            Q = {Q} J
          </text>
          {/* entropy bars */}
          <line x1={420} y1={250} x2={620} y2={250} stroke="var(--stage-grid)" strokeWidth={1} />
          <text x={520} y={56} textAnchor="middle" fontSize={14} fill="var(--stage-muted)">
            entropy change (J/K)
          </text>
    ...
    headline = `ΔS total +${dST.toFixed(2)} J/K`;
    feedback = 'The colder body gains more entropy than the hotter body loses, so total entropy increases.';
    instruments = (
      <>
        <div className="physics-probe">
          <span>Total entropy change</span>
          <strong>
            ΔS = {dST >= 0 ? '+' : '−'}
            {Math.abs(dST).toFixed(2)} J/K
          </strong>
          <small>{dST > 0.001 ? 'Spontaneous process' : 'Reversible limit'}</small>
        </div>
        <div className="physics-thermal-model">
          <Tex tex={'\\Delta S_{tot} = \\dfrac{Q}{T_c} - \\dfrac{Q}{T_h} > 0'} block />
          <span className="physics-explain">
            The cold body gains <strong className="physics-emphasis">more</strong> entropy than the hot body
            loses (same Q, smaller T). So heat flows hot→cold by itself; the reverse would lower total
            entropy, forbidden by the 2nd law.
          </span>
        </div>
      </>
    );
      ...
      <SceneSurface className="physics-entropy-scene">
        ...
        >
          ...
          <line
            ...
            opacity={1 - m}
          />
          <text x={bx + bw / 2} y={by + bh + 24} textAnchor="middle" fontSize={14} fill="var(--stage-muted)">
            {m < 0.02 ? 'gas confined to the left half' : m > 0.98 ? 'gas fills the whole box' : 'spreading…'}
          </text>
        </svg>
      </SceneSurface>
    );
    headline = `ΔS +${dS.toFixed(2)} J/K`;
    feedback =
      'A spread-out gas has vastly more possible arrangements, so it does not spontaneously return to one half.';
    instruments = (
      <>
        <div className="physics-probe">
          <span>Entropy increase</span>
          <strong>ΔS = +{dS.toFixed(2)} J/K</strong>
          <small>Accessible volume ×{ratio.toFixed(2)}</small>
        </div>
        <div className="physics-thermal-model">
          <Tex tex={'\\Delta S = nR\\,\\ln\\!\\dfrac{V_f}{V_i}'} block />
          <span className="physics-explain">
            No heat, no work, yet entropy still rises, because the gas now has far more ways to arrange
            itself. Fully open (Vf = 2Vi) gives{' '}
            <strong className="physics-emphasis">nR·ln 2 ≈ 5.76 J/K</strong>. You’ll never see it pile back
            into one half on its own.
          </span>
        </div>
      </>
    );
  ...
  const controls = (
    <>
      <>
        <Field label="process">
          <Segmented
            ...
            onChange={setMode}
            options={[
              { value: 'heat', label: 'heat flows hot→cold' },
              { value: 'expansion', label: 'free expansion (spreading)' },
            ]}
          />
        </Field>
      </>
      {mode === 'heat' ? (
        <>
          <Field label="hot Th" value={`${Th} K`}>
            <Slider value={Th} min={310} max={800} step={10} onChange={setTh} ariaLabel="hot temperature" />
          </Field>
          <Field label="cold Tc" value={`${Math.min(Tc, Th)} K`}>
            <Slider value={Tc} min={250} max={800} step={10} onChange={setTc} ariaLabel="cold temperature" />
          </Field>
          <Field label="heat Q" value={`${Q} J`}>
            <Slider value={Q} min={200} max={3000} step={100} onChange={setQ} ariaLabel="heat transferred" />
          </Field>
        </>
      ) : (
        <>
          <Field label="open the partition" value={`${Math.round(m * 100)}%`}>
            <Slider value={m} min={0} max={1} step={0.05} onChange={setM} ariaLabel="partition openness" />
          </Field>
        </>
      )}
    </>
  ...
  return (
    <ThermalActivity
      activity={activity}
      className="physics-entropy"
      title={title}
      prompt={prompt}
      status={
        <>
          <strong>{mode === 'heat' ? 'Heat flow' : 'Free expansion'}</strong>
          <span>{headline}</span>
        </>
      }
      figure={figure}
      ...
      transport={
        <ResetTransport
          onReset={reset}
          state={headline}
          detail={mode === 'heat' ? 'hot → cold' : 'more accessible microstates'}
          resetLabel="Reset entropy experiment"
        />
      }
      canvasLabel={`${mode === 'heat' ? 'Heat flow' : 'Free expansion'} entropy visualization`}
      inspectorLabel="Entropy calculation and variables"
    />
  );
}
