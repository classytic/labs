...
export function GeneticCrossLab({
  spec,
  loci,
  parent1,
  parent2,
  ...
  const defaultHet = (): string[][] =>
    models.map((m) => [m.alleles[0]!.symbol, m.alleles[Math.min(1, m.alleles.length - 1)]!.symbol]);
  const defaultP2 = (): string[][] =>
    models.map((m) => [m.alleles[0]!.symbol, m.alleles[m.alleles.length - 1]!.symbol]);

  const [p1, setP1] = useState<string[][]>(() =>
    toPairs(parent1, defaultHet()).map((pr, i) => models[i]!.norm(pr[0]!, pr[1]!)),
  );
  const [p2, setP2] = useState<string[][]>(() =>
    toPairs(parent2, multi ? defaultHet() : defaultP2()).map((pr, i) => models[i]!.norm(pr[0]!, pr[1]!)),
  );
        ...
        <span key={L} className="genetic-allele-pair">
          {pair.map((s, ai) => (
            <Button
              ...
              key={ai}
              type="button"
              className="lab-chip genetic-allele-button"
              style={{ '--genetic-color': alleleColor(s) } as CSSProperties}
              onClick={() => cycle(setter, parent, L, ai as 0 | 1)}
  ...
  const header = (
    <div className="genetic-parent-controls">
      ...
      <span className="genetic-control-hint">(tap an allele to cycle it)</span>
    </div>
  );
          ...
          {m.alleles.map((a, j) => (
            <span key={a.symbol}>
              <b
                ...
                style={{ '--genetic-color': alleleColor(a.symbol) } as CSSProperties}
              >
                {a.symbol}
              </b>
              ={a.trait}
  ...
  return (
    <CrossGrid
      ...
      prompt={
        prompt ??
        (multi
          ? 'Two genes assort independently, four gametes each, sixteen boxes, four phenotype classes.'
          : 'Alleles segregate into gametes, recombine, and the ratio falls out.')
      }
