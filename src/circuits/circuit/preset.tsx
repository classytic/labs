...
import { Chip, Slider, StatusPill } from '../../kit/controls.js';
import { Field, LiveRegion } from '../../kit/frame.js';
import { AuthoredActivityRuntime, AuthoredMetricGate } from '../../kit/authored-activity-runtime.js';
import type { AuthoredActivity } from '../../kit/activity-authoring.js';
import { circuitActivity } from './activity-plan.js';
...
const DEFAULT_BRANCHES: CircuitComponentSpec[][] = [
  [
...
function flatten(branches: CircuitComponentSpec[][]): {
  comps: { c: CircuitComponentSpec; b: number; p: number }[];
  switches: SwitchInfo[];
} {
  const comps: { c: CircuitComponentSpec; b: number; p: number }[] = [];
  const switches: SwitchInfo[] = [];
  branches.forEach((chain, b) =>
    chain.forEach((c, p) => {
      comps.push({ c, b, p });
      if (c.type === 'switch')
        switches.push({ id: `sw${switches.length}`, label: c.label ?? `switch ${switches.length + 1}` });
    }),
  );
  return { comps, switches };
}

export function circuitDoc({
  emf = 6,
  emfRange = [1, 12, 1],
  internalR = 0,
  ...
  const elements: SceneElement[] = [
    {
      ...
      free: { value: emf, min: emfMin, max: emfMax, step: emfStep },
    },
  ];
  switches.forEach((s, j) =>
    elements.push({
      ...
      a11y: { label: s.label },
      free: {
        value: (branches.flat().filter((c) => c.type === 'switch')[j] as { closed?: boolean }).closed ? 1 : 0,
        min: 0,
        max: 1,
        step: 1,
      },
  ...
  elements.push({
    id: 'circuit',
    kind: 'asset',
    def: { op: 'asset', asset: 'circuit-network', params, bind },
  });
  ...
  return {
    schemaVersion: 2,
    type: 'stage-scene',
    view: { xMin: -4.6, xMax: 4.6, yMin: -vy, yMax: vy },
    elements,
    bindings: [],
  };
}
...
export function CircuitNetworkLab(props: CircuitNetworkProps): ReactNode {
  const {
    ...
    prompt = 'Close the switch and power the circuit.',
    activity = 'circuit-network',
  } = props;
  const activityId = typeof activity === 'string' ? activity : 'circuit-network';
  const authoredActivity = typeof activity === 'string' ? circuitActivity : activity;
  ...
  const meta = isAssetGeom(geom)
    ? (geom.meta as { solved: boolean; Itotal: number; emf: number })
    : { solved: false, Itotal: 0, emf: 0 };
  const emf = Number(resolved.values.get('emf') ?? 0);

  const [emfMin, emfMax, emfStep] = emfRange;
  const controls = useMemo(
    () =>
      controlsFromScene(editor, [{ id: 'emf', name: 'battery', min: emfMin, max: emfMax, step: emfStep }]),
    [editor, emfMin, emfMax, emfStep],
  );
  useControlSurface(controlId, controls);
  const setEmf = (v: number): void => {
    editor.dispatch({ op: 'mutate', id: 'emf', patch: { free: { value: v } } });
  };
  ...
  const switchSummary = switches.length
    ? switches
        .map((s) => `${s.label} ${Number(resolved.values.get(s.id) ?? 0) >= 0.5 ? 'closed' : 'open'}`)
        .join(', ')
    : 'no switches';
  ...
  const figure = (
    <>
      ...
      <LiveRegion>
        {meta.solved
          ? 'Circuit powered, goal reached.'
          : `Current ${(meta.Itotal * 1000).toFixed(0)} milliamps.`}
      </LiveRegion>
    </>
  );
  ...
  const controlsUi = (
    <>
      {switches.map((s) => {
        ...
        // A switch is a real, focusable host control, so Tab + Enter/Space
        // already work. We add an explicit
      ...
      <Field label="🔋 battery" value={`${emf} V`}>
        <Slider
          ...
          className="electronics-slider-medium"
        />
      </Field>
    </>
  );
  ...
  const aside = (
    <>
      <StatusPill ok={meta.solved}>{meta.solved ? 'Circuit powered' : 'Path incomplete'}</StatusPill>
      <strong className="electronics-readout">I ≈ {(meta.Itotal * 1000).toFixed(0)} mA</strong>
    </>
  );
  ...
  return (
    <AuthoredActivityRuntime
      ...
      eyebrow="Circuit investigation"
      title={title}
      description={prompt}
      status={
        <>
          <span>{meta.solved ? 'goal reached' : 'open task'}</span>
          <span>{emf} V</span>
          <span>{(meta.Itotal * 1000).toFixed(0)} mA</span>
        </>
      }
      evidence={aside}
      controls={controlsUi}
      observation="Current requires a complete path. The author-defined source, branches, components and goal all feed one solved circuit state rather than separate visual approximations."
      transcript={
        <p>
          {figureLabel} Total current is {(meta.Itotal * 1000).toFixed(0)} milliamps.
        </p>
      }
    >
      {({ complete }) => (
        <>
          <AuthoredMetricGate
            conditionId="goal-reached"
            met={meta.solved}
            complete={complete}
            outcome={`${(meta.Itotal * 1000).toFixed(0)} mA`}
          />
          {figure}
        </>
      )}
