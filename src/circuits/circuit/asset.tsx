...
import { BulbGlyph, ResistorGlyph, SwitchGlyph } from '../../kit/electronics/index.js';
...
const numOr = (v: unknown, d: number): number =>
  typeof v === 'number' && Number.isFinite(v) ? v : typeof v === 'boolean' ? (v ? 1 : 0) : d;
...
function solveNetwork(
  ...
  for (let b = 0; b < nBranch; b++) {
    ...
    let prev = 1;
    let f: { a: number; b: number; R: number } | null = null;
    chain.forEach((r, idx) => {
...
function resolver({ params, bound }: AssetResolveArgs): AssetGeometry {
  ...
  for (let i = 0; i < nComp; i++) {
    ...
    raw.push({
      i,
      type,
      ohms: Math.max(0, n(params[`o${i}`], type === 'switch' ? 0 : 1)),
      branch: Math.round(n(params[`b${i}`], 0)),
      pos: n(params[`p${i}`], i),
      closed,
    });
  }
  ...
  if (goalType === 0) {
    ...
    solved = bulbs.length > 0 && bulbs.every((c) => c.brightness >= 0.1);
  }
...
function Component({ geom }: { geom: AssetGeometry; style?: ElementStyle; label?: string }): ReactNode {
  ...
  const renderComp = (comp: Comp, i: number): ReactNode => {
    ...
    const energised = Math.abs(comp.current) > 1e-4;
    if (comp.type === 'resistor') {
      // shared ResistorGlyph (banded pill) — leads off, this lab routes its own wires.
      return (
        <g key={i}>
          <ResistorGlyph cx={cx} cy={cy} half={HALF} live={energised} label={comp.label} leads={false} />
        </g>
      );
    }
    if (comp.type === 'bulb') {
      // shared BulbGlyph (glass + glowing filament coil) — one bulb across every circuit
      // lab; leads off since this lab routes its own wires. brightness drives the glow.
      return (
        <g key={i}>
          <BulbGlyph
            cx={cx}
    ...
    // shared SwitchGlyph (hinged lever) — leads off; the lab draws its own wires.
    return (
      <g key={i}>
        <SwitchGlyph
          cx={cx}
          ...
          m.wires
            ...
            .map((w, i) => (
              <polyline
                ...
                strokeLinecap="round"
                style={{ '--stage-current-duration': `${m.flowDur}s` } as CSSProperties}
              />
            ))}
