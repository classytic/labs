...
import { LampGlyph, AcDcSourceGlyph } from '../../kit/electronics/index.js';
...
function resolver({ sim }: AssetResolveArgs): AssetGeometry {
  ...
  const trace: Vec2[] = samples.map((s, i) => ({
    x: SCOPE.x0 + (i / cap) * w,
    y: mid - (s / volts) * halfH,
  }));
  ...
  return (
    <>
      ...
      {m.electrons.map((e, i) => {
        const [x, y] = P(e);
        return (
          <circle key={`e${i}`} cx={x} cy={y} r={3} fill="var(--stage-charge)" opacity={lit ? 0.9 : 0.25} />
        );
      })}
      {/* source + lamp (glyphs at projected anchors, fixed px size) */}
      {(() => {
        const [x, y] = P(geom.parts.src as Vec2);
        return <AcDcSourceGlyph cx={x} cy={y} mode={m.mode === 'ac' ? 'ac' : 'dc'} level={m.level} />;
      })()}
      {(() => {
        const [x, y] = P(geom.parts.lamp as Vec2);
        return <LampGlyph cx={x} cy={y} brightness={m.brightness} />;
      })()}
      {/* water-pipe analogy */}
      {(() => {
        ...
        return (
          <g>
            ...
            {m.drops.map((d, i) => {
              const [x, y] = P(d);
              return <circle key={`d${i}`} cx={x} cy={y} r={5} fill="var(--stage-accent)" opacity={0.85} />;
            })}
            {/* Above the pipe: below it, the stage Play button (centred on the scene) covered it. */}
            <text x={px0 - 6} y={py - 26} fontSize={12} fill="var(--stage-metal)">
              water analogy, {m.mode === 'dc' ? 'steady one-way flow' : 'back-and-forth slosh'} (same signal)
            </text>
          </g>
        );
      })()}
      ...
      <g>
        ...
        />
        <text x={scX0 + 10} y={scTop + 20} fontSize={13} fontWeight={700} fill="var(--stage-live)">
          voltage vs time
        </text>
        <text x={scX1 - 10} y={scTop + 20} textAnchor="end" fontSize={13} fill="var(--stage-metal)">
          {m.lastV >= 0 ? '+' : ''}
          {m.lastV.toFixed(1)} V
        </text>
      </g>
