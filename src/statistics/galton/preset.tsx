...
const CHALLENGE: ChallengeQuestion[] = [
  {
    id: 'shape',
    prompt: 'Thousands of balls fall, bouncing left or right at each peg. The pile will be…',
    choices: [
      { value: 'bell', label: 'a bell — most in the middle' },
      { value: 'flat', label: 'flat — evenly spread' },
      { value: 'side', label: 'piled on one side' },
    ],
    answer: 'bell',
    explain:
      'Each ball is a sum of many 50/50 bounces (a binomial), and a pile of those sums traces the normal curve — the Central Limit Theorem, live.',
  },
  {
    id: 'edge',
    prompt: 'For a ball to land in the far-left bin, its bounces must have been…',
    choices: [
      { value: 'allleft', label: 'almost all left (very unlikely)' },
      { value: 'mixed', label: 'a mix of left and right' },
      { value: 'allright', label: 'almost all right' },
    ],
    answer: 'allleft',
    explain:
      'The edges need a near-perfect run of one direction, which is rare — so the bars are tiny there and tallest in the middle.',
  },
];
...
export function GaltonBoardLab({
  rows = 12,
  seed = 7,
  showCurve = true,
  title = 'Galton board',
  ...
  const pmf = useMemo(() => {
    ...
    for (let i = 0; i < R; i++) {
      const next = [1];
      for (let k = 0; k < row.length - 1; k++) next.push(row[k]! + row[k + 1]!);
      next.push(1);
      row.length = 0;
      row.push(...next);
    }
  ...
  const settle = (n: number): void => {
    for (let i = 0; i < n; i++) counts.current[newBall().bin]!++;
  ...
  const drop = (n: number): void => {
    ...
    target.current = landed() + balls.current.length + n;
    setRunning(true);
  };
  ...
  useFrameLoop(
    (frame) => {
      ...
      while (
        spawnAcc.current >= 1 &&
        have + 0 < target.current &&
        balls.current.length < MAX_FLIGHT &&
        landed() + balls.current.length < target.current
      ) {
  ...
  const draw = useCallback(
    (ctx: CanvasRenderingContext2D, _c: CoordinateSystem) => {
      ...
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(slotX(R, 0) - barW, binsBottom + 0.5);
      ctx.lineTo(slotX(R, R) + barW, binsBottom + 0.5);
      ctx.stroke();
      ...
      if (curve && total > 0) {
        ...
        ctx.beginPath();
        for (let b = 0; b <= R; b++) {
          const h = ((total * pmf[b]!) / maxCount) * binH;
          const x = slotX(R, b),
            y = binsBottom - h;
      ...
      for (const ball of balls.current) {
        const r = Math.min(R - 1, Math.floor(ball.f));
        const frac = ball.f - Math.floor(ball.f);
        const x0 = slotX(r, ball.cum[r]!),
          x1 = slotX(r + 1, ball.cum[r + 1]!);
  ...
  const samples = useMemo(
    () => counts.current.flatMap((c, b) => new Array(c).fill(b)),
    [counts.current.reduce((a, b) => a + b, 0)],
  );
  const total = counts.current.reduce((a, b) => a + b, 0);
  ...
  useControlSurface(controlId, {
    drop100: {
      type: 'action',
      label: 'drop 100 balls',
      invoke: () => drop(100),
    },
    drop1000: {
      type: 'action',
      label: 'drop 1000 balls',
      invoke: () => drop(1000),
    },
    ...
    <div ref={viewRef} className="lab-playwrap">
      <div className="discrete-stage-scene">
        <CanvasLayer
          ...
          height={height}
          draw={draw}
          ariaLabel={`Galton board, ${total} balls dropped, mean bin ${mu.toFixed(1)}`}
        />
      </div>

      <div className="statistics-formula-summary">
        <strong>balls: {total.toLocaleString()}</strong>
        <span className="statistics-sampling-good">
          mean <Tex tex={`\\approx ${mu.toFixed(2)}`} />{' '}
          <span className="statistics-sampling-detail">
            (<Tex tex={`\\to ${(R / 2).toFixed(1)}`} />)
          </span>
        </span>
        <span className="statistics-sampling-accent">
          <Tex tex={`\\sigma \\approx ${sd.toFixed(2)}`} />{' '}
          <span className="statistics-sampling-detail">
            (<Tex tex={`\\to ${(Math.sqrt(R) / 2).toFixed(2)}`} />)
          </span>
        </span>
      </div>
    </div>
  ...
  const controls = (
    <div className="lab-activity-fields">
      <ActionButton onClick={() => drop(1000)}>Drop 1,000</ActionButton>
      <Chip selected={curve} onClick={() => setCurve((v) => !v)}>
        bell curve
      </Chip>
      <Field label="speed">
        <Slider value={speed} min={10} max={200} step={10} onChange={setSpeed} ariaLabel="balls per second" />
      </Field>
    </div>
  );
  ...
  const transport = (
    <RunTransport
      running={running}
      onReset={reset}
      onStep={() => (reduce ? settle(1) : drop(1))}
      onToggle={() => (running ? setRunning(false) : drop(100))}
      state={reduce ? 'Manual drops' : running ? 'Dropping balls' : total ? 'Experiment paused' : 'Ready'}
      detail={`${total.toLocaleString()} landed`}
      resetLabel="Reset Galton board"
      stepLabel="Drop one ball"
      runLabel="Drop 100 balls"
    />
  );
  ...
  return (
    <Activity.Root className="statistics-galton-activity" focusLayout="immersive">
      <Activity.Header>
        <Activity.Heading
          eyebrow="Random processes"
          title={title}
          description={
            prompt ??
            'Drop balls through repeated left/right choices and watch a binomial pile approach a bell curve.'
          }
        />
        <Activity.FocusButton />
      </Activity.Header>
      <Activity.Status>
        <strong>{total ? `${total.toLocaleString()} balls` : 'Ready to drop'}</strong>
        <span>mean {mu.toFixed(2)}</span>
        <span>σ {sd.toFixed(2)}</span>
      </Activity.Status>
      <Activity.Workspace>
        <Activity.Canvas label="Animated Galton board">{figure}</Activity.Canvas>
        <Activity.Inspector label="Experiment controls">{controls}</Activity.Inspector>
      </Activity.Workspace>
      <Activity.Feedback>
        <span>Observe</span>
        <div>
          Each landing position is the sum of repeated binary choices; many independent paths concentrate near
          the middle.
        </div>
      </Activity.Feedback>
      <section className="lab-authored-task" aria-label="Galton-board prediction and support">
        {footer}
      </section>
