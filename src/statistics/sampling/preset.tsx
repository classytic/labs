...
const CHALLENGE: ChallengeQuestion[] = [
  {
    id: 'spread',
    prompt: 'You draw many samples and pile up the mean of each one. That pile is…',
    choices: [
      { value: 'narrow', label: 'narrower than the population' },
      { value: 'same', label: 'the same width' },
      { value: 'wide', label: 'wider' },
    ],
    answer: 'narrow',
    explain:
      'Averaging cancels extremes, so sample means cluster tightly around μ — with spread σ/√n, shrinking as n grows. That’s the Central Limit Theorem.',
  },
  {
    id: 'ci',
    prompt: '“95% confidence” means…',
    choices: [
      { value: 'proc', label: '95% of such intervals capture the true μ' },
      { value: 'one', label: 'this one interval has a 95% chance to hold μ' },
      { value: 'data', label: '95% of the data lies inside' },
    ],
    answer: 'proc',
    explain:
      'Confidence is a property of the procedure, not of one interval: repeat it and ~95% of the intervals catch μ. Watch the running coverage converge to your chosen level.',
  },
];
...
export function SamplingDistributionLab({
  mu = 50,
  sigma = 10,
  n = 30,
  confidence = 0.95,
  ...
  const addSamples = (count: number): void => {
    for (let k = 0; k < count; k++) {
      ...
      if (mode === 'sampling') {
        ...
        if (means.current.length > 5000) means.current.shift();
      } else {
    ...
    setTick((value) => (value + 1) & 0xffffff);
  };
  ...
  const draw = useCallback(
    (ctx: CanvasRenderingContext2D, _c: CoordinateSystem) => {
      ...
      const fg = tok('--stage-fg', '#222'),
        grid = tok('--stage-grid', 'rgba(125,125,125,.3)'),
        muted = tok('--stage-muted', '#888'),
        accent = tok('--stage-accent', '#1c7ed6'),
        good = tok('--stage-good', '#2f9e44'),
        bad = tok('--stage-danger', '#e03131');
  ...
  useControlSurface(controlId, {
    mode: {
      ...
      options: ['sampling', 'ci'],
      get: () => mode,
      set: (v) => setMode(v as SamplingMode),
    },
    run: {
      type: 'action',
      label: reduce ? 'draw one sample' : running ? 'pause' : 'run',
      invoke: () => (reduce ? addSamples(1) : setRunning((r) => !r)),
    },
    step: { type: 'action', label: 'draw one sample', invoke: () => addSamples(1) },
  ...
  const meansSD =
    means.current.length > 1
      ? Math.sqrt(means.current.reduce((a, x) => a + (x - m) ** 2, 0) / means.current.length)
      : 0;
      ...
      <div className="lab-field-row">
        <Segmented
          ...
          options={[
            ...
            { value: 'sampling', label: 'sampling distribution' },
          ]}
        />
      </div>

      <div className="discrete-stage-scene statistics-sampling-scene">
        <CanvasLayer view={view} height={height} draw={draw} ariaLabel={`${mode}; SE ${f2(se)}`} />
      </div>

      <div className="statistics-sampling-summary">
        <span>
          <Tex tex={'\\mathrm{SE} = \\sigma/\\sqrt{n}'} /> ={' '}
          <span className="statistics-sampling-good">{f2(se)}</span>
        </span>
        {mode === 'ci' ? (
          <>
            <span>level: {(conf * 100).toFixed(0)}%</span>
            <span className="statistics-sampling-accent">
              coverage: {(coverage * 100).toFixed(1)}%{' '}
              <span className="statistics-sampling-detail">
                ({cover.current.hit}/{cover.current.total})
              </span>
            </span>
          </>
        ) : (
          <span className="statistics-sampling-accent">
            spread of means <Tex tex={`\\approx ${f2(meansSD)}`} />{' '}
            <span className="statistics-sampling-detail">
              (<Tex tex={`\\to \\mathrm{SE}\\ ${f2(se)}`} />)
            </span>{' '}
            · {means.current.length} samples
          </span>
        )}
      </div>
    </div>
  ...
  const controls = (
    <div className="lab-activity-fields">
      <Field label="n" value={nn}>
        <Slider value={nn} min={2} max={200} step={1} onChange={setNn} ariaLabel="sample size" />
      </Field>
      {mode === 'ci' && (
        /* `conf` is a number, so it is keyed by its string form here; a level outside the
           offered set simply matches no option and leaves the group unpressed. */
        <Segmented
          ariaLabel="confidence level"
          value={String(conf)}
          onChange={(v) => setConf(Number(v))}
          options={([0.8, 0.9, 0.95, 0.99] as const).map((c) => ({
            value: String(c),
            label: `${c * 100}%`,
          }))}
        />
  ...
  const transport = (
    <RunTransport
      ...
      onReset={reset}
      onStep={() => addSamples(1)}
      onToggle={() => (reduce ? addSamples(1) : setRunning((value) => !value))}
      state={reduce ? 'Manual sampling' : running ? 'Sampling' : 'Paused'}
      detail={mode === 'ci' ? `${cover.current.total} intervals` : `${means.current.length} sample means`}
      resetLabel="Reset sampling experiment"
      stepLabel="Draw one sample"
      runLabel={reduce ? 'Draw one sample' : 'Draw continuously'}
    />
  );
  ...
  return (
    <Activity.Root className="statistics-sampling-activity" focusLayout="immersive">
      <Activity.Header>
        <Activity.Heading
          ...
          description={
            prompt ??
            'Repeat the sampling process and watch sample means or confidence intervals accumulate into evidence.'
          }
        />
      ...
      <Activity.Status>
        <strong>
          {mode === 'ci'
            ...
            : `${means.current.length} sample means`}
        </strong>
        <span>n {nn}</span>
        <span>{mode === 'ci' ? `${conf * 100}% confidence` : `SE ${f2(se)}`}</span>
      </Activity.Status>
      <Activity.Workspace>
        <Activity.Canvas label="Sampling-distribution experiment">{figure}</Activity.Canvas>
        <Activity.Inspector label="Sampling parameters">{controls}</Activity.Inspector>
      </Activity.Workspace>
      <Activity.Feedback>
        <span>Observe</span>
        <div>
          {mode === 'ci'
            ? 'Coverage describes the long-run success rate of the interval procedure, not the probability attached to one completed interval.'
            : 'Sample means vary less than individual observations; increasing n shrinks their standard error.'}
        </div>
      </Activity.Feedback>
      <section className="lab-authored-task" aria-label="Sampling-distribution prediction and support">
        {footer}
      </section>
