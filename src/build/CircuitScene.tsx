...
import { JunctionDot, Wire, FlowDots } from '../kit/electronics/index.js';
...
import { registerBuiltinParts } from './parts/index.js';
...
export interface CircuitEditorBag {
  ...
  /** keyboard-activate a pin (Enter/Space): start a wire, or finish one to this pin. */
  onPinActivate?: (partId: string, pin: string) => void;
...
export interface CircuitSceneProps {
  ...
  viewBox?: string;
  /** render just the bare <svg> (the editor supplies its own framed, dot-gridded host). */
  bare?: boolean;
}
...
export function CircuitScene({
  ...
  const vbArr = viewBox ? viewBox.split(' ').map(Number) : [0, 0, W, H];
  const [vbx, vby, vbw, vbh] = vbArr as [number, number, number, number];
  ...
  // Selected NET: the electrical net(s) the selected part/wire belongs to. Every wire and
  // stub on that net gets a soft accent "highlighter" underlay, so a learner can trace WHAT
  // IS CONNECTED to what at a glance (the answer to "did my short actually connect?").
  const selNets = new Set<number>();
  const selPart = selectedId ? doc.parts.find((x) => x.id === selectedId) : undefined;
  if (selPart) {
    const d = getPart(selPart.kind);
    if (d) for (const pin of d.pins) selNets.add(sol.net(selPart.id, pin));
  }
  if (editor?.selectedWireId) {
    const w = (doc.wires ?? []).find((x) => x.id === editor.selectedWireId);
    if (w) selNets.add(sol.net(w.a.partId, w.a.pin));
  }
  const onNet = (partId: string, pin: string): boolean =>
    selNets.size > 0 && selNets.has(sol.net(partId, pin));
  const hilite = (pts: [number, number][], key: string): ReactNode => (
    <polyline
      key={key}
  ...
  const nodePos = (nid: string): Vec2 => {
    ...
    return ts.length
      ? { x: ts.reduce((s, t) => s + t.x, 0) / ts.length, y: ts.reduce((s, t) => s + t.y, 0) / ts.length }
      : { x: W / 2, y: H / 2 };
  };
  ...
  for (const p of doc.parts) {
    ...
    for (const pin of def.pins) {
      ...
      if (onNet(p.id, pin)) wires.push(hilite(seg, `hl-stub-${p.id}-${pin}`));
  ...
  for (const w of doc.wires ?? []) {
    ...
    wires.push(
      <g
        key={`wire-${w.id}`}
        className={editor?.onWireClick ? 'circuit-scene-wire-target' : undefined}
        onClick={
          editor?.onWireClick
            ? (e) => {
                e.stopPropagation();
      ...
      >
        {!isSel && (onNet(w.a.partId, w.a.pin) || onNet(w.b.partId, w.b.pin)) && hilite(seg, `hl-w-${w.id}`)}
        {isSel && (
          <polyline
            points={ptStr}
            fill="none"
        ...
        {editor?.onWireClick && (
          <polyline
            ...
            strokeLinecap="round"
            strokeLinejoin="round"
            className="circuit-scene-wire-hit"
            data-selected={isSel || undefined}
            pointerEvents="stroke"
            onPointerDown={
              isSel && editor.onWireBodyDown
                ? (e) => {
                    e.stopPropagation();
        ...
        {isSel &&
          (w.mid ?? []).map((m, i) => (
            <rect
              ...
              stroke="var(--stage-accent)"
              strokeWidth={2}
              className="circuit-scene-move-handle"
              onPointerDown={
                editor.onWireWaypointDown
                  ? (e) => {
                      e.stopPropagation();
  ...
  // pin handles for wiring: a big invisible hit-area + a dot that grows/glows on hover
  // (crosshair cursor + tooltip), so it's obvious WHERE a wire starts and finishes. The
  // armed pin pulses; when a wire is pending, the other pins read as connect targets.
  if (editor?.showPins) {
    const anyArmed = !!editor.wireStart;
    // a pin is CONNECTED if a wire ends on it, it shares a node with another pin, or it's grounded.
    // Connected pins render soft-filled; unconnected (floating) ones stay hollow, so you can see at
    // a glance which terminals still need wiring — without adding a separate marker.
    const pinConnected = (partId: string, pin: string): boolean => {
      if ((wireEndpoints.get(`${partId} ${pin}`) ?? 0) > 0) return true;
      const nid = doc.parts.find((x) => x.id === partId)?.pins?.[pin];
      if (!nid) return false;
      return nid === 'gnd' || (termsByNode.get(nid)?.length ?? 0) >= 2;
    };
    for (const p of doc.parts) {
      const def = getPart(p.kind);
      if (!def) continue;
      for (const pin of def.pins) {
        ...
        const connected = pinConnected(p.id, pin);
        pinHandles.push(
          <g
            key={`pin-${p.id}-${pin}`}
            className="lab-pin"
            data-armed={armed ? 'true' : undefined}
            data-target={anyArmed && !armed ? 'true' : undefined}
            role="button"
            tabIndex={editor.onPinActivate ? 0 : undefined}
            aria-label={
              armed
                ? `wiring from ${pin} of ${def.label}: activate another pin to finish`
                : `${pin} of ${def.label}: activate to start a wire`
            }
            onPointerDown={(e) => {
              e.stopPropagation();
            ...
            onKeyDown={
              editor.onPinActivate
                ? (e) => {
                    if (e.key === 'Enter' || e.key === ' ') {
                      e.preventDefault();
                      editor.onPinActivate!(p.id, pin);
                    }
                  }
                : undefined
            }
          >
            <circle cx={t.x} cy={t.y} r={13} fill="transparent" />
            <circle
              className="lab-pin-dot"
              cx={t.x}
              cy={t.y}
              r={armed ? 7.5 : 5.5}
              fill={
                armed
                  ? 'var(--stage-accent)'
                  : connected
                    ? 'color-mix(in oklab, var(--stage-accent) 30%, var(--stage-bg))'
                    : 'var(--stage-bg)'
              }
              stroke="var(--stage-accent)"
              strokeWidth={2}
            />
            <title>
              {armed ? 'Click another pin to finish the wire (Esc to cancel)' : 'Click to start a wire'}
            </title>
          </g>,
        );
      }
  ...
  // Skipped in the editor — every pin already shows a handle, so the gray dots on top of
  // them are redundant clutter; they only earn their place in the player (no handles),
  // where they distinguish a real junction from a mere crossing.
  const dots: ReactNode[] = [];
  if (!editor?.showPins) {
    for (const [nid, ts] of termsByNode)
      if (drawnNode(nid) && ts.length > 2) {
        const np = nodePos(nid);
        dots.push(<JunctionDot key={`j-${nid}`} x={np.x} y={np.y} live={false} />);
      }
    for (const [key, n] of wireEndpoints)
      if (n >= 2) {
        const [partId, pin] = key.split(' ');
        const t = partId && pin ? termOf(partId, pin) : undefined;
        if (t) dots.push(<JunctionDot key={`jw-${key}`} x={t.x} y={t.y} live={false} />);
      }
  }

  // CONNECTIVITY WARNING: a junction (node) that dead-ends — fewer than two wires — is a
  // wire that doesn't actually complete a connection (the classic "I drew a short but it did
  // nothing"). Flag it with a dashed danger ring so the dangling wire is visible, not silent.
  const warnMarks: ReactNode[] = [];
  for (const p of doc.parts) {
    if (p.kind !== 'node') continue;
    if ((wireEndpoints.get(`${p.id} p`) ?? 0) >= 2) continue;
    const t = termOf(p.id, 'p');
    if (t)
      warnMarks.push(
        <g key={`de-${p.id}`} pointerEvents="none">
          <circle
            cx={t.x}
            cy={t.y}
            r={8.5}
            ...
            opacity={0.9}
          />
          <title>Not connected: this junction needs at least two wires to complete a connection.</title>
        </g>,
      );
  }
  ...
  const svg = (
    <svg
      ...
      className={bare ? 'circuit-scene-svg' : undefined}
      role="img"
    ...
    >
      ...
      {flow &&
        !reduce &&
        live.map((seg, k) => <FlowDots key={`f${k}`} points={seg} phase={phase} spacing={54} r={2.3} />)}
      {dots}
      {warnMarks}
      {editor?.wirePreview &&
        (() => {
          ...
          return (
            <g pointerEvents="none">
              ...
              {mids.map((m, i) => (
                <circle key={i} cx={m.x} cy={m.y} r={3.5} fill={col} />
              ))}
      ...
      {doc.parts.map((p) => {
        ...
        const onActivate = selectable
          ? () => editor!.onSelect!(p.id)
          : tappable
            ? () => onPartTap!(p.id)
            : undefined;
        const label =
          !selectable && tappable ? `toggle ${def.label.toLowerCase()}` : `select ${def.label.toLowerCase()}`;
        return (
          <g
            key={`hit-${p.id}`}
            ...
            className={
              editor?.onPartPointerDown
                ? 'circuit-scene-part-target'
                : onActivate
                  ? 'circuit-scene-activate-target'
                  : undefined
            }
          ...
          >
            ...
            {(tappable || selectable) && (
              <rect x={p.at.x - 38} y={p.at.y - 28} width={76} height={56} rx={8} fill="transparent" />
            )}
            {glyph}
          </g>
        );
      ...
      {doc.parts.map((p) => {
        ...
        return (
          <g key={`dmg-${p.id}`} pointerEvents="none">
            <rect
              x={p.at.x - 34}
              y={p.at.y - 24}
              width={68}
            ...
            ({ end, t }) =>
              t && (
                <circle
                  ...
                  stroke="var(--stage-accent)"
                  strokeWidth={2.5}
                  className="circuit-scene-wire-handle"
                  role="button"
  ...
  return bare ? svg : <div className="circuit-scene-frame">{svg}</div>;
}
