/**
 * @classytic/labs/build — circuit builder, Phase 1 contracts.
 *
 * A circuit is DATA: a `CircuitDoc` of placed parts + electrical nodes. The same doc
 * is authored (config / future drag-drop), rendered (CircuitScene), and solved (the
 * MNA engine). One source of truth, no divergence.
 *
 * Electrical model: every part PIN names a node id; pins sharing a node id are
 * connected. Node "gnd" is the reference (engine node 0). A new component is added
 * by registering ONE PartDef (how it stamps the netlist + how it draws + its pins).
 */

import type { ReactNode } from 'react';
import type { Vec2 } from '@classytic/stage';
import type { Elem } from '@classytic/stage/circuit';
...
export interface CNode { id: string; at: Vec2 }

/** One end of a wire: a specific pin of a specific part. */
export interface PinRef { partId: string; pin: string }
