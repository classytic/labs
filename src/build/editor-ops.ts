...
export function updateProps(
  ...
  return {
    ...doc,
    parts: doc.parts.map((p) => (p.id === id ? { ...p, props: { ...p.props, ...patch } } : p)),
  };
}
...
export function rotatePart(doc: CircuitDoc, id: string): CircuitDoc {
  return {
    ...doc,
    parts: doc.parts.map((p) => (p.id === id ? { ...p, orient: p.orient === 'v' ? 'h' : 'v' } : p)),
  };
}
...
export function pruneJunctions(doc: CircuitDoc): CircuitDoc {
  ...
  for (;;) {
    ...
    const orphans = new Set(
      parts.filter((p) => p.kind === 'node' && (count.get(p.id) ?? 0) < 2).map((p) => p.id),
    );
...
export function setWireWaypoints(doc: CircuitDoc, wireId: string, mids: Vec2[]): CircuitDoc {
  return {
    ...doc,
    wires: (doc.wires ?? []).map((w) =>
      w.id === wireId ? { ...w, mid: mids.length ? mids : undefined } : w,
    ),
  };
}
...
export function setGround(doc: CircuitDoc, ref: PinRef): CircuitDoc {
  ...
  return {
    ...doc,
    parts: doc.parts.map((p) => (p.id === ref.partId ? { ...p, pins: { ...p.pins, [ref.pin]: next } } : p)),
  };
}
