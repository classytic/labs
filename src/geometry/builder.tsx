...
type Tool =
  'select' | 'point' | 'segment' | 'line' | 'circle' | 'intersect' | 'midpoint' | 'measure' | 'delete';
...
export function GeometryBuilder({
  ...
  const onBuild = (p: [number, number]): void => {
    ...
    if (tool === 'segment' || tool === 'line' || tool === 'midpoint' || tool === 'measure') {
      ...
      const a = next[0] as string,
        b = next[1] as string;
      if (tool === 'segment')
        set([...scene, { type: 'segment', from: a, to: b, color: 'var(--stage-good)' }]);
      else if (tool === 'line')
        set([
          ...scene,
          {
        ...
        set([...scene, { type: 'midpoint', id, of: [a, b], label: id }]);
      } else
        set([
          ...scene,
          {
    ...
    if (tool === 'circle') {
      ...
      set([
        ...scene,
        {
          type: 'circle',
          id: nextId(scene, 'c'),
          center: next[0] as string,
          through: next[1] as string,
    ...
    if (tool === 'intersect') {
      ...
      const add: GeoElement[] = [
        {
          type: 'intersect',
          id: id1,
          of: [next[0] as string, next[1] as string],
          pick: 0,
          label: id1,
  ...
  return (
    <div className="not-prose geometry-builder">
      <div className="geometry-builder-toolbar" role="toolbar" aria-label="Geometry construction tools">
        <span className="geometry-builder-title">{title}</span>
        {TOOLS.map((t) => (
          <Button
            key={t.id}
            type="button"
            ...
            className="geometry-builder-tool"
            aria-pressed={tool === t.id}
            data-selected={tool === t.id || undefined}
          >
        ...
        <Button
          ...
          variant="outline"
          onClick={() => set([])}
          className="geometry-builder-clear"
        >
          Clear
        </Button>
      </div>
      <p className="geometry-builder-hint" aria-live="polite">
        {hint}
        {pending.length ? ` · ${pending.length} selected` : ''}
      </p>
      <Stage
        view={VIEW}
        height={height}
        ...
        className="geometry-builder-stage"
      >
        <Grid />
        <Axes />
        ...
        {pending.map((id) => {
          ...
          return p ? (
            <Dot key={`sel-${id}`} x={p.x} y={p.y} r={8} color="var(--stage-warn)" opacity={0.6} />
          ) : null;
        })}
      </Stage>
    </div>
