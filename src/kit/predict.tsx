...
export function PredictPlot({
  ...
  // Gutters so the origin sits just off the corner and edge labels breathe. A taller
  // TOP gutter gives the y-axis title clear room above the top gridline (so "Volume"
  // isn't cramped against the 40 tick), and a wider LEFT gutter lets the tick numbers
  // and the crosshair readout sit off the axis without clipping.
  const view = { xMin: -xMax * 0.07, xMax: xMax * 1.05, yMin: -yMax * 0.06, yMax: yMax * 1.16 };
  ...
  return (
    ...
    >
      {/* axis titles, sit clearly in the gutters (the Label paints a bg halo so they
          stay legible over gridlines), the y-title above the top tick, x-title bottom-right. */}
      {yLabel && (
        <Label x={0} y={view.yMax} text={yLabel} dx={2} dy={-3} anchor="start" size={13} weight={750} />
      )}
      {xLabel && (
        <Label x={view.xMax} y={0} text={xLabel} dx={-2} dy={18} anchor="end" size={13} weight={750} />
      )}
