 ...
 * these let a lesson narrate the WHY *on* the figure: an annotation (leader line +
...
export interface DiagramLabelProps {
  ...
  /** Keep the estimated text box inside a pixel-space scene rectangle. */
  bounds?: { left: number; right: number; top: number; bottom: number };
  /** Compact long secondary annotations without changing their accessible scene description. */
  maxChars?: number;
  halo?: boolean;
}

/** Theme-safe SVG text with an optional background halo and edge clamping. */
export function DiagramLabel({
  ...
  maxChars,
  halo = true,
}: DiagramLabelProps): ReactNode {
  const shown =
    maxChars && text.length > maxChars ? `${text.slice(0, Math.max(1, maxChars - 1)).trimEnd()}…` : text;
  const width = shown.length * charW * (fontSize / 11);
  const leftExtent = anchor === 'start' ? 0 : anchor === 'middle' ? width / 2 : width;
  const rightExtent = anchor === 'end' ? 0 : anchor === 'middle' ? width / 2 : width;
  const px = bounds ? Math.max(bounds.left + leftExtent, Math.min(bounds.right - rightExtent, x)) : x;
  const py = bounds ? Math.max(bounds.top + fontSize, Math.min(bounds.bottom, y)) : y;
  const fill = tone === 'foreground' ? 'var(--stage-fg)' : TONE[tone];
  return (
    <text
      className={className}
      x={px}
      ...
      stroke={halo ? 'var(--stage-bg)' : undefined}
      strokeWidth={halo ? Math.max(3, fontSize * 0.3) : undefined}
 ...
 *  (Named `Pointer` to distinguish figure geometry from document-level prose blocks.) */
export function Pointer({
  ...
  const col = TONE[tone];
  const w = text.length * (charW * (fontSize / 11)) + 14,
    h = fontSize + 9;
  const bx = dx >= 0 ? x + dx : x + dx - w,
    by = y + dy - h / 2;
