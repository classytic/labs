...
export function roots(f: Fn1, a: number, b: number, opts: { steps?: number; tol?: number } = {}): number[] {
  ...
  for (let i = 1; i <= steps; i++) {
    ...
    if (fPrev * fx < 0) {
      ...
      // A sign change is NOT proof of a root: at a pole, f jumps from +infinity to -infinity, and
      // bisection happily converges on the asymptote. tan x would then be drawn with a dot sitting
      // on the x-axis at pi/2. Accept only where |f| is actually small, the same test pass 2 uses.
      const mid = (lo + hi) / 2;
  ...
  for (let i = 1; i < steps; i++) {
    ...
    if (Math.abs(c) <= Math.abs(p) && Math.abs(c) <= Math.abs(q) && Math.abs(c) < 0.05 * (1 + maxAbs)) {
      ...
      if (r != null && r >= a - grid && r <= b + grid && Math.abs(f(r)) <= acceptTol)
        pushUnique(out, r, grid);
    }
  }
  out.sort((u, v) => u - v);
  return out;
...
export function areaBetween(f: Fn1, g: Fn1, a: number, b: number, n = 1000): number {
  ...
  const xs = roots(d, a, b)
    .filter((x) => x > a + 1e-9 && x < b - 1e-9)
    .sort((u, v) => u - v);
  const cuts = [a, ...xs, b];
