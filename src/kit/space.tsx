  ...
  return (
    <g style={{ pointerEvents: 'none' }}>
      ...
      <g clipPath={`url(#${uid}-clip)`}>
        ...
        <path
          d={`M ${fmt(cx - R * 0.5)} ${fmt(cy - R * 0.45)} q ${fmt(R * 0.3)} ${fmt(-R * 0.15)} ${fmt(R * 0.5)} ${fmt(R * 0.1)} q ${fmt(R * 0.1)} ${fmt(R * 0.3)} ${fmt(-R * 0.15)} ${fmt(R * 0.4)} q ${fmt(-R * 0.35)} ${fmt(R * 0.1)} ${fmt(-R * 0.45)} ${fmt(-R * 0.2)} Z`}
          fill={land}
          opacity={0.88}
        />
        <path
          d={`M ${fmt(cx + R * 0.05)} ${fmt(cy - R * 0.1)} q ${fmt(R * 0.3)} ${fmt(-R * 0.05)} ${fmt(R * 0.4)} ${fmt(R * 0.25)} q ${fmt(-R * 0.05)} ${fmt(R * 0.45)} ${fmt(-R * 0.3)} ${fmt(R * 0.5)} q ${fmt(-R * 0.25)} ${fmt(-R * 0.1)} ${fmt(-R * 0.1)} ${fmt(-R * 0.7)} Z`}
          fill={land}
          opacity={0.82}
        />
        <ellipse
    ...
    <g style={{ pointerEvents: 'none' }}>
      <defs>
        <radialGradient id={`${uid}-core`} cx="42%" cy="38%" r="60%">
          ...
          <stop offset="45%" stopColor="#FDB813" />
          <stop offset="100%" stopColor="#F97316" />
        </radialGradient>
        <radialGradient id={`${uid}-glow`} cx="50%" cy="50%" r="50%">
          <stop offset="55%" stopColor="rgba(253,184,19,0)" />
          <stop offset="80%" stopColor="rgba(253,184,19,0.25)" />
