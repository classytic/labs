...
export function FrequencyTree({
  root,
  width = 360,
  ariaLabel = 'Natural-frequency tree',
}: FrequencyTreeProps): ReactNode {
  const NODE_W = 88,
    NODE_H = 34,
    ROW = 46,
    PADY = 8,
  ...
  return (
    ...
    >
      ...
      {placed.map((p, i) => {
        ...
        const fill = p.node.lit
          ? `color-mix(in oklab, ${accent} 26%, var(--stage-bg))`
          : 'color-mix(in oklab, var(--stage-fg) 5%, var(--stage-bg))';
        ...
        return (
          <g key={`n${i}`}>
            ...
            >
              {Math.round(p.node.count).toLocaleString()}
            </text>
            <text x={p.x + 9} y={p.y + 11} fontSize={9.5} fill="var(--stage-muted)">
              {p.node.label}
            </text>
          </g>
        );
