...
export function normalizeMathInput(raw: string): string {
  ...
  // 2x → 2*x, 3pi → 3*pi, 2(x+1) → 2*(x+1). The lookahead spares scientific notation: a student
  // writing 3.0e8 for the speed of light means one number, not 3.0 times e. Only an `e` that
  // begins a real exponent is spared, so `2e` is still 2 times Euler's number and `2exp(x)` still
  // multiplies. The expression compiler reads `3.0e8` as a numeric literal already.
...
export function checkExpression(student: string, answer: string, opts: ExprCheckOpts = {}): boolean {
  ...
  try {
    if (
      isZeroNode(
        simplify(parseExpr(`(${normalizeMathInput(student)}) - (${normalizeMathInput(answer)})`)),
        opts.tol ?? 1e-9,
      )
    )
      return true;
