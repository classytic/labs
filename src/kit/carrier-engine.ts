 ...
 *
 *  Pass CONSECUTIVE seeds, and give each region its own offset (1, 11, 21…). A stride breaks
 *  the spread: with `i * 7` the y step is 7 × 0.5698 ≈ 3.989, which is −0.011 of the box per
 *  carrier, so every carrier in a region landed on one horizontal line. */
...
export function recombine(
  ...
  r: number,
  step: number,
  respawn?: (c: Carrier, step: number) => Partial<Carrier>,
): Carrier[] {
  if (!r) return cs;
  const r2 = r * r;
