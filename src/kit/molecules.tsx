...
const bond = (x1: number, y1: number, x2: number, y2: number): ReactNode => (
  <line
    key={`b${x1},${y1},${x2},${y2}`}
    x1={x1}
    y1={y1}
...
const MOLECULES: Record<MoleculeKind, { label: string; draw: () => ReactNode }> = {
  ...
  h2o: {
    ...
    draw: () => (
      <g>
        {bond(0, 0.12, -0.58, -0.42)}
        {bond(0, 0.12, 0.58, -0.42)}
        {atom(0, 0.12, 0.44, O, 'O')}
        {atom(-0.58, -0.42, 0.28, H, 'H', DARK)}
        {atom(0.58, -0.42, 0.28, H, 'H', DARK)}
      </g>
    ),
  },
  glucose: {
    ...
    draw: () => {
      const pts = Array.from({ length: 6 }, (_, i) => {
        ...
        return [Math.cos(a) * 0.66, Math.sin(a) * 0.66] as [number, number];
      });
      return (
        <g>
          <polygon
  ...
  AB: {
    ...
    draw: () => (
      <g>
        {bond(-0.4, 0, 0.4, 0)}
        {atom(-0.44, 0, 0.46, 'var(--stage-accent)', 'A')}
        {atom(0.44, 0, 0.46, 'var(--stage-accent-2)', 'B')}
      </g>
    ),
  },
};
...
export function MoleculeGlyph({
  ...
  return (
    <g transform={`translate(${x} ${y})`}>
      ...
      {showLabel && (
        <text x={0} y={s + 12} fill="var(--stage-fg)" fontSize={11} fontWeight={600} textAnchor="middle">
          {m.label}
        </text>
      )}
    </g>
