...
export function VesselGlyph({
  ...
  objects = 0,
  objectColor = 'var(--stage-object, #e85aa6)',
  guessFrac,
  guessTone = 'idle',
  ...
  if (objects > 0) {
    ...
    for (let i = 0; i < objects; i++) {
      ...
      cubes.push(
        <g
          key={`o${i}`}
          style={{ animation: `vessel-drop 0.45s cubic-bezier(0.3,1.3,0.6,1) ${i * 0.08}s backwards` }}
        >
  ...
  return (
    <g>
      {/* liquid body + a left cylinder sheen + meniscus (rises smoothly on change) */}
      <rect
        x={lx}
        y={liquidTop}
        width={innerW}
      ...
      <path
        ...
        strokeLinecap="round"
      />
      {/* a soft glass reflection down the left wall */}
      <line
        x1={lx + 2.5}
        y1={innerTop + 5}
        x2={lx + 2.5}
...
export function Vessel({
  ...
  // When a unit is shown, reserve a small header band above the vessel so the unit
  // ("mL") sits clear of the top scale number instead of colliding with it.
  const padTop = unit && scaleMax != null ? 22 : 10;
    ...
    for (let v = 0; v <= scaleMax + 1e-6; v += step) {
      ...
      ticks.push(
        <g key={`t${v}`}>
          ...
          <text x={vx + vw + 8} y={ty} fontSize={10} fill={MUTED} dominantBaseline="middle">
            {v}
          </text>
        </g>,
      );
  ...
  return (
    ...
    >
      ...
      />
      {unit && scaleMax != null && (
        // In the header band, above the top tick (padTop reserves the room).
        <text x={vx + vw + 5} y={7} fontSize={9} fontWeight={600} fill={MUTED} dominantBaseline="hanging">
          {unit}
        </text>
      )}
      {label && (
        <text x={width / 2} y={height - 8} fontSize={12} fontWeight={700} fill={FG} textAnchor="middle">
          {label}
        </text>
      )}
    </svg>
