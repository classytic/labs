...
/** @deprecated Use `useLearningSequence` with `LearningSequenceNav`. */
...
/** @deprecated Use `useLearningSequence` with authored step data. */
...
export function StepNav({
  ...
  return (
    <div className="lab-stepnav">
      <ActionButton className="lab-btn-ghost" onClick={steps.prev} disabled={steps.atStart}>
        ← Back
      </ActionButton>
      <span className="lab-dots" aria-hidden>
        {Array.from({ length: steps.total }, (_, i) => (
          <span key={i} className="lab-dot" data-on={i <= steps.step} />
        ))}
      </span>
      <ActionButton onClick={steps.next} disabled={steps.atEnd}>
        {steps.atEnd ? doneLabel : nextLabel}
      </ActionButton>
    </div>
  );
}
