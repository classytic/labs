...
export function ThermometerGlyph({
  ...
  return (
    <g aria-hidden>
      {/* mercury bulb + a soft rim (not a hard metal outline) + a sheen highlight */}
      <circle cx={cx} cy={bulbCy} r={bulbR} fill={col} stroke={GLASS} strokeWidth={1.5} />
      <ellipse
        cx={cx - bulbR * 0.32}
        cy={bulbCy - bulbR * 0.34}
        rx={bulbR * 0.3}
        ry={bulbR * 0.22}
...
export function BurnerGlyph({
  ...
  const teardrop = (scale: number, fill: string, op: number): ReactNode => {
    ...
    return (
      <path
        d={`M ${cx} ${baseY + 3} C ${cx - hw} ${baseY - ht * 0.4} ${cx - hw * 0.5} ${ty} ${cx} ${ty} C ${cx + hw * 0.5} ${ty} ${cx + hw} ${baseY - ht * 0.4} ${cx} ${baseY + 3} Z`}
        fill={fill}
        opacity={op}
      />
    );
  ...
  return (
    <g aria-hidden>
      ...
      {cold > 0.02 && (
        <g
          opacity={0.5 + cold * 0.45}
          stroke="color-mix(in oklab, #2b7fff 70%, var(--stage-bg))"
          strokeWidth={2}
          strokeLinecap="round"
        >
          {[-1, 0, 1].map((k) => {
            const sx = cx + k * w * 0.28,
              len = 10 + cold * 12;
            return (
              <g key={k}>
                <line x1={sx} y1={baseY + 14} x2={sx} y2={baseY + 14 + len} />
                <line x1={sx - 4} y1={baseY + 18} x2={sx + 4} y2={baseY + 22} />
                <line x1={sx + 4} y1={baseY + 18} x2={sx - 4} y2={baseY + 22} />
              </g>
            );
          })}
