...
/**
 * ── keeping the axis numbers off the figure ──────────────────────────────────
 *
 * A dot drawn ON an axis lands exactly where that axis prints its numbers (an intercept on the y
 * axis, a focus on the x axis), and the number comes out half-covered. The stage places its numbers
 * around a list of zones for that reason, and these build the list.
 *
 * The type and the helper are declared HERE rather than imported from the stage on purpose: a
 * consumer may still be on a stage that predates the feature, and importing a name a module does
 * not export fails at load. Handed over as a prop it is simply ignored there, and the numbers print
 * as they always did. `PlaneAxes` is the one place that hands it over.
 */
...
/** The zone a `<Label>` covers, from the same props the label is given. */
export function clearOfLabel(
  x: number,
  y: number,
  text: string,
  {
    ...
    dx = 0,
    dy = 0,
  }: { size?: number; anchor?: 'start' | 'middle' | 'end'; dx?: number; dy?: number } = {},
): ClearZone {
  const w = text.length * size * 0.58; // digits and letters average a little over half an em
  const shift = anchor === 'start' ? w / 2 : anchor === 'end' ? -w / 2 : 0;
  return { x, y, dx: dx + shift, dy, rx: w / 2 + 3, ry: size * 0.5 + 2 };
}

/** The zone a dot (or a draggable handle, at the default size) covers. */
export const dotZone = (p: Vec2, r = 13): ClearZone => ({ x: p.x, y: p.y, rx: r, ry: r });

type StageAxesProps = Parameters<typeof Axes>[0];

/** Stage axes whose numbers keep off the given zones. Use wherever a figure draws ON an axis. */
export function PlaneAxes({
  keepClear,
  ...rest
}: StageAxesProps & { keepClear?: readonly ClearZone[] }): ReactNode {
  return <Axes {...rest} {...({ keepClear } as Record<string, unknown>)} />;
}
...
export interface CoordPlaneProps {
  ...
  labels?: boolean;
  /**
   * Places the axis numbers must leave clear: the dots and labels this figure draws ON an axis
   * (an intercept, a focus, a directrix name). Points get a dot-sized zone; use `clearOfLabel`
   * from the stage for a label.
   */
...
export function CoordPlane({
  ...
  return (
    <Stage view={view} height={height} pad={innerPad} preserveAspect={preserveAspect} ariaLabel={ariaLabel}>
      ...
      <PlaneAxes ticks step={step} stepX={stepX} stepY={stepY} labels={labels} keepClear={keepClear} />
      {children}
    </Stage>
  );
}
