...
/**
 * Geometry / vector annotations built on @classytic/stage primitives (angle arcs,
 * right-angle marks, labelled vectors). Pure stage — NO electronics import — so a
 * math/vector lab that only annotates never drags in the circuit-glyph library.
 * Render these INSIDE a `<Stage>` (they read the coordinate context).
 */
...
/**
 * An angle arc drawn at point `at`, swept from direction `from` to direction `to`
 * (both as vectors from `at`), with an optional label (e.g. "θ" or "53°"). Radius is
 * in pixels so it stays a constant on-screen size; computed in pixel space so it's
 * correct regardless of the (possibly non-uniform) view scale.
 */
...
export function AngleArc({
  ...
  const sx = o[0] + rPx * Math.cos(a1),
    sy = o[1] + rPx * Math.sin(a1);
  const ex = o[0] + rPx * Math.cos(a2),
    ey = o[1] + rPx * Math.sin(a2);
  const am = a1 + da / 2;
  const lx = o[0] + (rPx + 13) * Math.cos(am),
    ly = o[1] + (rPx + 13) * Math.sin(am);
  return (
    <Fragment>
      <path
...
export function RightAngleMark({
  ...
  const c = useCoords();
  const o = c.toPx(at.x, at.y);
  const dir = (d: Vec2): [number, number] => {
...
/**
 * An arrow (tail → tail+comp) with an optional tip label and optional dashed x/y
 * component decomposition, the SVG equivalent of the old canvas
 * `drawVector(..., { label, components })`.
 */
export function LabeledVector({
  tail = { x: 0, y: 0 },
  comp,
