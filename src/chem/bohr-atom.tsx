...
const NUCLEONS: { dx: number; dy: number; p: boolean }[] = [
  { dx: 0, dy: 0, p: true },
  { dx: 0.6, dy: 0.25, p: false },
  { dx: -0.55, dy: 0.35, p: true },
  { dx: 0.3, dy: -0.55, p: false },
  { dx: -0.4, dy: -0.5, p: true },
  { dx: 0.7, dy: -0.25, p: true },
  { dx: -0.7, dy: -0.1, p: false },
  { dx: 0.1, dy: 0.6, p: false },
];
...
const ACTIVITY: AuthoredActivity = {
  ...
  objectives: ['Connect atomic number to electron count', 'Read shell configurations'],
  steps: [
    {
      id: 'observe',
      phase: 'observe',
      title: 'Read an electron configuration',
      lead: 'Connect atomic number to the occupied shells.',
      reveal: ['model'],
    },
    {
      id: 'act',
      phase: 'act',
      title: 'Build another atom',
      lead: 'Change the proton count and watch electrons fill shells.',
      controls: true,
    },
    {
      id: 'explain',
      phase: 'explain',
      title: 'Explain a closed shell',
      lead: 'Use the shell capacities and configuration as evidence.',
    },
  ],
};
...
export function BohrAtom({
      ...
      >
        {shells.map((_count, i) => (
          <Circle
            key={`s-${i}`}
            center={{ x: 0, y: 0 }}
            r={shells.length === 1 ? baseR + 2.2 : baseR + i * gap}
            color="var(--stage-fg)"
            opacity={0.25}
            weight={1}
            fill="none"
  ...
  const aside = (
    <StatList>
      <Stat label="element" value={SYMBOLS[z] ?? ', '} />
      <Stat label="Z" value={z} />
      <Stat label="config" value={shells.join(', ')} />
    </StatList>
  );
  ...
  return (
    <AuthoredActivityRuntime
      ...
      activity={activity}
      activityId="bohr-atom"
      eyebrow="Atomic structure"
      title={title}
      description="Change atomic number and watch electrons fill the shells 2, 8, 8, …"
      status={
        <>
          <span>{SYMBOLS[z]}</span>
          <span>Z {z}</span>
      ...
      observation={
        <>
          A neutral atom has {z} electrons arranged as {shells.join(', ')} across {shells.length} occupied
          shell{shells.length === 1 ? '' : 's'}.
        </>
      }
      transcript={
        <p>
          {SYMBOLS[z]} has atomic number {z} and electron configuration {shells.join(', ')}.
        </p>
      }
    >
      {figure}
