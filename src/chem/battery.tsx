...
const ACTIVITY: AuthoredActivity = {
  ...
  title: 'Galvanic cell: electrons on the move',
  objectives: ['Trace electrons from anode to cathode', 'Relate resistance to current'],
  steps: [
    {
      id: 'observe',
      phase: 'observe',
      title: 'Trace electron flow',
      lead: 'Follow electrons from oxidation at the anode to reduction at the cathode.',
      reveal: ['model'],
    },
    {
      id: 'act',
      ...
      lead: 'Change resistance and compare current while EMF stays fixed.',
      controls: true,
    },
    {
      id: 'explain',
      phase: 'explain',
      title: 'Connect the half-reactions',
      lead: 'Use the electrode labels and current as evidence.',
    },
  ],
};
  ...
  const figure = (
    <PlayWrap gate={gate}>
      ...
      >
        ...
        />
        <Segment
          from={{ x: LX - 16, y: BTOP - 1.2 }}
          to={{ x: LX + 16, y: BTOP - 1.2 }}
          color="var(--stage-accent)"
          opacity={0.7}
          weight={1.5}
        />
        <Segment
          from={{ x: RX - 16, y: BTOP - 1.2 }}
          to={{ x: RX + 16, y: BTOP - 1.2 }}
    ...
    <AuthoredActivityRuntime
      ...
      status={
        <>
          ...
          <span>{(current * 1000).toFixed(0)} mA</span>
        </>
      }
      evidence={aside}
      controls={controls}
