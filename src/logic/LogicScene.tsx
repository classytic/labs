...
import { Wire, JunctionDot } from '../kit/electronics/index.js';
...
export function LogicScene({
  ...
  sol.levels.forEach((ids, lvl) =>
    ids.forEach((id, i) => raw.set(id, { x: MX + lvl * COL, y: slotC(ids.length, i), gate: lvl > 0 })),
  );
  ...
  // ── long connections go AROUND, not through. A comb tooth runs horizontally at its sink's height,
  //    which is fine to the next column and wrong across a column that has a gate at that height:
  //    the tooth passes under the gate's body and merges visually with that gate's own inputs (a
  //    2-to-1 multiplexer's select wire ran straight through the upper AND this way). Such a tooth is
  //    rerouted through a LANE above or below the whole diagram, then up or down into its sink just
  //    before the sink's column, the way a person drawing a schematic would. Lanes are staggered so
  //    two rerouted nets never share a line, and the canvas grows to hold them.
  const columnOf = (x: number): number => Math.round((x - MX) / COL);
  const gateBoxes = doc.gates
    .map((g) => ({ p: pos.get(g.id) }))
    .filter((item): item is { p: P } => !!item.p)
    .map(({ p }) => ({ col: columnOf(p.x), top: p.y - GATE / 2 - 6, bottom: p.y + GATE / 2 + 6 }));
  const blockedAt = (y: number, fromCol: number, toCol: number): boolean =>
    gateBoxes.some((box) => box.col > fromCol && box.col < toCol && y > box.top && y < box.bottom);

  let gatesTop = Infinity,
    gatesBottom = -Infinity;
  pos.forEach((p) => {
    gatesTop = Math.min(gatesTop, p.y - halfH(p.gate) - 18); // clear an input switch's label too
    gatesBottom = Math.max(gatesBottom, p.y + halfH(p.gate));
  });
  outY.forEach((y) => {
    gatesTop = Math.min(gatesTop, y - LED_R);
    gatesBottom = Math.max(gatesBottom, y + LED_R + 20); // an LED's label sits under it
  });
  const midY = (gatesTop + gatesBottom) / 2;

  type Detour = { src: string; sink: { x: number; y: number }; laneY: number; approachX: number };
  const detours: Detour[] = [];
  const longSinks = new Set<{ x: number; y: number }>();
  let lanesAbove = 0,
    lanesBelow = 0;
  const approachesInto = new Map<number, number>();
  sinkPorts.forEach((list, src) => {
    const sp = outPort(src);
    if (!sp) return;
    const fromCol = columnOf(sp.x - SW_W / 2);
    for (const sink of list) {
      const toCol = columnOf(sink.x);
      if (toCol - fromCol < 2 || !blockedAt(sink.y, fromCol, toCol)) continue;
      const above = (sp.y + sink.y) / 2 < midY;
      const laneY = above ? gatesTop - 10 - lanesAbove++ * 10 : gatesBottom + 10 + lanesBelow++ * 10;
      const arriving = approachesInto.get(toCol) ?? 0;
      approachesInto.set(toCol, arriving + 1);
      detours.push({ src, sink, laneY, approachX: sink.x - 12 - arriving * 8 });
      longSinks.add(sink);
    }
  });
  const viewTop = Math.min(0, gatesTop - 10 - lanesAbove * 10);
  const viewBottom = Math.max(H, gatesBottom + 10 + lanesBelow * 10);

  const wires: ReactNode[] = [];
  const junctions: ReactNode[] = [];
  sinkPorts.forEach((all, src) => {
    const sp = outPort(src);
    if (!sp) return;
    const on = lit(src),
      col = hue(src),
      tx = trunkX.get(src) ?? sp.x + 14;
    const list = all.filter((s) => !longSinks.has(s));
    const lanes = detours.filter((d) => d.src === src);
    // The trunk reaches every lane this net detours through, so the detour leaves from the trunk.
    const ys = [sp.y, ...list.map((s) => s.y), ...lanes.map((d) => d.laneY)];
    ...
    const tap = (yy: number, k: string): void => {
      if (yy > lo + 0.5 && yy < hi - 0.5)
        junctions.push(<JunctionDot key={`j-${src}-${k}`} x={tx} y={yy} r={3} live={on} color={col} />);
    };
    tap(sp.y, 's');
    list.forEach((s, i) => tap(s.y, `${i}`));
    lanes.forEach((d, i) => tap(d.laneY, `lane-${i}`));
  });

  return (
    <div className="logic-scene-frame" style={{ '--logic-scene-width': `${W}px` } as CSSProperties}>
      <svg
      ...
      >
        ...
        {doc.inputs.map((inp) => {
          ...
          return (
            <g
              key={inp.id}
              role={onToggleInput ? 'button' : undefined}
              tabIndex={onToggleInput ? 0 : undefined}
              aria-label={onToggleInput ? `toggle ${inp.label ?? inp.id}` : undefined}
              className="logic-scene-control"
              data-interactive={!!onToggleInput || undefined}
                ...
                (() => {
                  ...
                  return (
                    <text
                      ...
                      x={on ? p.x + SW_H * 0.55 : p.x + SW_W - SW_H * 0.55}
                      y={p.y}
                      fill="var(--stage-fg)"
                      fontSize={11}
                      fontWeight={800}
        ...
        {doc.outputs.map((o) => {
          ...
          return (
            <g
              key={o.id}
              role={onOutputClick ? 'button' : undefined}
              tabIndex={onOutputClick ? 0 : undefined}
              aria-label={onOutputClick ? `set ${o.label ?? o.id}` : undefined}
              className="logic-scene-control"
              data-interactive={!!onOutputClick || undefined}
