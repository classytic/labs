...
export function addNode(
  ...
  return {
    doc: { ...doc, gates: [...doc.gates, { id, kind, in: Array(arity).fill(''), x: at.x, y: at.y }] },
    id,
  };
}
...
export function connect(doc: LogicDoc, fromId: string, sink: { nodeId: string; slot?: number }): LogicDoc {
  ...
  return {
    ...
    gates: doc.gates.map((g) =>
      g.id === sink.nodeId ? { ...g, in: g.in.map((v, i) => (i === (sink.slot ?? 0) ? fromId : v)) } : g,
    ),
...
export function disconnect(doc: LogicDoc, sink: { nodeId: string; slot?: number }): LogicDoc {
  return {
    ...doc,
    gates: doc.gates.map((g) =>
      g.id === sink.nodeId ? { ...g, in: g.in.map((v, i) => (i === (sink.slot ?? 0) ? '' : v)) } : g,
    ),
