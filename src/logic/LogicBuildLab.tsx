...
function columnByLabels(doc: LogicDoc, inputLabels: string[], outputLabel: string): boolean[] | null {
  ...
  for (let m = 0; m < 2 ** n; m++) {
    ...
    const probe: LogicDoc = {
      ...doc,
      inputs: doc.inputs.map((i) => (setVals.has(i.id) ? { ...i, value: setVals.get(i.id)! } : i)),
    };
...
const eqCol = (a: boolean[] | null, b: boolean[] | null): boolean =>
...
export function LogicBuildLab({
  doc: doc0,
  goal,
  title = 'Build the circuit',
  prompt,
  activity = 'logic-build',
}: LogicBuildProps = {}): ReactNode {
  const goalDoc: LogicDoc | null =
    goal === undefined ? null : typeof goal === 'string' ? presetDoc(goal) : goal;
  const [doc, setDoc] = useState<LogicDoc>(
    () => doc0 ?? (goalDoc ? seedFromGoal(goalDoc) : { size: SIZE, inputs: [], gates: [], outputs: [] }),
  );

  const inputLabels = goalDoc ? goalDoc.inputs.map(labelOf) : [];
  const targets = goalDoc
    ? goalDoc.outputs.map((o) => ({
        label: labelOf(o),
      ...
      <table className="logic-truth-table">
        <thead>
          <tr>
            ...
            {perOutput.map((p) => (
              <th key={p.label} data-matched={p.ok || undefined}>
                {p.label}
                {p.ok ? ' ✓' : ''}
              </th>
            ))}
        ...
        <tbody>
          {Array.from({ length: 1 << inputLabels.length }, (_, m) => (
            <tr key={m}>
              {inputLabels.map((_l, k) => (
                <td key={k}>{(m >> (inputLabels.length - 1 - k)) & 1}</td>
              ))}
              {targets.map((t) => (
                <td key={t.label} data-value={t.col?.[m] || undefined}>
                  {t.col?.[m] ? 1 : 0}
                </td>
              ))}
            </tr>
  ...
  return (
    <LogicEditor
      ...
      description={
        prompt ??
        (goalDoc
          ? 'Drag gates from the palette and wire them so each LED follows the target column for every input.'
          : 'Build any circuit: drop sources, gates, and LEDs, wire them up, and flip the switches to watch the signal flow.')
      }
    >
      {goalDoc && (
        <div className="logic-build-brief">
          <StatusPill ok={solved} role="status">
            {solved ? '✓ your circuit matches the target' : 'wire gates so each output matches its column'}
          </StatusPill>
          <div className="logic-build-target">
            <span>Target truth table (build a circuit that does this)</span>
            {targetTable}
          </div>
        </div>
      )}
