...
export interface LogicSolution {
  ...
  allGoalsMet: boolean;
  /** Structural problems found while evaluating. Dangling references, unknown gates and
   * feedback that never settles are reported explicitly, never silently treated as logic. */
  diagnostics: LogicDiagnostic[];
  /** True when nothing was reported: the network is well formed and reached a stable state. */
  valid: boolean;
  /** Every gate's settled value. Feed this back as `evaluate`'s `seed` on the next
   *  evaluation and a feedback circuit REMEMBERS: this is what turns two cross-coupled
   *  NAND gates into a latch rather than a fresh guess each render. */
  settled: Record<string, boolean>;
  /** Did the network reach a stable state? False means it oscillates. */
  stable: boolean;
  /** Passes taken to settle. 1 means purely combinational with no propagation delay to show. */
  settledIn: number;
}

export interface LogicDiagnostic {
  code: 'combinational-cycle' | 'unknown-node' | 'unknown-gate' | 'invalid-sr-input';
  nodeId: string;
  message: string;
  /** For a cycle, the closed path that produced it. */
  path?: string[];
}
