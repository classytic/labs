...
const Q: ChallengeQuestion[] = [
  {
    id: 'narrower',
    prompt:
      "Same urn, draw n. One version puts each ball back (independent draws), the other keeps it out. Which distribution of 'number of winners' is narrower (less spread)?",
    choices: [
...
export function HypergeometricLab({
  N: N0 = 10,
  K: K0 = 4,
  n: n0 = 3,
  ...
  const p = Kc / N;
  const bin = useMemo(
    () => Array.from({ length: nc + 1 }, (_, k) => nCr(nc, k) * p ** k * (1 - p) ** (nc - k)),
    [nc, p],
  );
  const hyp = useMemo(
    () =>
      Array.from({ length: nc + 1 }, (_, k) =>
        k <= Kc && nc - k <= N - Kc ? (nCr(Kc, k) * nCr(N - Kc, nc - k)) / nCr(N, nc) : 0,
      ),
    [N, Kc, nc],
  );
  const mean = nc * p;
  const varBin = nc * p * (1 - p),
    varHyp = (varBin * (N - nc)) / (N - 1 || 1);
      ...
      >
        {bin.map((_, k) => (
          <g
            ...
            role="button"
            tabIndex={0}
            aria-label={`compare probability of exactly ${k} winners`}
            aria-pressed={sel === k}
          ...
          >
            ...
            <rect
              ...
              y={yOf(bin[k]!)}
              width={bw}
              height={Math.max(0, MT + PH - yOf(bin[k]!))}
              rx={2}
              fill={BIN}
              opacity={0.85}
            />
            <rect
              ...
              y={yOf(hyp[k]!)}
              width={bw}
              height={Math.max(0, MT + PH - yOf(hyp[k]!))}
              rx={2}
              fill={HYP}
              opacity={0.9}
            />
        ...
        {/* legend */}
        <rect x={W - 150} y={MT + 2} width={10} height={10} fill={BIN} />
        <text x={W - 136} y={MT + 11} fontSize={10} fill="var(--stage-fg)">
          with (binomial)
        </text>
        <rect x={W - 150} y={MT + 16} width={10} height={10} fill={HYP} />
        <text x={W - 136} y={MT + 25} fontSize={10} fill="var(--stage-fg)">
          without (hyper)
        </text>
      </svg>
    </div>
  ...
  const aside = (
    <>
      <Readout
        ...
        value={
          <>
            <span className="probability-series" data-series="with">
              {varBin.toFixed(2)}
            </span>{' '}
            vs{' '}
            <span className="probability-series" data-series="without">
              {varHyp.toFixed(2)}
            </span>
          </>
        }
        sub={
          <>
            same <Tex tex={`\\mu = n \\cdot K / N = ${mean.toFixed(2)}`} />
          </>
        }
      />
      {sel != null ? (
        <div className="probability-inspector">
          <b>P(exactly {sel} winners)</b>
          <span className="probability-series" data-series="with">
            with:{' '}
            <Tex
              tex={`C(${nc},${sel}) \\cdot ${p.toFixed(2)}^{${sel}} \\cdot ${(1 - p).toFixed(2)}^{${nc - sel}} = ${f3(bin[sel]!)}`}
            />
          </span>
          <span className="probability-series" data-series="without">
            without:{' '}
            <Tex tex={`C(${Kc},${sel}) \\cdot C(${N - Kc},${nc - sel}) / C(${N},${nc}) = ${f3(hyp[sel]!)}`} />
          </span>
        </div>
      ) : (
        <p className="lab-prompt probability-prompt">
          Click a bar to compare the two probabilities. The blue (with replacement) is always a touch wider
          than the orange (without).
        </p>
      )}
    </>
  );
  ...
  const controls = (
    <div className="lab-activity-fields">
      <Field label="population N" value={N}>
        <Slider
          value={N}
          min={2}
          max={60}
      ...
      <Field label="winners K" value={Kc}>
        <Slider value={Kc} min={0} max={N} step={1} onChange={setK} ariaLabel="winners" />
      </Field>
      <Field label="draw n" value={nc}>
        <Slider value={nc} min={1} max={N} step={1} onChange={setN_} ariaLabel="draw size" />
      </Field>
    </div>
  );
  ...
  const footer = (
    <>
      ...
      <p className="lab-prompt discrete-prompt-muted">
        Without replacement is narrower by the factor (N−n)/(N−1) = {((N - nc) / (N - 1 || 1)).toFixed(2)}.
        Push N up (with K/N fixed) and the bars converge, for a huge population, taking one out barely changes
        the odds.
      </p>
  ...
  return (
    <Activity.Root className="discrete-hypergeometric-activity">
      <Activity.Header>
        <Activity.Heading
          eyebrow="Finite populations"
          title={title}
          description={
            prompt ??
            'Compare repeated independent draws with sampling from a population that changes after every draw.'
          }
        />
      ...
      <Activity.Workspace>
        <Activity.Canvas label="Binomial and hypergeometric distributions">{figure}</Activity.Canvas>
        <Activity.Inspector label="Spread evidence and population controls">
          {aside}
          {controls}
        </Activity.Inspector>
      </Activity.Workspace>
      <Activity.Feedback>
        ...
        <div>
          {sel != null
            ? `At exactly ${sel} winners, replacement gives ${f3(bin[sel]!)} while no replacement gives ${f3(hyp[sel]!)}.`
            : `Both models have mean ${mean.toFixed(2)}, but without replacement multiplies variance by ${((N - nc) / (N - 1 || 1)).toFixed(2)}.`}
        </div>
      </Activity.Feedback>
      <section className="lab-authored-task" aria-label="Explain sampling without replacement">
        {footer}
      </section>
      <Activity.LiveRegion>
        Population {N}, with {Kc} winners and {nc} draws. Both means are {mean.toFixed(2)}. With-replacement
        variance {varBin.toFixed(2)}; without-replacement variance {varHyp.toFixed(2)}
        {sel != null
          ? `; at ${sel} winners probabilities are ${f3(bin[sel]!)} with replacement and ${f3(hyp[sel]!)} without replacement`
          : ''}
        .
      </Activity.LiveRegion>
      <Activity.Transport>
        <div className="lab-transport-state" aria-live="polite">
          <strong>
            {ch.allCorrect
              ...
              : sel != null
                ? `Comparing k = ${sel}`
                : 'Inspect the paired bars'}
          </strong>
          <span>
            variance {varBin.toFixed(2)} vs {varHyp.toFixed(2)}
          </span>
        </div>
      </Activity.Transport>
    </Activity.Root>
