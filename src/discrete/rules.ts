...
import { factorial, nPr, nCr, permutationsWithRepetition, ruleOfProduct, ruleOfSum } from './core/combinatorics.js';
...
export function explainNCr(n: number, r: number): Worked {
  ...
  return calc()
    .step(`C(${n},${r}) = \\frac{P(${n},${r})}{${r}!} = \\frac{${n}!}{${r}!\\,(${n}-${r})!}`, 'order ignored: divide out the r! re-orderings')
...
export const PRODUCT_RULE: RuleDef = {
  id: 'rule-of-product', name: 'Rule of product (AND → multiply)',
  formula: '\\#(A \\text{ then } B) = \\#A \\times \\#B',
  analogy: 'An outfit = pick a shirt AND pick trousers: 4 shirts × 3 trousers = 12 outfits.',
  inputs: [{ key: 'a', label: 'stage 1', default: 4, min: 1, max: 20 }, { key: 'b', label: 'stage 2', default: 3, min: 1, max: 20 }, { key: 'c', label: 'stage 3', default: 2, min: 1, max: 20 }],
  compute: (v) => explainProduct([v.a ?? 1, v.b ?? 1, v.c ?? 1]),
  derivation: [
    { tex: '\\text{Each of the } \\#A \\text{ first choices}' },
    { tex: '\\text{pairs with all } \\#B \\text{ second choices}', note: 'independent' },
    { tex: '\\Rightarrow \\#A \\times \\#B \\text{ outcomes}', note: 'an A × B grid' },
  ],
  tricks: ['Use it when steps happen in SEQUENCE ("and then"), independently.', 'Repetition allowed ⇒ same count each stage ⇒ nʳ.', 'No repetition ⇒ the pool shrinks (n, n−1, …) ⇒ P(n,r).'],
};

export const SUM_RULE: RuleDef = {
  id: 'rule-of-sum', name: 'Rule of sum (OR → add)',
  formula: '\\#(A \\text{ or } B) = \\#A + \\#B \\quad (A \\cap B = \\varnothing)',
  analogy: 'A drink = a tea OR a coffee: 3 teas + 5 coffees = 8 choices.',
  inputs: [{ key: 'a', label: 'case A', default: 3, min: 0, max: 50 }, { key: 'b', label: 'case B', default: 5, min: 0, max: 50 }],
  compute: (v) => explainSum([v.a ?? 0, v.b ?? 0]),
  tricks: ['Use it for a choice BETWEEN exclusive cases ("either … or").', 'If the cases OVERLAP, subtract the double-count: |A∪B| = |A|+|B|−|A∩B|.', 'The exam trap in one line: "and" → ×, "or" → +.'],
};
...
export const FACTORIAL_RULE: RuleDef = {
  ...
  derivation: [
    ...
    { tex: 'n \\times (n-1) \\times \\cdots \\times 1 = n!', note: 'rule of product' },
  ],
  tricks: ['0! = 1 (one way to arrange nothing).', 'Grows explosively: 13! > 6 billion.', 'The engine under both P(n,r) and C(n,r).'],
};

export const PERMUTATION_RULE: RuleDef = {
  id: 'permutation', name: 'Permutations: P(n, r)',
  formula: 'P(n,r) = \\frac{n!}{(n-r)!}',
  analogy: 'Gold / silver / bronze to r of n runners: ORDER matters.',
  inputs: [{ key: 'n', label: 'n', default: 5, min: 0, max: 12 }, { key: 'r', label: 'r', default: 3, min: 0, max: 12 }],
  compute: (v) => explainNPr(v.n ?? 0, v.r ?? 0),
  derivation: [
    { tex: '\\text{Fill } r \\text{ ordered slots from } n' },
    { tex: 'n \\times (n-1) \\times \\cdots \\times (n-r+1)', note: 'pool shrinks each slot' },
    { tex: '= \\frac{n!}{(n-r)!}', note: 'the tail (n−r)! cancels' },
  ],
  tricks: ['Order MATTERS → permutation; AB ≠ BA.', 'P(n,n) = n! (arrange everyone).', 'P(n,r) = C(n,r) × r! (choose, then order).'],
};

export const COMBINATION_RULE: RuleDef = {
  id: 'combination', name: 'Combinations: C(n, r)',
  formula: 'C(n,r) = \\binom{n}{r} = \\frac{n!}{r!\\,(n-r)!}',
  analogy: 'Picking a team of r from n: ORDER does NOT matter.',
  inputs: [{ key: 'n', label: 'n', default: 5, min: 0, max: 20 }, { key: 'r', label: 'r', default: 3, min: 0, max: 20 }],
  compute: (v) => explainNCr(v.n ?? 0, v.r ?? 0),
  derivation: [
    { tex: '\\text{Order them first: } P(n,r) = \\frac{n!}{(n-r)!}', note: 'the ordered count' },
    { tex: '\\text{Each team was counted } r! \\text{ times}', note: 'its r! orderings' },
    { tex: 'C(n,r) = \\frac{P(n,r)}{r!} = \\frac{n!}{r!\\,(n-r)!}', note: 'divide out the over-count' },
  ],
  tricks: ['Order does NOT matter → combination; {A,B} = {B,A}.', 'Symmetry: C(n,r) = C(n, n−r) (choose who is OUT).', 'Row n of Pascal’s triangle IS C(n,0) … C(n,n).', '"Overcount by order, then divide it out": C = P ÷ r!.'],
};

export const PERM_REP_RULE: RuleDef = {
  id: 'perm-with-rep', name: 'Permutations with repetition: nʳ',
  formula: '\\#\\text{strings} = n^{r}',
  analogy: 'A 4-digit PIN from 10 digits: each slot independently has all 10 → 10⁴.',
  inputs: [{ key: 'n', label: 'options n', default: 10, min: 1, max: 26 }, { key: 'r', label: 'positions r', default: 4, min: 0, max: 10 }],
  compute: (v) => explainPermWithRep(v.n ?? 1, v.r ?? 0),
  tricks: ['Repetition ALLOWED and order matters → nʳ.', 'It is the rule of product with the SAME n each stage.', 'Counts functions A→B, binary strings (2ⁿ), dice-roll sequences.'],
};

/** The full counting rulebook, in teaching order, for a gallery / rulebook lab. */
export const COUNTING_RULES: RuleDef[] = [
  PRODUCT_RULE, SUM_RULE, FACTORIAL_RULE, PERMUTATION_RULE, COMBINATION_RULE, PERM_REP_RULE,
];
