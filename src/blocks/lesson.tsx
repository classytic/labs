...
export function PredictWidget({
  ...
  const answerIdx = Math.max(
    0,
    list.findIndex((c) => c.correct),
  );
...
export const PredictBlock = defineBlock({
  ...
  description:
    'A predict-first multiple-choice question. The learner commits to an answer; a correct pick reveals the explanation and completes the step (so it can gate a guided lesson). Author your own prompt, choices and explanation, drop it before or after any lab.',
  ...
  Component: ({ attributes, mode, updateAttributes }) => {
    const choices: Choice[] =
      Array.isArray(attributes.choices) && attributes.choices.length ? attributes.choices : DEFAULT_CHOICES;
    const widget = (
      <PredictWidget
    ...
    return (
      <div>
        <ConfigPanel>
          ...
          <ConfigRow label="explain (on correct)">
            <TextField
        ...
        <RowsEditor
          ...
          columns={[
            { key: 'label', label: 'choice', type: 'text', grow: true },
            { key: 'correct', label: 'correct', type: 'bool' },
          ]}
