...
export const pascalTag = (key: string): string =>
  key
    .split('-')
 ...
 */
export type LabBlock = ReturnType<typeof defineBlock> & {
  readonly tag: string;
  readonly lab: (a: any) => ReactNode;
};

export function labBlock<S extends z.ZodObject<any>>({
  key,
  label,
  description,
  schema,
      ...
      return (
        <div>
          <LabConfig
            schema={schema}
            value={attributes}
            onChange={updateAttributes as (patch: Record<string, unknown>) => void}
            omit={omit}
          />
          {widget}
        </div>
