...
describe('combinatorics', () => {
  ...
  it('enumerates a stable Cartesian product for shared builders', () => {
    expect(
      cartesianProduct([
        ['H', 'T'],
        [1, 2],
    ...
    expect(cartesianProduct<string>([])).toEqual([[]]);
  });
...
describe('sets (the logic↔sets spine)', () => {
  ...
  it('subset / equal / powerset / cartesian', () => {
    ...
    expect(cartesian([1, 2], ['a'])).toEqual([
      [1, 'a'],
      [2, 'a'],
    ]);
  });
...
describe('inclusion–exclusion (overcount, then correct)', () => {
  it('|A∪B| with the signed terms', () => {
    const r = inclusionExclusion([
      [1, 2, 3],
      [3, 4],
    ]);
    expect(r.unionSize).toBe(4);
    expect(r.terms.map((t) => [t.indices.length, t.size, t.sign])).toEqual([
      [1, 3, 1],
      [1, 2, 1],
      [2, 1, -1],
    ]); // +|A| +|B| −|A∩B|
