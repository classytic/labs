...
describe('language schemas reject malformed data', () => {
  ...
  it('deck: an item missing the required term is rejected', () => {
    expect(
      deckSchema.safeParse({ termLang: 'en', transLang: 'bn', items: [{ translation: 'বই' }] }).success,
    ).toBe(false);
    expect(
      deckSchema.safeParse({ termLang: 'en', transLang: 'bn', items: [{ term: 'book', translation: 'বই' }] })
        .success,
    ).toBe(true);
  });

  it('deck: rejects blank content, empty decks, and invalid language tags', () => {
    expect(deckSchema.safeParse({ termLang: 'english', transLang: 'bn', items: [] }).success).toBe(false);
    expect(
      deckSchema.safeParse({ termLang: 'en', transLang: 'bn', items: [{ term: ' ', translation: 'বই' }] })
        .success,
    ).toBe(false);
    expect(
      deckSchema.safeParse({
        termLang: 'en-US',
        transLang: 'bn-BD',
        items: [{ term: 'book', translation: 'বই' }],
      }).success,
    ).toBe(true);
  });

  it('agreement: options must be an array of strings', () => {
    expect(agreementItemSchema.safeParse({ subject: 'She', options: 'goes', correct: 'goes' }).success).toBe(
      false,
    );
  });
