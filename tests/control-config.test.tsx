 ...
 * uniformly (Activity composition provides; Field/Control consume by `name ?? label`). One lab
...
describe('controlConfig hides + locks individual knobs', () => {
  ...
  it('hide also works on non-Field controls via <Control> (the components toggle)', () => {
    ...
    expect([...visible.container.querySelectorAll('*')].some((n) => n.textContent === 'components')).toBe(
      true,
    );
    const hidden = render(<RampForcesLab controlConfig={{ hide: ['components'] }} />);
    expect([...hidden.container.querySelectorAll('*')].some((n) => n.textContent === 'components')).toBe(
      false,
    );
  });
});
