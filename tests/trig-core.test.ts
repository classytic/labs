...
describe('trig kernel: quadrants, signs, CAST', () => {
  ...
  it('signs follow CAST (All / Sin / Tan / Cos)', () => {
    expect(sign('sin', 30)).toBe(1);
    expect(sign('cos', 30)).toBe(1);
    expect(sign('tan', 30)).toBe(1); // Q1 all +
    expect(sign('sin', 150)).toBe(1);
    expect(sign('cos', 150)).toBe(-1);
    expect(sign('tan', 150)).toBe(-1); // Q2 sin +
    expect(sign('tan', 210)).toBe(1);
    expect(sign('sin', 210)).toBe(-1); // Q3 tan +
    expect(sign('cos', 300)).toBe(1);
    expect(sign('sin', 300)).toBe(-1); // Q4 cos +
