...
import { explainDeMoivre } from '../src/math/complex/rules.js';
...
describe('De Moivre teaching model', () => {
  it('raises the modulus and multiplies the angle without changing the kernel result', () => {
    const worked = explainDeMoivre(2, 30, 3);
    expect(worked.value).toBe(8);
    expect(worked.steps.at(-1)?.tex).toContain('90');
  });
});
