...
describe('answer-check · scientific notation', () => {
  it('reads 3.0e8 as one number, not 3.0 times Euler', () => {
    // Physics answers are written this way. Implicit multiplication used to split them, so a
    // student typing the speed of light was told they were wrong.
    expect(checkAnswer({ kind: 'number', value: 3.0e8 }, '3.0e8')).toBe(true);
    expect(checkAnswer({ kind: 'number', value: 6.2e-21 }, '6.2e-21')).toBe(true);
    expect(checkAnswer({ kind: 'number', value: 300 }, '3.0E2')).toBe(true);
    expect(checkAnswer({ kind: 'number', value: 3.0e8 }, '3.0e9')).toBe(false);
  });
  it('still multiplies where the letter is not an exponent', () => {
    expect(parseValue('2e')).toBeCloseTo(2 * Math.E, 6);
    expect(parseValue('2exp(1)')).toBeCloseTo(2 * Math.E, 6);
    expect(parseValue('2pi')).toBeCloseTo(2 * Math.PI, 6);
  });
});
