...
const circMeta = (doc: ReturnType<typeof circuitDoc>) => {
  ...
  return g.meta as unknown as {
    solved: boolean;
    Itotal: number;
    comps: { type: string; brightness: number }[];
  };
};
...
describe('labs templates on the stage engine', () => {
  ...
  it('balance-lever balances when torques match (4·3 = x·2 ⇒ x=6)', () => {
    const doc = leverBalanceDoc({
      items: [
        { side: 'L', dist: 3, weight: 4 },
        { side: 'R', dist: 2, weight: 'unknown' },
      ],
      start: 1,
    });
  ...
  it('circuit: two parallel bulbs both light (series-parallel reduction)', () => {
    const doc = circuitDoc({
      emf: 6,
      branches: [[{ type: 'bulb', ohms: 6 }], [{ type: 'bulb', ohms: 6 }]],
      goal: { kind: 'allLit' },
    });
