...
Status: implemented and release-hardened. Author: pairing session 2026-06-18.
...
Browser voices vary by operating system, installed voice packs, and language.
**Browser TTS is a progressive fallback, not a guaranteed capability.** Creators
can provide an uploaded or external recording per item. The lab never creates
audio and must expose an unavailable/error state when neither source can play.

Native `Translator` and `LanguageDetector` APIs are deliberately outside the
learner runtime: their support and model availability vary, and generated text
must never silently become an assessed answer key. A host/CMS may offer them as
an optional authoring aid behind feature detection, download progress, explicit
creator review, and a manual fallback. Core Labs accepts only reviewed deck data.
